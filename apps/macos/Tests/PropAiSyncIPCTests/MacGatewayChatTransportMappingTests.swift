...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: PropAiSyncProtocol.AnyCodable(["ok": PropAiSyncProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: PropAiSyncProtocol.AnyCodable(["ok": PropAiSyncProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = PropAiSyncProtocol.AnyCodable([
            "runId": PropAiSyncProtocol.AnyCodable("run-1"),
            "sessionKey": PropAiSyncProtocol.AnyCodable("main"),
            "state": PropAiSyncProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: PropAiSyncProtocol.AnyCodable(["a": PropAiSyncProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
