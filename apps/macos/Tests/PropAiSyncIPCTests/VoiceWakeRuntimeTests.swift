...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "PropAi Sync"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["PropAi Sync"]
        let text = "hey PropAi Sync"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey PropAi Sync do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["PropAi Sync"], minPostTriggerGap: 0.3)
