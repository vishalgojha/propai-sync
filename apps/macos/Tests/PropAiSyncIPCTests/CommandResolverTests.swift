...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() throws {
        ...
        let PropAiSyncPath = tmp.appendingPathComponent("node_modules/.bin/PropAiSync")
        try makeExecutableForTests(at: PropAiSyncPath)

        let cmd = CommandResolver.propaiCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([PropAiSyncPath.path, "gateway"]))
    }
    ...
    @Test func `prefers open claw binary over pnpm`() throws {
        ...
        let PropAiSyncPath = binDir.appendingPathComponent("PropAi Sync")
        ...
        #expect(cmd.prefix(2).elementsEqual([PropAiSyncPath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "PropAi Sync", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "PropAi Sync", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("propai@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/PropAiSync", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "propai@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/PropAiSync'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("PropAi Sync"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == PropAiSyncPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
