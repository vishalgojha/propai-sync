...
struct MacNodeRuntimeTests {
    ...
    @Test func `handle invoke rejects empty system run`() async throws {
        let runtime = MacNodeRuntime()
        let params = PropAiSyncSystemRunParams(command: [])
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2", command: PropAiSyncSystemCommand.run.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }

    @Test func `handle invoke rejects empty system which`() async throws {
        let runtime = MacNodeRuntime()
        let params = PropAiSyncSystemWhichParams(bins: [])
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2b", command: PropAiSyncSystemCommand.which.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }

    @Test func `handle invoke rejects empty notification`() async throws {
        let runtime = MacNodeRuntime()
        let params = PropAiSyncSystemNotifyParams(title: "", body: "")
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-3", command: PropAiSyncSystemCommand.notify.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }
    ...
    @Test func `handle invoke camera list requires enabled camera`() async {
        await TestIsolation.withUserDefaultsValues([cameraEnabledKey: false]) {
            let runtime = MacNodeRuntime()
            let response = await runtime.handleInvoke(
                BridgeInvokeRequest(id: "req-4", command: PropAiSyncCameraCommand.list.rawValue))
            #expect(response.ok == false)
        ...
        final class FakeMainActorServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
                ...
                outPath: String?) async throws -> (path: String, hasAudio: Bool)
            {
                let url = FileManager().temporaryDirectory
                    .appendingPathComponent("propai-test-screen-record-\(UUID().uuidString).mp4")
    ...
    @Test func `handle invoke browser proxy uses injected request`() async {
        ...
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(
                id: "req-browser",
                command: PropAiSyncBrowserCommand.proxy.rawValue,
    ...
    @Test func `handle invoke browser proxy rejects disabled browser control`() async throws {
        ...
        try await TestIsolation.withEnvValues(["PROPAI_CONFIG_PATH": override]) {
