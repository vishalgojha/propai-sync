...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: PropAiSyncProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: PropAiSyncProtocol.AnyCodable] = [
            "presence": .init([PropAiSyncProtocol.AnyCodable(entry)]),
        ]
        let payload = PropAiSyncProtocol.AnyCodable(payloadMap)
