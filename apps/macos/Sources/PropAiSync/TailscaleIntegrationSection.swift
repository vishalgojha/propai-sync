...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.propai/propai.json. Prefer PROPAI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.propai/propai.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.propai/propai.json. Restart the gateway to apply."
        }
