...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.propai", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = PropAiSyncConfigFile.hostKey(sshHost)
        let urlKey = PropAiSyncConfigFile.hostKey(host)
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.propai.remote.tunnel.port", qos: .utility)
            do {
