...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PropAiSyncProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.propai", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PropAiSyncKit.AnyCodable]()) {
                $0[$1.key] = PropAiSyncKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: PropAiSyncProtocol.AnyCodable?) -> [String: PropAiSyncProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PropAiSyncProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PropAiSyncKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PropAiSyncProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("PropAiSync.control.heartbeat")
    static let controlAgentEvent = Notification.Name("PropAiSync.control.agent")
}
