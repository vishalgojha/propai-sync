...
enum PropAiSyncLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = PropAiSyncOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PropAiSyncFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PropAiSyncLogging.bootstrapIfNeeded()
        let label = PropAiSyncLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PropAiSyncOSLogHandler: AppLogLevelBackedHandler {
...
struct PropAiSyncFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PropAiSyncLogging.parseLabel(self.label)
