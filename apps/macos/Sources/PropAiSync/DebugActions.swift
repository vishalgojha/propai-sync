...
enum DebugActions {
    private static let verboseDefaultsKey = "PropAiSync.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "PropAiSync.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = PropAiSyncPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "PropAi Sync", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = PropAiSyncPaths.configURL
        guard
