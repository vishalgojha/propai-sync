...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("PropAi Sync").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing PropAi Sync CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed PropAi Sync \($0)." } ?? "Installed PropAiSync."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://PropAiSync.bot/install-cli.sh | \
