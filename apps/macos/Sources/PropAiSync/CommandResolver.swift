...
enum CommandResolver {
    private static let projectRootDefaultsKey = "PropAiSync.gatewayProjectRootPath"
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/PropAiSync")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let PropAiSyncPaths = self.propaiManagedPaths(home: home)
        if !PropAiSyncPaths.isEmpty {
            extras.insert(contentsOf: PropAiSyncPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + PropAiSyncPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func PropAiSyncManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".propai"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func PropAiSyncExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectPropAiSyncExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func hasAnyPropAiSyncInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.propaiExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func PropAiSyncNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let PropAiSyncPath = self.projectPropAiSyncExecutable(projectRoot: root) {
            return [PropAiSyncPath, subcommand] + extraArgs
        }
        if let PropAiSyncPath = self.propaiExecutable(searchPaths: searchPaths) {
            return [PropAiSyncPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "PropAi Sync", subcommand] + extraArgs
        }
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real PropAi Sync CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/PropAiSync"
    ...
    {
        let root = configRoot ?? PropAiSyncConfigFile.loadDict()
