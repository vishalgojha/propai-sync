...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["PROPAI_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/PropAiSync")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("propai-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("propai-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/propai/ (rolling or stdout), or nil if none exist.
