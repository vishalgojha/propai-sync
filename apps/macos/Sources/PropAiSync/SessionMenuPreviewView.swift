...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.propai", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PropAiSyncSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PropAiSyncSessionPreviewEntry,
    ...
    private static func cache(payload: PropAiSyncSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PropAiSyncChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PropAiSyncKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PropAiSyncKit.AnyCodable]) -> [PropAiSyncChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PropAiSyncChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PropAiSyncChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PropAiSyncChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PropAiSyncChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PropAiSyncChatMessage) -> String? {
