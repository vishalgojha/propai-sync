...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.propai", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = PropAiSyncConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: PropAiSyncNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [PropAiSyncCapability.canvas.rawValue, PropAiSyncCapability.screen.rawValue]
        if PropAiSyncConfigFile.browserControlEnabled() {
            caps.append(PropAiSyncCapability.browser.rawValue)
        }
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(PropAiSyncCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if PropAiSyncLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(PropAiSyncCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            PropAiSyncSystemCommand.which.rawValue,
            PropAiSyncSystemCommand.run.rawValue,
            PropAiSyncSystemCommand.execApprovalsGet.rawValue,
            PropAiSyncSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(PropAiSyncCapability.browser.rawValue) {
            commands.append(PropAiSyncBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(PropAiSyncCapability.camera.rawValue) {
        ...
        if capsSet.contains(PropAiSyncCapability.location.rawValue) {
