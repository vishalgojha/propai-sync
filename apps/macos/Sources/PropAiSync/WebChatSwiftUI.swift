...
private let webChatSwiftLogger = Logger(subsystem: "ai.propai", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "PropAiSync.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: PropAiSyncChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> PropAiSyncChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(PropAiSyncChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> PropAiSyncChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                PropAiSyncGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    PropAiSyncChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    PropAiSyncAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: PropAiSyncProtocol.ModelChoice) -> PropAiSyncChatModelChoice {
        PropAiSyncChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<PropAiSyncChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any PropAiSyncChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = PropAiSyncChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: PropAiSyncChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<PropAiSyncChatView>) -> NSViewController
    {
