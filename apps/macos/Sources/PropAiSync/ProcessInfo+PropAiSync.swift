...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.PROPAI.mac`) even if the shipped
    ...
    {
        if environment["PROPAI_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "PROPAI.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "PROPAI.nixMode") { return true }
