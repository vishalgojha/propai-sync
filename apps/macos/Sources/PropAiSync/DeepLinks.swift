...
private let deepLinkLogger = Logger(subsystem: "ai.propai", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "PropAi Sync is paused", message: "Unpause PropAi Sync to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run PropAi Sync agent?", message: body) else { return }
        }
