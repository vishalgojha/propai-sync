    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so PropAi Sync can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = PropAiSyncLocationMode.off.rawValue
    ...
    private var locationMode: PropAiSyncLocationMode {
        PropAiSyncLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: PropAiSyncLocationMode) async -> Bool {
