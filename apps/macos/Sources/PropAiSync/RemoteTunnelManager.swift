...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.propai", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous PropAi Sync run already has an SSH listener on the expected port (common after restarts),
