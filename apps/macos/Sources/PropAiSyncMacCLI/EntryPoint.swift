...
struct PropAiSyncMacCLI {
    static func main() async {
        ...
        default:
            fputs("propai-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      propai-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      propai-mac discover [--timeout <ms>] [--json] [--include-local]
      propai-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      propai-mac connect
      propai-mac connect --url ws://127.0.0.1:18789 --json
      propai-mac discover --timeout 3000 --json
      propai-mac wizard --mode local
    """)
}
