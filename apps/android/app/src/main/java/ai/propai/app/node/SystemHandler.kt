...
private const val NOTIFICATION_CHANNEL_BASE_ID = "PropAiSync.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "PropAi Sync Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "PropAi Sync Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "PropAi Sync Active")
      }
