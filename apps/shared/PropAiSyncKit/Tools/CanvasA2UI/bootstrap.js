...
import "@a2ui/lit/ui";
import { themeContext } from "@propai/a2ui-theme-context";
...
class PropAiSyncA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--propai-a2ui-inset-top, 0px)
        var(--propai-a2ui-inset-right, 0px)
        var(--propai-a2ui-inset-bottom, 0px)
        var(--propai-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--propai-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--propai-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--propai-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--propai-a2ui-empty-top, var(--propai-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.propaiA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["PropAi Sync:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__PropAiSyncLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.propaiCanvasA2UIAction ??
      globalThis.propaiCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.propaiCanvasA2UIAction) {
...
if (!customElements.get("propai-a2ui-host")) {
  customElements.define("propai-a2ui-host", PropAiSyncA2UIHost);
}
