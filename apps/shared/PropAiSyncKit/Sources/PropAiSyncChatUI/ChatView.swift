...
@MainActor
public struct PropAiSyncChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: PropAiSyncChatViewModel
    ...
    public init(
        viewModel: PropAiSyncChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                PropAiSyncChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [PropAiSyncChatMessage] {
        let base: [PropAiSyncChatMessage]
    ...
    private func mergeToolResults(in messages: [PropAiSyncChatMessage]) -> [PropAiSyncChatMessage] {
        ...
        for message in messages {
            ...
            let merged = PropAiSyncChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: PropAiSyncChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: PropAiSyncChatMessage) -> Bool {
    ...
    private func primaryText(in message: PropAiSyncChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: PropAiSyncChatMessage) -> Bool {
    ...
    private func toolCalls(in message: PropAiSyncChatMessage) -> [PropAiSyncChatMessageContent] {
    ...
    private func inlineToolResults(in message: PropAiSyncChatMessage) -> [PropAiSyncChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: PropAiSyncChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: PropAiSyncChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(PropAiSyncChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
