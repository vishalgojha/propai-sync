...
struct ChatMessageBubble: View {
    let message: PropAiSyncChatMessage
    let style: PropAiSyncChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? PropAiSyncChatTheme.userText : PropAiSyncChatTheme.assistantText
    ...
    private var inlineAttachments: [PropAiSyncChatMessageContent] {
    ...
    private var toolCalls: [PropAiSyncChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [PropAiSyncChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? PropAiSyncChatTheme.userBubble
        }
        if self.style == .onboarding {
            return PropAiSyncChatTheme.onboardingAssistantBubble
        }
        return PropAiSyncChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return PropAiSyncChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: PropAiSyncChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? PropAiSyncChatTheme.userText : PropAiSyncChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: PropAiSyncChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(PropAiSyncChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [PropAiSyncChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: PropAiSyncChatTheme.assistantText)
            }
        }
    }
}
