...
private let chatUILogger = Logger(subsystem: "ai.propai", category: "PropAiSyncChatUI")

@MainActor
@Observable
public final class PropAiSyncChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [PropAiSyncChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [PropAiSyncChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [PropAiSyncPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [PropAiSyncChatPendingToolCall] = []
    public private(set) var sessions: [PropAiSyncChatSessionEntry] = []
    private let transport: any PropAiSyncChatTransport
    private var sessionDefaults: PropAiSyncChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: PropAiSyncChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any PropAiSyncChatTransport,
    ...
    public var sessionChoices: [PropAiSyncChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [PropAiSyncChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: PropAiSyncPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [PropAiSyncChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: PropAiSyncChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: PropAiSyncChatMessage) -> PropAiSyncChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> PropAiSyncChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return PropAiSyncChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: PropAiSyncChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [PropAiSyncChatMessage],
        incoming: [PropAiSyncChatMessage]) -> [PropAiSyncChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [PropAiSyncChatMessage]) -> [PropAiSyncChatMessage] {
        var result: [PropAiSyncChatMessage] = []
    ...
    private static func dedupeKey(for message: PropAiSyncChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [PropAiSyncChatMessageContent] = [
            PropAiSyncChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> PropAiSyncChatAttachmentPayload in
            PropAiSyncChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> PropAiSyncChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = PropAiSyncChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: PropAiSyncChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: PropAiSyncChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: PropAiSyncAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = PropAiSyncChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> PropAiSyncPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
