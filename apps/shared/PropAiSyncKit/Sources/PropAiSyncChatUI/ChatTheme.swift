...
enum PropAiSyncChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("PropAiSyncChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("PropAiSyncChatTheme.onboardingAssistantBubble"),
...
enum PropAiSyncPlatformImageFactory {
    static func image(_ image: PropAiSyncPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
