...
import Foundation
import Testing
@testable import PropAiSyncChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> PropAiSyncChatHistoryPayload
{
    PropAiSyncChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> PropAiSyncChatSessionEntry {
    PropAiSyncChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> PropAiSyncChatSessionEntry
{
    PropAiSyncChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> PropAiSyncChatModelChoice {
    PropAiSyncChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [PropAiSyncChatHistoryPayload],
    sessionsResponses: [PropAiSyncChatSessionsListResponse] = [],
    modelResponses: [[PropAiSyncChatModelChoice]] = [],
    setSessionModelHook: (@Sendable (String?) async throws -> Void)? = nil,
    setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil,
    initialThinkingLevel: String? = nil,
    onThinkingLevelChanged: (@MainActor @Sendable (String) -> Void)? = nil) async
    -> (TestChatTransport, PropAiSyncChatViewModel)
{
...
private func sendUserMessage(_ vm: PropAiSyncChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, PropAiSyncChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [PropAiSyncChatHistoryPayload]
    private let sessionsResponses: [PropAiSyncChatSessionsListResponse]
    private let modelResponses: [[PropAiSyncChatModelChoice]]
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<PropAiSyncChatTransportEvent>
    private let continuation: AsyncStream<PropAiSyncChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<PropAiSyncChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> PropAiSyncChatHistoryPayload {
        ...
        return self.historyResponses.last ?? PropAiSyncChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [PropAiSyncChatAttachmentPayload]) async throws -> PropAiSyncChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return PropAiSyncChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> PropAiSyncChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? PropAiSyncChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: PropAiSyncChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = PropAiSyncChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = PropAiSyncChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: PropAiSyncChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(PropAiSyncChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == PropAiSyncChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = PropAiSyncChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: PropAiSyncChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = PropAiSyncChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = PropAiSyncChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = PropAiSyncChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { PropAiSyncChatViewModel(sessionKey: "main", transport: transport) }
