...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.propai.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to PropAi Sync", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to PropAi Sync gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to PropAiSync.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "PropAiSyncShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "PropAi Sync is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "PropAi Sync Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("propai-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
