...
@MainActor
final class PropAiSyncAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.propai.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.propai.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.propai.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "PropAiSync.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "PropAiSync.watch.promptId"
    static let sessionKeyKey = "PropAiSync.watch.sessionKey"
    static let actionPrimaryIDKey = "PropAiSync.watch.action.primary.id"
    static let actionPrimaryLabelKey = "PropAiSync.watch.action.primary.label"
    static let actionSecondaryIDKey = "PropAiSync.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "PropAiSync.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "PropAiSync.watch.action.primary"
    static let actionSecondaryIdentifier = "PropAiSync.watch.action.secondary"
    static let actionIdentifierPrefix = "PropAiSync.watch.action."
    static let actionIDKeyPrefix = "PropAiSync.watch.action.id."
    static let actionLabelKeyPrefix = "PropAiSync.watch.action.label."
    static let categoryPrefix = "PropAiSync.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: PropAiSyncWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> PropAiSyncWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return PropAiSyncWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "PropAi Sync" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [PropAiSyncWatchAction]) -> [UNNotificationAction] {
...
@main
struct PropAiSyncApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(PropAiSyncAppDelegate.self) private var appDelegate
...
extension PropAiSyncApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("PropAi Sync: installing uncaught exception handler")
