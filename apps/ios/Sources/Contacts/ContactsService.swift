...
final class ContactsService: ContactsServicing {
    ...
    func search(params: PropAiSyncContactsSearchParams) async throws -> PropAiSyncContactsSearchPayload {
        ...
        return PropAiSyncContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: PropAiSyncContactsAddParams) async throws -> PropAiSyncContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return PropAiSyncContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return PropAiSyncContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> PropAiSyncContactPayload {
        PropAiSyncContactPayload(
