...
final class MotionService: MotionServicing {
    func activities(params: PropAiSyncMotionActivityParams) async throws -> PropAiSyncMotionActivityPayload {
        ...
        let mapped: [PropAiSyncMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return PropAiSyncMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: PropAiSyncPedometerParams) async throws -> PropAiSyncPedometerPayload {
        ...
        let payload: PropAiSyncPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = PropAiSyncPedometerPayload(
