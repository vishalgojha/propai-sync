...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.propai.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.propai.ios", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.propai.ios", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.propai.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.propai.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = PropAiSyncCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = PropAiSyncCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let message = PropAiSyncCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = PropAiSyncCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: PropAiSyncLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(PropAiSyncGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: PropAiSyncNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: PropAiSyncNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PropAiSyncNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(PropAiSyncLocationGetParams.self, from: req.paramsJSON)) ??
            PropAiSyncLocationGetParams()
        ...
        let payload = PropAiSyncLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(PropAiSyncCanvasPresentParams.self, from: req.paramsJSON)) ??
                PropAiSyncCanvasPresentParams()
        ...
        case PropAiSyncCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PropAiSyncCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(PropAiSyncCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PropAiSyncCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(PropAiSyncCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PropAiSyncCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(PropAiSyncCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case PropAiSyncCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.propaiA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing PropAiSyncA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case PropAiSyncCanvasA2UICommand.push.rawValue, PropAiSyncCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [PropAiSyncKit.AnyCodable]
            if command == PropAiSyncCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(PropAiSyncCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try PropAiSyncCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(PropAiSyncCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try PropAiSyncCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.propaiA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PropAiSyncCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(PropAiSyncCameraSnapParams.self, from: req.paramsJSON)) ??
                PropAiSyncCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PropAiSyncCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(PropAiSyncCameraClipParams.self, from: req.paramsJSON)) ??
                PropAiSyncCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(PropAiSyncScreenRecordParams.self, from: req.paramsJSON)) ??
            PropAiSyncScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(PropAiSyncSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PropAiSyncNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PropAiSyncNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PropAiSyncNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(PropAiSyncChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PropAiSyncNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = PropAiSyncChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(PropAiSyncPhotosLatestParams.self, from: req.paramsJSON)) ??
            PropAiSyncPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(PropAiSyncContactsSearchParams.self, from: req.paramsJSON)) ??
                PropAiSyncContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncContactsCommand.add.rawValue:
            let params = try Self.decodeParams(PropAiSyncContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(PropAiSyncCalendarEventsParams.self, from: req.paramsJSON)) ??
                PropAiSyncCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(PropAiSyncCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(PropAiSyncRemindersListParams.self, from: req.paramsJSON)) ??
                PropAiSyncRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(PropAiSyncRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(PropAiSyncMotionActivityParams.self, from: req.paramsJSON)) ??
                PropAiSyncMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(PropAiSyncPedometerParams.self, from: req.paramsJSON)) ??
                PropAiSyncPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([PropAiSyncLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([PropAiSyncScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([PropAiSyncSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([PropAiSyncChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            PropAiSyncWatchCommand.status.rawValue,
            PropAiSyncWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([PropAiSyncPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            PropAiSyncRemindersCommand.list.rawValue,
            PropAiSyncRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PropAiSyncWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = PropAiSyncWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PropAiSyncWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(PropAiSyncWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = PropAiSyncWatchNotifyPayload(
    ...
    func locationMode() -> PropAiSyncLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return PropAiSyncLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to PropAiSync."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to PropAiSync."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "propai-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "PropAi Sync Share Self-Test",
            url: URL(string: "https://propai.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.propaiPushKind(userInfo)
        let receivedMessage =
    ...
    private static func PropAiSyncPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["PropAi Sync"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["PropAi Sync"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
