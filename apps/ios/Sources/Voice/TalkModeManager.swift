...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__PROPAI_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<PropAiSyncTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.propai", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = PropAiSyncTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> PropAiSyncTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return PropAiSyncTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> PropAiSyncTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> PropAiSyncTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return PropAiSyncTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> PropAiSyncTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = PropAiSyncTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: PropAiSyncTalkPTTStopPayload) {
