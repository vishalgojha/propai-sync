...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = PropAiSyncLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = PropAiSyncLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.propai.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
                ...
                Section {
                    DisclosureGroup(isExpanded: self.$gatewayExpanded) {
                        if !self.isGatewayConnected {
                            Text(
                                "1. Open a chat with your PropAi Sync agent and send /pair\n"
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        self.featureToggle(
                            ...
                            help: "Enables voice conversation mode with your connected PropAi Sync agent.") { newValue in
                        ...
                        self.featureToggle(
                            ...
                            help: "Allows the gateway to request photos or short video clips "
                                + "while PropAi Sync is foregrounded."
                        )
                        ...
                        HStack(spacing: 8) {
                            ...
                            Button {
                                self.activeFeatureHelp = FeatureHelp(
                                    ...
                                    message: "Controls location permissions for PropAiSync. "
                        ...
                        Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                            Text("Off").tag(PropAiSyncLocationMode.off.rawValue)
                            Text("While Using").tag(PropAiSyncLocationMode.whileUsing.rawValue)
                            Text("Always").tag(PropAiSyncLocationMode.always.rawValue)
                        }
                        .labelsHidden()
                        ...
                        self.featureToggle(
                            ...
                            help: "Keeps the screen awake while PropAi Sync is open.")
                    ...
                    DisclosureGroup("Device Info") {
                        ...
                        LabeledContent("PropAi Sync", value: DeviceInfoHelper.propaiVersionString())
                    }
                }
            }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = PropAiSyncLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Go back to your PropAi Sync chat and run /pair approve, then tap Connect again."
        }
