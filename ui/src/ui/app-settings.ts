...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as PropAiSyncApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as PropAiSyncApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as PropAiSyncApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as PropAiSyncApp);
    await loadToolsCatalog(host as unknown as PropAiSyncApp);
    await loadConfig(host as unknown as PropAiSyncApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as PropAiSyncApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as PropAiSyncApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as PropAiSyncApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as PropAiSyncApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as PropAiSyncApp);
    await loadDevices(host as unknown as PropAiSyncApp);
    await loadConfig(host as unknown as PropAiSyncApp);
    await loadExecApprovals(host as unknown as PropAiSyncApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as PropAiSyncApp);
    await loadConfig(host as unknown as PropAiSyncApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as PropAiSyncApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as PropAiSyncApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__PROPAI_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as PropAiSyncApp, false),
    loadPresence(host as unknown as PropAiSyncApp),
    loadSessions(host as unknown as PropAiSyncApp),
    loadCronStatus(host as unknown as PropAiSyncApp),
    loadDebug(host as unknown as PropAiSyncApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as PropAiSyncApp, true),
    loadConfigSchema(host as unknown as PropAiSyncApp),
    loadConfig(host as unknown as PropAiSyncApp),
  ]);
}

export async function loadCron(host: SettingsHost) {
  const cronHost = host as unknown as PropAiSyncApp;
  await Promise.all([
    loadChannels(host as unknown as PropAiSyncApp, false),
