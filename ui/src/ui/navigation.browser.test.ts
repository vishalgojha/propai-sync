...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/propai/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/PropAiSync");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/propai/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PROPAI_CONTROL_UI_BASE_PATH__ = "/PropAiSync";
    const app = mountApp("/propai/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/PropAiSync");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/propai/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("PropAiSync.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "PropAiSync.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/PropAiSync" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("PropAiSync.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/PropAiSync",
    });
    expect(JSON.parse(localStorage.getItem("PropAiSync.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/PropAiSync";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/PropAiSync");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/PropAiSync#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/PropAiSync");
    expect(app.settings.token).toBe("");
