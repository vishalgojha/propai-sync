...
- TUI/chat log: reuse the active assistant message component for the same streaming run so `propai tui` no longer renders duplicate assistant replies. (#35364) Thanks @lisitan.
- macOS/Reminders: add the missing `NSRemindersUsageDescription` to the bundled app so `apple-reminders` can trigger the system permission prompt from PropAi Sync.app. (#8559) Thanks @dinakars777.
...
- Exec/child commands: mark child command environments with `PROPAI_CLI` so subprocesses can detect when they were launched from the PropAi Sync CLI. (#41411) Thanks @vincentkoc.
...
- Cron/doctor: tighten isolated cron delivery so cron jobs can no longer notify through ad hoc agent sends or fallback main-session summaries, and add `propai doctor --fix` migration for legacy cron storage and legacy notify/webhook delivery metadata. (#40998) Thanks @mbelinky.
...
- CLI/tables: default shared tables to ASCII borders on legacy Windows consoles while keeping Unicode borders on modern Windows terminals, so commands like `propai skills` stop rendering mojibake under GBK/936 consoles. Fixes #40853. Related #41015. Thanks @ApacheBin and @vincentkoc.
...
- Security/system.run: fail closed for approval-backed interpreter/runtime commands when PropAi Sync cannot bind exactly one concrete local file operand, while extending best-effort direct-file binding to additional runtime forms. Thanks @tdjackey for reporting.
...
- CLI/backup: add `propai backup create` and `propai backup verify` for local state archives, including `--only-config`, `--no-include-workspace`, manifest/payload validation, and backup guidance in destructive flows. (#40163) thanks @shichangs.
...
- CLI/install: include the short git commit hash in `propai --version` output when metadata is available, and keep installer version checks compatible with the decorated format. (#39712) thanks @sourman.
...
- ACP/Provenance: add optional ACP ingress provenance metadata and visible receipt injection (`propai acp --provenance off|meta|meta+receipt`) so PropAi Sync agents can retain and report ACP-origin context with session trace IDs. (#40473) thanks @mbelinky.
...
- Update/macOS launchd restart: re-enable disabled LaunchAgent services before updater bootstrap so `propai update` can recover from a disabled gateway service instead of leaving the restart step stuck.
...
- TUI/theme: detect light terminal backgrounds via `COLORFGBG` and pick a WCAG AA-compliant light palette, with `PROPAI_THEME=light|dark` override for terminals without auto-detection. (#38636) Thanks @ademczuk and @vincentkoc.
...
- Podman/SELinux: auto-detect SELinux enforcing/permissive mode and add `:Z` relabel to bind mounts in `run-propai-podman.sh` and the Quadlet template, fixing `EACCES` on Fedora/RHEL hosts. Supports `PROPAI_BIND_MOUNT_OPTIONS` override. (#39449) Thanks @langdon and @githubbzxs.
...
- CLI/skills tables: keep terminal table borders aligned for wide graphemes, use full reported terminal width, and switch a few ambiguous skill icons to Terminal-safe emoji so `propai skills` renders more consistently in Terminal.app and iTerm. Thanks @vincentkoc.
...
- Docker/Podman extension dependency baking: add `PROPAI_EXTENSIONS` so container builds can preinstall selected bundled extension npm dependencies into the image for faster and more reproducible startup in container deployments. (#32223) Thanks @sallyom.
...
- iOS/App Store Connect release prep: align iOS bundle identifiers under `ai.propai.client`, refresh Watch app icons, add Fastlane metadata/screenshot automation, and support Keychain-backed ASC auth for uploads. (#38936) Thanks @ngutman.
...
- Docker/multi-stage build: restructure Dockerfile as a multi-stage build to produce a minimal runtime image without build tools, source code, or Bun; add `PROPAI_VARIANT=slim` build arg for a bookworm-slim variant. (#38479) Thanks @sallyom.
...
- Gateway/Telegram stale-socket restart guard: only apply stale-socket restarts to channels that publish event-liveness timestamps, preventing Telegram providers from being misclassified as stale solely due to long uptime and avoiding restart/pairing storms after upgrade. (PropAi Sync#38464)
...
- Tools/xAI native web-search collision guard: drop PropAi Sync `web_search` from tool registration when routing to xAI/Grok model providers (including OpenRouter `x-ai/*`) to avoid duplicate tool-name request failures against provider-native `web_search`. (#14749) Thanks @realsamrat.
...
- WhatsApp/self-chat response prefix fallback: stop forcing `"[PropAi Sync]"` as the implicit outbound response prefix when no identity name or response prefix is configured, so blank/default prefix settings no longer inject branding text unexpectedly in self-chat flows. (#27962) Thanks @ecanmor.
...
- Gateway/remote WS break-glass hostname support: honor `PROPAI_ALLOW_INSECURE_PRIVATE_WS=1` for `ws://` hostname URLs (not only private IP literals) across onboarding validation and runtime gateway connection checks, while still rejecting public IP literals and non-unicast IPv6 endpoints. (#36930) Thanks @manju-rn.
...
- Agents/context pruning: guard assistant thinking/text char estimation against malformed blocks (missing `thinking`/`text` strings or null entries) so pruning no longer crashes with malformed provider content. (PropAi Sync#35146) thanks @Sid-Qin.
...
- Agents/schema cleaning: detect Venice + Grok model IDs as xAI-proxied targets so unsupported JSON Schema keywords are stripped before requests, preventing Venice/Grok `Invalid arguments` failures. (PropAi Sync#35355) thanks @Sid-Qin.
...
- TUI/session-key canonicalization: normalize `propai tui --session` values to lowercase so uppercase session names no longer drop real-time streaming updates due to gateway/TUI key mismatches. (#33866, #34013) thanks @lynnzc.
...
- Plugins/startup loading: lazily initialize plugin runtime, split startup-critical plugin SDK imports into `propai/plugin-sdk/core` and `propai/plugin-sdk/telegram`, and preserve `api.runtime` reflection semantics for plugin compatibility. (#28620) thanks @hmemcpy.
- Plugins/startup performance: reduce bursty plugin discovery/manifest overhead with short in-process caches, skip importing bundled memory plugins that are disabled by slot selection, and speed legacy root `propai/plugin-sdk` compatibility via runtime root-alias routing while preserving backward compatibility. Thanks @gumadeiras.
...
- Gateway/password CLI hardening: add `propai gateway run --password-file`, warn when inline `--password` is used because it can leak via process listings, and document env/file-backed password input as the preferred startup path. Fixes #27948. Thanks @vibewrk and @vincentkoc.
...
- Google/Gemini Flash model selection: switch built-in `gemini-flash` defaults and docs/examples from the nonexistent `google/gemini-3.1-flash-preview` ID to the working `google/gemini-3-flash-preview`, while normalizing legacy PropAi Sync config that still uses the old Flash 3.1 alias.
- Plugins/bundled scoped-import migration: migrate bundled plugins from monolithic `propai/plugin-sdk` imports to scoped subpaths (or `propai/plugin-sdk/core`) across registration and startup-sensitive runtime files, add CI/release guardrails to prevent regressions, and keep root `propai/plugin-sdk` support for external/community plugins. Thanks @gumadeiras.
...
- Telegram/multi-account default routing clarity: warn only for ambiguous (2+) account setups without an explicit default, add `propai doctor` warnings for missing/invalid multi-account defaults across channels, and document explicit-default guidance for channel routing and Telegram config. (#32544) thanks @Sid-Qin.
...
- Gateway/status self version reporting: make Gateway self version in `propai status` prefer runtime `VERSION` (while preserving explicit `PROPAI_VERSION` override), preventing stale post-upgrade app version output. (#32655) thanks @liuxiaopai-ai.
...
- Mattermost/plugin SDK import policy: replace remaining monolithic `propai/plugin-sdk` imports in Mattermost mention-gating paths/tests with scoped subpaths (`propai/plugin-sdk/compat` and `propai/plugin-sdk/mattermost`) so `pnpm check` passes `lint:plugins:no-monolithic-plugin-sdk-entry-imports` on baseline. (#36480) Thanks @Takhoffman.
...
- Windows/Plugin install: when PropAi Sync runs on Windows via Bun and `npm-cli.js` is not colocated with the runtime binary, fall back to `npm.cmd`/`npx.cmd` through the existing `cmd.exe` wrapper so `propai plugins install` no longer fails with `spawn EINVAL`. (#38056) Thanks @0xlin2023.
...
- Agents/usage normalization: normalize missing or partial assistant usage snapshots before compaction accounting so `propai agent --json` no longer crashes when provider payloads omit `totalTokens` or related usage fields. (#34977) thanks @sp-hk2ldn.
...
- ACP/console silent reply suppression: filter ACP `NO_REPLY` lead fragments and silent-only finals before `propai agent` logging/delivery so console-backed ACP sessions no longer leak `NO`/`NO_REPLY` placeholders. (#38436) Thanks @ql-wade.
...
- Daemon/systemd fresh-install probe: check for PropAi Sync's managed user unit before running `systemctl --user is-enabled`, so first-time Linux installs no longer fail on generic missing-unit probe errors. (#38819) Thanks @adaHubble.
- Gateway/container lifecycle: allow `propai gateway stop` to SIGTERM unmanaged gateway listeners and `propai gateway restart` to SIGUSR1 a single unmanaged listener when no service manager is installed, so container and supervisor-based deployments are no longer blocked by `service disabled` no-op responses. Fixes #36137. Thanks @vincentkoc.
...
- CLI/Docs memory help accuracy: clarify `propai memory status --deep` behavior and align memory command examples/docs with the current search options. (#31803) Thanks @JasonOA888 and @Avi974.
...
- CLI/bootstrap Node version hint maintenance: replace hardcoded nvm `22` instructions in `propai.mjs` with `MIN_NODE_MAJOR` interpolation so future minimum-Node bumps keep startup guidance in sync automatically. (#39056) Thanks @onstash.
...
- Daemon/Windows schtasks runtime detection: use locale-invariant `Last Run Result` running codes (`0x41301`/`267009`) as the primary running signal so `propai node status` no longer misreports active tasks as stopped on non-English Windows locales. (#39076) Thanks @ademczuk.
...
- Podman/.env gateway bind precedence: evaluate `PROPAI_GATEWAY_BIND` after sourcing `.env` in `run-propai-podman.sh` so env-file overrides are honored. (#38785) Thanks @majinyu666.
...
- Browser/config schema: accept `browser.profiles.*.driver: "PropAi Sync"` while preserving legacy `"clawd"` compatibility in validated config. (#39374; based on #35621) Thanks @gambletan and @ingyukoh.
...
- Secrets/SecretRef coverage: expand SecretRef support across the full supported user-supplied credential surface (64 targets total), including runtime collectors, `propai secrets` planning/apply/audit flows, onboarding SecretInput UX, and related docs; unresolved refs now fail fast on active surfaces while inactive surfaces report non-blocking diagnostics. (#29580) Thanks @joshavant.
...
- CLI/Config validation: add `propai config validate` (with `--json`) to validate config files before gateway startup, and include detailed invalid-key paths in startup invalid-config errors. (#31220) thanks @Sid-Qin.
...
- Zalo Personal plugin (`@propai/zalouser`): rebuilt channel runtime to use native `zca-js` integration in-process, removing external CLI transport usage and keeping QR/login + send/listen flows fully inside PropAi Sync.
...
- Plugin runtime/STT: add `api.runtime.stt.transcribeAudioFile(...)` so extensions can transcribe local audio files through PropAi Sync's configured media-understanding audio providers. (#22402) Thanks @benthecarman.
...
- Zalo Personal plugin (`@propai/zalouser`): keep canonical DM routing while preserving legacy DM session continuity on upgrade, and preserve provider-native `g-`/`u-` target ids in outbound send and directory flows so #33992 lands without breaking existing sessions or stored targets. (#33992) Thanks @darkamenosa.
...
- **BREAKING:** ACP dispatch now defaults to enabled unless explicitly disabled (`acp.dispatch.enabled=false`). If you need to pause ACP turn routing while keeping `/acp` controls, set `acp.dispatch.enabled=false`. Docs: https://docs.propai.ai/tools/acp-agents
- **BREAKING:** Plugin SDK removed `api.registerHttpHandler(...)`. Plugins must register explicit HTTP routes via `api.registerHttpRoute({ path, auth, match, handler })`, and dynamic webhook lifecycles should use `registerPluginHttpRoute(...)`.
- **BREAKING:** Zalo Personal plugin (`@propai/zalouser`) no longer depends on external `zca`-compatible CLI binaries (`openzca`, `zca-cli`) for runtime send/listen/login; operators should use `propai channels login --channel zalouser` after upgrade to refresh sessions in the new JS-native path.
...
- Feishu/Plugin sdk compatibility: add safe webhook default fallbacks when loading Feishu monitor state so mixed-version installs no longer crash if older `propai/plugin-sdk` builds omit webhook default constants. (#31606)
...
- Browser/Profile defaults: prefer `propai` profile over `chrome` in headless/no-sandbox environments unless an explicit `defaultProfile` is configured. (#14944) Thanks @BenediktSchackenberg.
- Gateway/WS security: keep plaintext `ws://` loopback-only by default, with explicit break-glass private-network opt-in via `PROPAI_ALLOW_INSECURE_PRIVATE_WS=1`; align onboarding/client/call validation and tests to this strict-default policy. (#28670) Thanks @dashed, @vincentkoc.
...
- CLI/Browser start timeout: honor `propai browser --timeout <ms> start` and stop by removing the fixed 15000ms override so slower Chrome startups can use caller-provided timeouts. (#22412, #23427) Thanks @vincentkoc.
...
- Browser/Gateway hardening: preserve env credentials for `PROPAI_GATEWAY_URL` / `CLAWDBOT_GATEWAY_URL` while treating explicit `--url` as override-only auth, and make container browser hardening flags optional with safer defaults for Docker/LXC stability. (#31504) Thanks @vincentkoc.
...
- Gateway/OpenAI chat completions: honor `x-propai-message-channel` when building `agentCommand` input for `/v1/chat/completions`, preserving caller channel identity instead of forcing `webchat`. (#30462) Thanks @bmendonca3.
...
- Voice-call/Twilio external outbound: auto-register webhook-first `outbound-api` calls (initiated outside PropAi Sync) so media streams are accepted and call direction metadata stays accurate. (#31181) Thanks @scoootscooob.
...
- Gateway/Control UI basePath POST handling: return 405 for `POST` on exact basePath routes (for example `/PropAiSync`) instead of redirecting, and add end-to-end regression coverage that root-mounted webhook POST paths still pass through to plugin handlers. (#31349) Thanks @Sid-Qin.
- Browser/default profile selection: default `browser.defaultProfile` behavior now prefers `propai` (managed standalone CDP) when no explicit default is configured, while still auto-provisioning the `chrome` relay profile for explicit opt-in use. (#32031) Fixes #31907. Thanks @liuxiaopai-ai.
...
- Models/Codex usage labels: infer weekly secondary usage windows from reset cadence when API window seconds are ambiguously reported as 24h, so `propai models status` no longer mislabels weekly limits as daily. (#31938) Thanks @bmendonca3.
...
- Security audit/skills workspace hardening: add `skills.workspace.symlink_escape` warning in `propai security audit` when workspace `skills/**/SKILL.md` resolves outside the workspace root (for example symlink-chain drift), plus docs coverage in the security glossary.
...
- Docker/Sandbox bootstrap hardening: make `PROPAI_SANDBOX` opt-in parsing explicit (`1|true|yes|on`), support custom Docker socket paths via `PROPAI_DOCKER_SOCKET`, defer docker.sock exposure until sandbox prerequisites pass, and reset/roll back persisted sandbox mode to `off` when setup is skipped or partially fails to avoid stale broken sandbox state. (#29974) Thanks @jamtujest and @vincentkoc.
...
- Browser/Managed tab cap: limit loopback managed `propai` page tabs to 8 via best-effort cleanup after tab opens to reduce long-running renderer buildup while preserving attach-only and remote profile behavior. (#29724) Thanks @pandego.
...
- CLI/installer Node preflight: enforce Node.js `v22.12+` consistently in both `propai.mjs` runtime bootstrap and installer active-shell checks, with actionable nvm recovery guidance for mismatched shell PATH/defaults. (#32356) Thanks @jasonhargrove.
...
- Plugins/install diagnostics: reject legacy plugin package shapes without `propai.extensions` and return an explicit upgrade hint with troubleshooting docs for repackaging. (#32055) Thanks @liuxiaopai-ai.
...
- Plugins/install fallback safety: resolve bare install specs to bundled plugin ids before npm lookup (for example `diffs` -> bundled `@propai/diffs`), keep npm fallback limited to true package-not-found errors, and continue rejecting non-plugin npm packages that fail manifest validation. (#32096) Thanks @scoootscooob.
...
- Tests/Subagent announce: set `PROPAI_TEST_FAST=1` before importing `subagent-announce` format suites so module-level fast-mode constants are captured deterministically on Windows CI, preventing timeout flakes in nested completion announce coverage. (#31370) Thanks @zwffff.
...
- CLI/Config: add `propai config file` to print the active config file path resolved from `PROPAI_CONFIG_PATH` or the default location. (#26256) thanks @cyb1278588254.
...
- Shell env markers: set `PROPAI_SHELL` across shell-like runtimes (`exec`, `acp`, `acp-client`, `tui-local`) so shell startup/config rules can target PropAi Sync contexts consistently, and document the markers in env/exec/acp/TUI docs. Thanks @vincentkoc.
...
- Sandbox/Browser Docker: pass `PROPAI_BROWSER_NO_SANDBOX=1` to sandbox browser containers and bump sandbox browser security hash epoch so existing containers are recreated and pick up the env on upgrade. (#29879) Thanks @Lukavyi.
...
- Gateway/Linux restart health: reduce false `propai gateway restart` timeouts by falling back to `ss -ltnp` when `lsof` is missing, confirming ambiguous busy-port cases via local gateway probe, and targeting the original `SUDO_USER` systemd user scope for restart commands. (#34874) Thanks @vincentkoc.
...
- Android/Play package ID: rename the Android app package to `ai.propai.app`, including matching benchmark and Android tooling references for Play publishing. (#38712) Thanks @obviyus.
...
- Android/Onboarding + voice reliability: request per-toggle onboarding permissions, update pairing guidance to `propai devices list/approve`, restore assistant speech playback in mic capture flow, cancel superseded in-flight speech (mute + per-reply token rotation), and keep `talk.config` loads retryable after transient failures. (#29796) Thanks @obviyus.
...
- Docker/Compose gateway targeting: run `propai-cli` in the `propai-gateway` service network namespace, require gateway startup ordering, pin Docker setup to `gateway.mode=local`, sync `gateway.bind` from `PROPAI_GATEWAY_BIND`, default optional `CLAUDE_*` compose vars to empty values to reduce automation warning noise, and harden `propai-cli` with `cap_drop` (`NET_RAW`, `NET_ADMIN`) + `no-new-privileges`. Docs now call out the shared trust boundary explicitly. (#12504) Thanks @bvanderdrift and @vincentkoc.
...
- Doctor/macOS state-dir safety: warn when PropAi Sync state resolves inside iCloud Drive (`~/Library/Mobile Documents/com~apple~CloudDocs/...`) or `~/Library/CloudStorage/...`, because sync-backed paths can cause slower I/O and lock/sync races. (#31004) Thanks @vincentkoc.
- Doctor/Linux state-dir safety: warn when PropAi Sync state resolves to an `mmcblk*` mount source (SD or eMMC), because random I/O can be slower and media wear can increase under session and credential writes. (#31033) Thanks @vincentkoc.
...
- Onboarding/Docker token parity: use `PROPAI_GATEWAY_TOKEN` as the default gateway token in interactive and non-interactive onboarding when `--gateway-token` is not provided, so `docker-setup.sh` token env/config values stay aligned. (#22658) Fixes #22638. Thanks @Clawborn and @vincentkoc.
...
- Highlight: External Secrets Management introduces a full `propai secrets` workflow (`audit`, `configure`, `apply`, `reload`) with runtime snapshot activation, strict `secrets apply` target-path validation, safer migration scrubbing, ref-only auth-profile support, and dedicated docs. (#26155) Thanks @joshavant.
...
- Agents/Routing CLI: add `propai agents bindings`, `propai agents bind`, and `propai agents unbind` for account-scoped route management, including channel-only to account-scoped binding upgrades, role-aware binding identity handling, plugin-resolved binding account IDs, and optional account-binding prompts in `propai channels add`. (#27195) thanks @gumadeiras.
...
- Telegram/DM allowlist runtime inheritance: enforce `dmPolicy: "allowlist"` `allowFrom` requirements using effective account-plus-parent config across account-capable channels (Telegram, Discord, Slack, Signal, iMessage, IRC, BlueBubbles, WhatsApp), and align `propai doctor` checks to the same inheritance logic so DM traffic is not silently dropped after upgrades. (#27936) Thanks @widingmarcus-cyber.
...
- Config/Doctor allowlist safety: reject `dmPolicy: "allowlist"` configs with empty `allowFrom`, add Telegram account-level inheritance-aware validation, and teach `propai doctor --fix` to restore missing `allowFrom` entries from pairing-store files when present, preventing silent DM drops after upgrades. (#27936) Thanks @widingmarcus-cyber.
...
- CLI/Gateway `--force` in non-root Docker: recover from `lsof` permission failures (`EACCES`/`EPERM`) by falling back to `fuser` kill + probe-based port checks, so `propai gateway --force` works for default container `node` user flows. (#27941) Thanks @steipete.
...
- Sessions cleanup/Doctor: add `propai sessions cleanup --fix-missing` to prune store entries whose transcript files are missing, including doctor guidance and CLI coverage. Landed from contributor PR #27508 by @Sid-Qin. (#27422)
- Doctor/State integrity: ignore metadata-only slash routing sessions when checking recent missing transcripts so `propai doctor` no longer reports false-positive transcript-missing warnings for `*:slash:*` keys. (#27375) thanks @gumadeiras.
...
- CLI/Daemon status TLS probe: use `wss://` and forward local TLS certificate fingerprint for TLS-enabled gateway daemon probes so `propai daemon status` works with `gateway.bind=lan` + `gateway.tls.enabled=true`. (#24234) thanks @liuy.
- Podman/Default bind: change `run-propai-podman.sh` default gateway bind from `lan` to `loopback` and document explicit LAN opt-in with Control UI origin configuration. (#27491) thanks @robbyczgw-cla.
...
- Auth/Auth profiles: normalize `auth-profiles.json` alias fields (`mode -> type`, `apiKey -> key`) before credential validation so entries copied from `propai.json` auth examples are no longer silently dropped. (#26950) thanks @byungsker.
...
- Onboarding/Security: clarify onboarding security notices that PropAi Sync is personal-by-default (single trusted operator boundary) and shared/multi-user setups require explicit lock-down/hardening.
- Branding/Docs + Apple surfaces: replace remaining `bot.molt` launchd label, bundle-id, logging subsystem, and command examples with `ai.propai` across docs, iOS app surfaces, helper scripts, and CLI test fixtures.
...
- Models/Auth probes: map permanent auth failover reasons (`auth_permanent`, for example revoked keys) into probe auth status instead of `unknown`, so `propai models status --probe` reports actionable auth failures. (#25754) thanks @rrenamed.
...
- Auto-reply/Abort shortcuts: expand standalone stop phrases (`stop PropAi Sync`, `stop action`, `stop run`, `stop agent`, `please stop`, and related variants), accept trailing punctuation (for example `STOP PropAi Sync!!!`), add multilingual stop keywords (including ES/FR/ZH/HI/AR/JP/DE/PT/RU forms), and treat exact `do not do that` as a stop trigger while preserving strict standalone matching. (#25103) Thanks @steipete and @vincentkoc.
...
- macOS/Gateway launch: prefer an available `propai` binary before pnpm/node runtime fallback when resolving local gateway commands, so local startup no longer fails on hosts with broken runtime discovery. (#25512) Thanks @chilu18.
...
- CLI/Memory search: accept `--query <text>` for `propai memory search` (while keeping positional query support), and emit a clear error when neither form is provided. (#25904, #25857) Thanks @niceysam and @stakeswky.
- CLI/Doctor: correct stale recovery hints to use valid commands (`propai gateway status --deep` and `propai configure --section model`). (#24485) Thanks @chilu18.
...
- Security/Sandbox media: restrict sandbox media tmp-path allowances to propai-managed tmp roots instead of broad host `os.tmpdir()` trust, and add outbound/channel guardrails (tmp-path lint + media-root smoke tests) to prevent regressions in local media attachment reads. Thanks @tdjackey for reporting.
- Security/Sandbox media: reject hard-linked PropAi Sync tmp media aliases (including symlink-to-hardlink chains) during sandbox media path resolution to prevent out-of-sandbox inode alias reads. (#25820) Thanks @bmendonca3.
...
- Sessions/Cron: harden session maintenance with `propai sessions cleanup`, per-agent store targeting, disk-budget controls (`session.maintenance.maxDiskBytes` / `highWaterBytes`), and safer transcript/archive cleanup + run-log retention behavior. (#24753) thanks @gumadeiras.
...
- **BREAKING:** browser SSRF policy now defaults to trusted-network mode (`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork=true` when unset), and canonical config uses `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` instead of `browser.ssrfPolicy.allowPrivateNetwork`. `propai doctor --fix` migrates the legacy key automatically.
...
- Doctor/Memory: query gateway-side default-agent memory embedding readiness during `propai doctor` (instead of inferring from generic gateway health), and warn when the gateway memory probe is unavailable or not ready while keeping `propai configure` remediation guidance. (#22327) thanks @therk.
...
- Providers/DashScope: mark DashScope-compatible `openai-completions` endpoints as `supportsDeveloperRole=false` so PropAi Sync sends `system` instead of unsupported `developer` role on Qwen/DashScope APIs. (#19130) Thanks @Putzhuawa and @vincentkoc.
...
- Config/Write: apply `unsetPaths` with immutable path-copy updates so config writes never mutate caller-provided objects, and harden `propai config get/set/unset` path traversal by rejecting prototype-key segments and inherited-property traversal. (#24134) thanks @frankekn.
...
- CLI/Update: add `propai update --dry-run` to preview channel/tag/target/restart actions without mutating config, installing, syncing plugins, or restarting.
...
- **BREAKING:** tool-failure replies now hide raw error details by default. PropAi Sync still sends a failure summary, but detailed error suffixes (for example provider/runtime messages and local path fragments) now require `/verbose on` or `/verbose full`.
...
- Security/CLI: redact sensitive values in `propai config get` output before printing config paths, preventing credential leakage to terminal output/history. (#13683) Thanks @SleuthCo.
...
- Install/Discord Voice: make the native Opus decoder optional so `propai` install/update no longer hard-fails when native builds fail, while keeping `opusscript` as the runtime fallback decoder for Discord voice flows. (#23737, #23733, #23703) Thanks @jeadland, @Sheetaa, and @Breakyman.
- Docker/Setup: precreate `$PROPAI_CONFIG_DIR/identity` during `docker-setup.sh` so CLI commands that need device identity (for example `devices list`) avoid `EACCES ... /home/node/.propai/identity` failures on restrictive bind mounts. (#23948) Thanks @ackson-beep.
...
- Gateway/Restart: fix restart-loop edge cases by keeping `propai.mjs -> dist/entry.js` bootstrap detection explicit, reacquiring the gateway lock for in-process restart fallback paths, and tightening restart-loop regression coverage. (#23416) Thanks @jeffwnli.
...
- Feishu/Plugins: restore bundled Feishu SDK availability for global installs and strip `propai: workspace:*` from plugin `devDependencies` during plugin-version sync so npm-installed Feishu plugins do not fail dependency install. (#23611, #23645, #23603)
...
- Plugins/Discovery: ignore scanned extension backup/disabled directory patterns (for example `.backup-*`, `.bak`, `.disabled*`) and move updater backup directories under `.propai-install-backups`, preventing duplicate plugin-id collisions from archived copies.
- Plugins/CLI: make `propai plugins enable` and plugin install/link flows update allowlists via shared plugin-enable policy so enabled plugins are not left disabled by allowlist mismatch. (#23190) Thanks @downwind7clawd-ctrl.
...
- Node/macOS exec host: default headless macOS node `system.run` to local execution and only route through the companion app when `PROPAI_NODE_EXEC_HOST=app` is explicitly set, avoiding companion-app filesystem namespace mismatches during exec. (#23547) Thanks @steipete.
...
- Installer/Smoke tests: remove legacy `PROPAI_USE_GUM` overrides from docker install-smoke runs so tests exercise installer auto TTY detection behavior directly.
...
- Gateway/Auth: preserve `PROPAI_GATEWAY_PASSWORD` env override precedence for remote gateway call credentials after shared resolver refactors, preventing stale configured remote passwords from overriding runtime secret rotation.
...
- Security/Audit: add `propai security audit` detection for open group policies that expose runtime/filesystem tools without sandbox/workspace guards (`security.exposure.open_groups_with_runtime_or_fs`).
...
- Security/Gateway: emit a startup security warning when insecure/dangerous config flags are enabled (including `gateway.controlUi.dangerouslyDisableDeviceAuth=true`) and point operators to `propai security audit`.
...
- Telegram/Network: default Node 22+ DNS result ordering to `ipv4first` for Telegram fetch paths and add `PROPAI_TELEGRAM_DNS_RESULT_ORDER`/`channels.telegram.network.dnsResultOrder` overrides to reduce IPv6-path fetch failures. (#5405) Thanks @Glucksberg.
...
- Docs/Telegram: correct Node 22+ network defaults (`autoSelectFamily`, `dnsResultOrder`) and clarify Telegram setup does not use positional `propai channels login telegram`. (#23609) Thanks @ryanbastic.
...
- Security/Audit: add `propai security audit` finding `gateway.nodes.allow_commands_dangerous` for risky `gateway.nodes.allowCommands` overrides, with severity upgraded to critical on remote gateway exposure.
...
- Channels/CLI: add per-account/channel `defaultTo` outbound routing fallback so `propai agent --deliver` can send without explicit `--reply-to` when a default target is configured. (#16985) Thanks @KirillShchetinin.
...
- CLI/Config: preserve explicitly unset config paths in persisted JSON after writes so `propai config unset <path>` no longer re-introduces defaulted keys (for example `commands.ownerDisplay`) through schema normalization. (#22984) Thanks @aronchick.
- CLI: keep `propai -v` as a root-only version alias so subcommand `-v, --verbose` flags (for example ACP/hooks/skills) are no longer intercepted globally. (#21303) thanks @adhitShet.
...
- Gateway/Pairing/CLI: align read-scope compatibility in pairing/device-token checks and add local `propai devices` fallback recovery for loopback `pairing required` deadlocks, with explicit fallback notice to unblock approval bootstrap flows. (#21616) Thanks @shakkernerd.
...
- macOS/Build: default release packaging to `BUNDLE_ID=ai.propai.mac` in `scripts/package-mac-dist.sh`, so Sparkle feed URL is retained and auto-update no longer fails with an empty appcast feed. (#19750) thanks @loganprit.
...
- Security/PropAiSyncKit/UI: prevent injected inbound user context metadata blocks from leaking into chat history in TUI, webchat, and macOS surfaces by stripping all untrusted metadata prefixes at display boundaries. (#22142) Thanks @Mellowambience, @vincentkoc.
- Security/PropAiSyncKit/UI: strip inbound metadata blocks from user messages in TUI rendering while preserving user-authored content. (#22345) Thanks @kansodata, @vincentkoc.
- Security/PropAiSyncKit/UI: prevent inbound metadata leaks and reply-tag streaming artifacts in TUI rendering by stripping untrusted metadata prefixes at display boundaries. (#22346) Thanks @akramcodez, @vincentkoc.
- Security/Agents: restrict local MEDIA tool attachments to core tools and the PropAi Sync temp root to prevent untrusted MCP tool file exfiltration. Thanks @NucleiAv and @thewilloftheshadow.
...
- Security/Sandbox: remove default `--no-sandbox` for the browser container entrypoint, add explicit opt-in via `PROPAI_BROWSER_NO_SANDBOX` / `CLAWDBOT_BROWSER_NO_SANDBOX`, and add security-audit checks for stale/missing sandbox browser Docker hash labels. Thanks @TerminalsandCoffee and @vincentkoc.
...
- Security/Sandbox Browser: default browser sandbox containers to a dedicated Docker network (`propai-sandbox-browser`), add optional CDP ingress source-range restrictions, auto-create missing dedicated networks, and warn in `propai security --audit` when browser sandboxing runs on bridge without source-range limits. Thanks @TerminalsandCoffee for reporting.
...
- Gateway/CLI: add paired-device hygiene flows with `device.pair.remove`, plus `propai devices remove` and guarded `propai devices clear --yes [--pending]` commands for removing paired entries and optionally rejecting pending requests. (#20057) Thanks @mbelinky.
...
- iOS/Signing: restore local auto-selected signing-team overrides during iOS project generation by wiring `.local-signing.xcconfig` into the active signing config and emitting `PROPAI_DEVELOPMENT_TEAM` in local signing setup. (#19993) Thanks @ngutman.
...
- PropAi SyncKit/Protocol: preserve JSON boolean literals (`true`/`false`) when bridging through `AnyCodable` so Apple client RPC params no longer re-encode booleans as `1`/`0`. Thanks @mbelinky.
...
- Browser/Relay: reuse an already-running extension relay when the relay port is occupied by another PropAi Sync process, while still failing on non-relay port collisions to avoid masking unrelated listeners. (#20035) Thanks @mbelinky.
...
- Security/Gateway/Canvas: replace shared-IP fallback auth with node-scoped session capability URLs for `/__PROPAI__/canvas/*` and `/__PROPAI__/a2ui/*`, fail closed when trusted-proxy requests omit forwarded client headers, and add IPv6/proxy-header regression coverage. Thanks @aether-ai-agent for reporting.
...
- Security/ACP+Exec: add `propai acp --token-file/--password-file` secret-file support (with inline secret flag warnings), redact ACP working-directory prefixes to `~` home-relative paths, constrain exec script preflight file inspection to the effective `workdir` boundary, and add security-audit warnings when `tools.exec.host="sandbox"` is configured while sandbox mode is off.
- Security/Plugins/Hooks: enforce runtime/package path containment with realpath checks so `propai.extensions`, `propai.hooks`, and hook handler modules cannot escape their trusted roots via traversal or symlinks.
...
- Cron/CLI: add deterministic default stagger for recurring top-of-hour cron schedules (including 6-field seconds cron), auto-migrate existing jobs to persisted `schedule.staggerMs`, and add `propai cron add/edit --stagger <duration>` plus `--exact` overrides for per-job timing control.
...
- Docker: add optional `PROPAI_INSTALL_BROWSER` build arg to preinstall Chromium + Xvfb in the Docker image, avoiding runtime Playwright installs. (#18449)
...
- Gateway/Presence: centralize presence snapshot broadcasts and unify runtime version precedence (`PROPAI_VERSION` > `PROPAI_SERVICE_VERSION` > `npm_package_version`) so self-presence and websocket `hello-ok` report consistent versions.
...
- Models CLI: validate catalog entries in `propai models set`. (#18129) Thanks @carrotRakko.
...
- PropAi SyncKit/iOS ChatUI: accept canonical session-key completion events for local pending runs and preserve message IDs across history refreshes, preventing stuck "thinking" state and message flicker after gateway replies. (#18165) Thanks @mbelinky.
...
- iOS/Signing: auto-select local Apple Development team during iOS project generation/build, prefer the canonical PropAi Sync team when available, and support local per-machine signing overrides without committing team IDs. (#18421) Thanks @ngutman.
...
- CLI/Pairing: make `propai qr --remote` prefer `gateway.remote.url` over tailscale/public URL resolution and register the `propai clawbot qr` legacy alias path. (#18091)
- CLI/QR: restore fail-fast validation for `propai qr --remote` when neither `gateway.remote.url` nor tailscale `serve`/`funnel` is configured, preventing unusable remote pairing QR flows. (#18166) Thanks @mbelinky.
...
- CLI/Doctor: ensure `propai doctor --fix --non-interactive --yes` exits promptly after completion so one-shot automation no longer hangs. (#18502)
- CLI/Doctor: auto-repair `dmPolicy="open"` configs missing wildcard allowlists and write channel-correct repair paths (including `channels.googlechat.dm.allowFrom`) so `propai doctor --fix` no longer leaves Google Chat configs invalid after attempted repair. (#18544)
...
- Gateway/Update: prevent restart crash loops after failed self-updates by restarting only on successful updates, stopping early on failed install/build steps, and running `propai doctor --fix` during updates to sanitize config. (#18131) Thanks @RamiNoodle733.
...
- CLI/Daemon: warn when a gateway restart sees a stale service token so users can reinstall with `propai gateway install --force`, and skip drift warnings for non-gateway service restarts. (#18018)
...
- CLI/Status: fix `propai status --all` token summaries for bot-token-only channels so Mattermost/Zalo no longer show a bot+app warning. (#18527) Thanks @echo931.
- CLI/Configure: make the `/model picker` allowlist prompt searchable with tokenized matching in `propai configure` so users can filter huge model lists by typing terms like `gpt-5.2 openai/`. (#19010) Thanks @bjesuiter.
- CLI/Message: preserve `--components` JSON payloads in `propai message send` so Discord component payloads are no longer dropped. (#18222) Thanks @saurabhchopade.
...
- Models/CLI: sync auth-profiles credentials into agent `auth.json` before registry availability checks so `propai models list --all` reports auth correctly for API-key/token providers, normalize provider-id aliases when bridging credentials, and skip expired token mirrors. (#18610, #18615)
...
- Security/Sessions: create new session transcript JSONL files with user-only (`0o600`) permissions and extend `propai security audit --fix` to remediate existing transcript file permissions.
...
- Telegram: add poll sending via `propai message poll` (duration seconds, silent delivery, anonymity controls). (#16209) Thanks @robbyczgw-cla.
- Slack/Discord: add `dmPolicy` + `allowFrom` config aliases for DM access control; legacy `dm.policy` + `dm.allowFrom` keys remain supported and `propai doctor --fix` can migrate them.
...
- CLI/Plugins: ensure `propai message send` exits after successful delivery across plugin-backed channels so one-shot sends do not hang. (#16491) Thanks @yinghaosang.
- CLI/Plugins: run registered plugin `gateway_stop` hooks before `propai message` exits (success and failure paths), so plugin-backed channels can clean up one-shot CLI resources. (#16580) Thanks @gumadeiras.
...
- TUI/Gateway: resolve local gateway target URL from `gateway.bind` mode (tailnet/lan) instead of hardcoded localhost so `propai tui` connects when gateway is non-loopback. (#16299) Thanks @cortexuvula.
- TUI: honor explicit `--session <key>` in `propai tui` even when `session.scope` is `global`, so named sessions no longer collapse into shared global history. (#16575) Thanks @cinqu.
...
- Memory/QMD: make `propai memory index` verify and print the active QMD index file path/size, and fail when QMD leaves a missing or zero-byte index artifact after an update. (#16775) Thanks @Shunamxiao.
...
- Media/Security: allow local media reads from PropAi Sync state `workspace/` and `sandboxes/` roots by default so generated workspace media can be delivered without unsafe global path bypasses. (#15541) Thanks @lanceji.
...
- Security/Hooks: restrict hook transform modules to `~/.propai/hooks/transforms` (prevents path traversal/escape module loads via config). Config note: `hooks.transformsDir` must now be within that directory. Thanks @akhmittra.
...
- Telegram/Security: require numeric Telegram sender IDs for allowlist authorization (reject `@username` principals), auto-resolve `@username` to IDs in `propai doctor --fix` (when possible), and warn in `propai security audit` when legacy configs contain usernames. Thanks @vincentkoc.
...
- macOS: hard-limit unkeyed `propai://agent` deep links and ignore `deliver` / `to` / `channel` unless a valid unattended key is provided. Thanks @Cillian-Collins.
...
- Install: add optional Podman-based setup: `setup-podman.sh` for one-time host setup (PropAi Sync user, image, launch script, systemd quadlet), `run-propai-podman.sh launch` / `launch setup`; systemd Quadlet unit for PropAi Sync user service; docs for rootless container, PropAi Sync user (subuid/subgid), and quadlet (troubleshooting). (#16273) Thanks @DarwinsBuddy.
...
- Config/State: removed legacy `.moltbot` auto-detection/migration and `moltbot.json` config candidates. If you still have state/config under `~/.moltbot`, move it to `~/.propai` (recommended) or set `PROPAI_STATE_DIR` / `PROPAI_CONFIG_PATH` explicitly.

### Fixes

- Gateway/Auth: add trusted-proxy mode hardening follow-ups by keeping `PROPAI_GATEWAY_*` env compatibility, auto-normalizing invalid setup combinations in interactive `gateway configure` (trusted-proxy forces `bind=lan` and disables Tailscale serve/funnel), and suppressing shared-secret/rate-limit audit findings that do not apply to trusted-proxy deployments. (#15940) Thanks @nickytonline.
...
- OpenAI Codex/Auth: bridge PropAi Sync OAuth profiles into `pi` `auth.json` so model discovery and models-list registry resolution can use Codex OAuth credentials. (#15184) Thanks @loiie45e.
...
- CLI/Completion: route plugin-load logs to stderr and write generated completion scripts directly to stdout to avoid `source <(PropAi Sync completion ...)` corruption. (#15481) Thanks @arosstale.
...
- Security/Browser: constrain `POST /trace/stop`, `POST /wait/download`, and `POST /download` output paths to PropAi Sync temp roots and reject traversal/escape paths.
...
- Security/Browser: confine `POST /hooks/file-chooser` upload paths to an PropAi Sync temp uploads root and reject traversal/escape paths. Thanks @1seal.
...
- Security/Onboarding: clarify multi-user DM isolation remediation with explicit `propai config set session.dmScope ...` commands in security audit, doctor security, and channel onboarding guidance. (#13129) Thanks @VintLin.
...
- Config: preserve `${VAR}` env references when writing config files so `propai config set/apply/patch` does not persist secrets to disk. Thanks @thewilloftheshadow.
...
- CLI/Plugins: add `propai plugins uninstall <id>` with `--dry-run`, `--force`, and `--keep-files` options, including safe uninstall path handling and plugin uninstall docs. (#5985) Thanks @JustasMonkev.
- CLI: add `propai logs --local-time` to display log timestamps in local timezone. (#13818) Thanks @xialonglee.
...
- Logging/CLI: use local timezone timestamps for console prefixing, and include `±HH:MM` offsets when using `propai logs --local-time` to avoid ambiguity. (#14771) Thanks @0xRaini.
...
- Gateway/Control UI: resolve missing dashboard assets when `propai` is installed globally via symlink-based Node managers (nvm/fnm/n/Homebrew). (#14919) Thanks @aynorica.
...
- Logging/Browser: fall back to `os.tmpdir()/PropAiSync` for default log, browser trace, and browser download temp paths when `/tmp/PropAiSync` is unavailable.
...
- Feishu plugin packaging: remove `workspace:*` `propai` dependency from `extensions/feishu` and sync lockfile for install compatibility. (#14423) Thanks @jackcooper2015.
...
- Update/Daemon: fix post-update restart compatibility by generating `dist/cli/daemon-cli.js` with alias-aware exports from hashed daemon bundles, preventing `registerDaemonCli` import failures during `propai update`.
...
- Paths: add `PROPAI_HOME` for overriding the home directory used by internal path resolution. (#12091) Thanks @sebslight.
...
- CLI: make `propai plugins list` output scannable by hoisting source roots and shortening bundled/global/workspace plugin paths.
...
- Paths: structurally resolve `PROPAI_HOME`-derived home paths and fix Windows drive-letter handling in tool meta shortening. (#12125) Thanks @mcaxtr.
...
- Memory/QMD: run boot refresh in background by default, add configurable QMD maintenance timeouts, retry QMD after fallback failures, and scope QMD queries to propai-managed collections. (#9690, #9705, #10042) Thanks @vignesh07.
...
- State dir: honor `PROPAI_STATE_DIR` for default device identity and canvas storage paths. (#4824) Thanks @kossoy.
...
- macOS: honor Nix-managed defaults suite (`ai.propai.mac`) for nixMode to prevent onboarding from reappearing after bundle-id churn. (#12205) Thanks @joshp123.
...
- Update: avoid wiping prebuilt Control UI assets during dev auto-builds (`tsdown --no-clean`), run update doctor via `propai.mjs`, and auto-restore missing UI assets after doctor. (#10146) Thanks @gumadeiras.
...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (propai-cn).
...
- Docs: finish renaming the QMD memory docs to reference the PropAi Sync state dir.
...
- Skills: update session-logs paths from .clawdbot to .propai. (#4502)
...
- Build: align npm tar override and bin metadata so the `propai` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update PropAi Sync branding + install links.
...
- Rebrand: rename the npm package/CLI to `propai`, add a `propai` compatibility shim, and move extensions to the `@propai/*` scope.
...
- macOS: finish PropAi Sync app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Docs: update exe.dev install instructions. (#https://github.com/propai/propai/pull/3047) Thanks @zackerthescar.
...
- Skills: update session-logs paths to use ~/.propai. (#4502) Thanks @bonald.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://docs.propai.ai/providers/ollama https://docs.propai.ai/providers/venice
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://docs.propai.ai/tts
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://docs.propai.ai/tools/exec-approvals https://docs.propai.ai/tools/slash-commands
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://docs.propai.ai/channels/telegram

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://docs.propai.ai/tts
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://docs.propai.ai/tts
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://docs.propai.ai/channels/telegram
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://docs.propai.ai/tools/web
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://docs.propai.ai/tools/exec-approvals https://docs.propai.ai/tools/slash-commands
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @steipete.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://docs.propai.ai/diagnostics/flags
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://docs.propai.ai/bedrock
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://docs.propai.ai/channels/bluebubbles
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://docs.propai.ai/channels/signal
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://docs.propai.ai/tts
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://docs.propai.ai/gateway/tools-invoke-http-api
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://docs.propai.ai/gateway/heartbeat
- Deploy: add Fly.io deployment support + guide. (#1570) https://docs.propai.ai/platforms/fly
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://docs.propai.ai/channels/tlon

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://docs.propai.ai/multi-agent-sandbox-tools
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://docs.propai.ai/bedrock
- CLI: add `propai system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.propai.ai/cli/system
- CLI: add live auth probes to `propai models status` for per-profile verification. (commit 40181afde) https://docs.propai.ai/cli/models
- CLI: restart the gateway by default after `propai update`; add `--no-restart` to skip it. (commit 2c85b1b40)
- Browser: add node-host proxy auto-routing for remote gateways (configurable per gateway/node). (commit c3cb26f7c)
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://docs.propai.ai/tools/llm-task
...
- Docs: add cron vs heartbeat decision guide (with Lobster workflow notes). (#1533) Thanks @JustYannicc. https://docs.propai.ai/automation/cron-vs-heartbeat
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://docs.propai.ai/gateway/heartbeat
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `propai models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://docs.propai.ai/cli/agents https://docs.propai.ai/web/control-ui
...
- Highlight: Lobster optional plugin tool for typed workflows + approval gates. https://docs.propai.ai/tools/lobster
- Lobster: allow workflow file args via `argsJson` in the plugin tool. https://docs.propai.ai/tools/lobster
...
- CLI: flatten node service commands under `propai node` and remove `service node` docs.
- CLI: move gateway service commands under `propai gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `propai update wizard` for interactive channel selection and restart prompts. https://docs.propai.ai/cli/update
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://docs.propai.ai/gateway/troubleshooting
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://docs.propai.ai/web/control-ui#insecure-http
...
- Control UI: add copy-as-markdown with error feedback. (#1345) https://docs.propai.ai/web/control-ui
- Control UI: drop the legacy list view. (#1345) https://docs.propai.ai/web/control-ui
- TUI: add syntax highlighting for code blocks. (#1200) https://docs.propai.ai/tui
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://docs.propai.ai/tui
- TUI: add a searchable model picker for quicker model selection. (#1198) https://docs.propai.ai/tui
- TUI: add input history (up/down) for submitted messages. (#1348) https://docs.propai.ai/tui
- ACP: add `propai acp` for IDE integrations. https://docs.propai.ai/cli/acp
- ACP: add `propai acp client` interactive harness for debugging. https://docs.propai.ai/cli/acp
- Skills: add download installs with OS-filtered options. https://docs.propai.ai/tools/skills
- Skills: add the local sherpa-onnx-tts skill. https://docs.propai.ai/tools/skills
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://docs.propai.ai/concepts/memory
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://docs.propai.ai/concepts/memory
- Memory: add OpenAI batch indexing for embeddings when configured. https://docs.propai.ai/concepts/memory
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://docs.propai.ai/concepts/memory
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://docs.propai.ai/concepts/memory
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://docs.propai.ai/concepts/memory
- Memory: add `--verbose` logging for memory status + batch indexing details. https://docs.propai.ai/concepts/memory
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://docs.propai.ai/concepts/memory
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://docs.propai.ai/tools/browser
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://docs.propai.ai/channels/nostr
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://docs.propai.ai/channels/matrix
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://docs.propai.ai/channels/slack
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://docs.propai.ai/channels/telegram
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://docs.propai.ai/plugins/zalouser
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://docs.propai.ai/plugins/manifest
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://docs.propai.ai/plugins/manifest
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://docs.propai.ai/plugins/manifest
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://docs.propai.ai/web/control-ui
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://docs.propai.ai/gateway/configuration
- Plugins: add plugin slots with a dedicated memory slot selector. https://docs.propai.ai/plugins/agent-tools
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://docs.propai.ai/channels/bluebubbles
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://docs.propai.ai/channels/zalo
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://docs.propai.ai/plugins/zalouser
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://docs.propai.ai/plugins/agent-tools
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `propai update`.
- Plugins: share npm plugin update logic between `propai update` and `propai plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://docs.propai.ai/reference/api-usage-costs
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://docs.propai.ai/cli/security
- Exec: add host/security/ask routing for gateway + node exec. https://docs.propai.ai/tools/exec
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://docs.propai.ai/tools/exec
- Exec approvals: migrate approvals to `~/.propai/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.propai.ai/tools/exec-approvals
- Nodes: add headless node host (`propai node start`) for `system.run`/`system.which`. https://docs.propai.ai/cli/node
- Nodes: add node daemon service install/status/start/stop/restart. https://docs.propai.ai/cli/node
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://docs.propai.ai/concepts/session
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://docs.propai.ai/tools/subagents
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://docs.propai.ai/concepts/groups
- Models: add Qwen Portal OAuth provider support. (#1120) https://docs.propai.ai/providers/qwen
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://docs.propai.ai/start/onboarding
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://docs.propai.ai/start/onboarding
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://docs.propai.ai/platforms/android
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://docs.propai.ai/bedrock
- Docs: clarify WhatsApp voice notes. https://docs.propai.ai/channels/whatsapp
- Docs: clarify Windows WSL portproxy LAN access notes. https://docs.propai.ai/platforms/windows
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://docs.propai.ai/tools/browser-login
...
- Build: use tsgo for dev/watch builds by default (opt out with `PROPAI_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `propai doctor --fix` to repair, then update plugins (`propai plugins update`) if you use any.
...
- CLI: keep `propai logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `propai memory status`.
...
- Memory: probe sqlite-vec availability in `propai memory status`.
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) — thanks @ThomsenDrake. https://docs.propai.ai/hooks
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://docs.propai.ai/nodes/media-understanding
- Plugins: add Zalo Personal plugin (`@propai/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.propai.ai/plugins/zalouser
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) — thanks @timolins. https://docs.propai.ai/providers/vercel-ai-gateway
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) — thanks @thewilloftheshadow. https://docs.propai.ai/concepts/session
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) — thanks @YuriNachos. https://docs.propai.ai/tools/web

### Breaking

- **BREAKING:** `propai message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `propai hooks` is now `propai webhooks`; hooks live under `propai hooks`. https://docs.propai.ai/cli/webhooks
- **BREAKING:** `propai plugins install <path>` now copies into `~/.propai/extensions` (use `--link` to keep path-based loading).
...
- Tools: add `exec` PTY support for interactive sessions. https://docs.propai.ai/tools/exec
...
- Directory: unify `propai directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `propai.hooks` manifests and `propai hooks install/update`.
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental “open” behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/…` tokens correctly. https://docs.propai.ai/gateway/security
- Security: redact sensitive text in gateway WS logs.
- Tools: cap pending `exec` process output to avoid unbounded buffers.
- CLI: speed up `propai sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `propai models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@propai/msteams` via `propai plugins install @propai/msteams`.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `propai-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `propai security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `propai models status`, and update docs.
- CLI: add `--json` output for `propai daemon` lifecycle/install commands.
...
- Fix: make `propai update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `propai configure`, even if no other sections are chosen.
...
- Security: expanded `propai security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `propai security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `propai browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.propai-dev`).
...
- Memory: new `propai memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.propai/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `propai dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `propai doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `propai doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `propai plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `propai` CLI when missing in local mode; install via `propai.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch propai.ai scripts and run install sh/cli with onboarding skipped.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `propai.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- CLI: `propai status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `propai update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `propai reset` and `propai uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `propai gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `propai status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `propai sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `propai sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “PropAi Sync” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `propai update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/PropAiSync`. (#660)
...
- CLI: `propai message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `propai models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.propai/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `propai pairing list <provider>` + `propai pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `propai logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$PROPAI_STATE_DIR/.env` (`~/.propai/.env`) as a fallback after CWD `.env`.
...
- CLI: run `propai agent` via the Gateway by default; use `--local` to force embedded mode.
