...
} from "propai/plugin-sdk/phone-control";
...
function createApi(params: {
  ...
  return {
    ...
    } as PropAiSyncPluginApi["runtime"],
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-phone-control-test-"));
    try {
      ...
      let command: PropAiSyncPluginCommandDefinition | undefined;
