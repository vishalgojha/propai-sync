...
import type { PropAiSyncPluginApi, PropAiSyncPluginService } from "propai/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: PropAiSyncPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: PropAiSyncPluginApi["config"]): string[] {
...
export default function register(api: PropAiSyncPluginApi) {
  ...
  const timerService: PropAiSyncPluginService = {
