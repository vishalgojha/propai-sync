# Google Gemini CLI Auth (PropAi Sync plugin)
...
```bash
PropAi Sync plugins enable google-gemini-cli-auth
```
...
```bash
PropAi Sync models auth login --provider google-gemini-cli --set-default
```
...
- `PROPAI_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `PROPAI_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
