import type { RuntimeEnv, WizardPrompter } from "propai/plugin-sdk/irc";
...
describe("irc onboarding", () => {
  it("configures host and nick via onboarding prompts", async () => {
    const prompter = createPrompter({
      text: vi.fn(async ({ message }: { message: string }) => {
        ...
        if (message === "IRC real name") {
          return "PropAi Sync Bot";
        }
        if (message.startsWith("Auto-join IRC channels")) {
          return "#PropAi Sync, #ops";
        }
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("propai-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#PropAi Sync", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#PropAi Sync", "#ops"]);
  });
