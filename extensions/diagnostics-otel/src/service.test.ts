...
vi.mock("propai/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("propai/plugin-sdk/diagnostics-otel")>(
    "propai/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { PropAiSyncPluginServiceContext } from "propai/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "propai/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/propai-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): PropAiSyncPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("PropAiSync.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("PropAiSync.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("PropAiSync.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("PropAiSync.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("PropAiSync.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("PropAiSync.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("PropAiSync.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("PropAiSync.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("PropAiSync.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("PropAiSync.webhook.processed");
    expect(spanNames).toContain("PropAiSync.message.processed");
    expect(spanNames).toContain("PropAiSync.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["PropAiSync.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("PropAiSync.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "PropAiSync.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["PropAiSync.reason"]).toBe("string");
    expect(String(attrs?.["PropAiSync.reason"])).not.toContain(
