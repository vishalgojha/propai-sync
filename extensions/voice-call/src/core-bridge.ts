...
function resolvePropAiSyncRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.propai_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set PROPAI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolvePropAiSyncRoot(), "dist", "extensionAPI.js");
