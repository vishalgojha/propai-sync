...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__PROPAI_channel_id";
...
type InteractionCallbackConfig = Pick<PropAiSyncConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "propai-mattermost-interactions").update(botToken).digest("hex");
}
