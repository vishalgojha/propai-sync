...
read_when:
  - You want to place an outbound voice call from propai
...
Voice calls for propai via a plugin. Supports outbound notifications and
...
- Use `propai voicecall ...` or the `voice_call` tool
...
```bash
propai plugins install ./extensions/voice-call
...
```bash
propai voicecall call --to "+15555550123" --message "Hello from propai"
propai voicecall continue --call-id <id> --message "Any questions?"
propai voicecall speak --call-id <id> --message "One moment"
propai voicecall end --call-id <id>
propai voicecall status --call-id <id>
propai voicecall tail
