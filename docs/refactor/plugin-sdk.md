...
- Publish as `propai/plugin-sdk` (or export from core under `propai/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `propaiPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: propaiConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: propaiConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: propaiConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: propaiConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: propaiConfig): string;
  };
};
```
...
- Introduce `propai/plugin-sdk`.
- Add `api.runtime` to `propaiPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `propaiRuntime: ">=2026.2.0"`).
