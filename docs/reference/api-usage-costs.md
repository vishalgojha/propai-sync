...
propai features that can generate provider usage or paid API calls.
...
- `propai status --usage` and `propai channels list` show provider **usage windows**
...
propai can pick up credentials from:
...
`propai models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
