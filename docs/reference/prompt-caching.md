...
- With Anthropic API-key auth profiles, propai seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, propai injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
propai exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.propai/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$PROPAI_STATE_DIR/logs/cache-trace.jsonl`
...
- `PROPAI_CACHE_TRACE=1` enables cache tracing.
- `PROPAI_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `PROPAI_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `PROPAI_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `PROPAI_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
