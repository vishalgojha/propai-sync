...
propai integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- propai owns lifecycle when `channels.signal.autoStart=true`.
...
- propai spawns `imsg rpc` as a child process (legacy iMessage integration).
