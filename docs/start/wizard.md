...
The onboarding wizard is the **recommended** way to set up propai on macOS,
...
`propai dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`propai configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.propai/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `propai doctor` first.
</Note>
...
Use `propai agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.propai/workspace-<agentId>`.
