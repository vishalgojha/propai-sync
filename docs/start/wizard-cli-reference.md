...
read_when:
  - You need detailed behavior for propai onboard
...
This page is the full reference for `propai onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.propai/propai.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `propai doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `propai pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `propai health`.
    - `propai status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.propai/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.propai/agents/<agentId>/agent/auth-profiles.json`
...
`~/.propai/credentials/oauth.json` (or `$PROPAI_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`propai onboard`](/cli/onboard)
