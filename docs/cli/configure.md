---
summary: "CLI reference for `propai configure` (interactive configuration prompts)"
read_when:
...
Tip: `propai config` without a subcommand opens the same wizard. Use
`propai config get|set|unset` for non-interactive edits.
...
```bash
propai configure
propai configure --section model --section channels
```
