---
summary: "CLI reference for `propai onboard` (interactive onboarding wizard)"
read_when:
...
```bash
propai onboard
propai onboard --flow quickstart
propai onboard --flow manual
propai onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`PROPAI_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `propai dashboard` (Control UI, no channel setup).
