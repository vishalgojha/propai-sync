...
- Extend propai's behavior without modifying core code

## Getting Started

### Bundled Hooks

propai ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.propai/workspace/memory/`) when you issue `/new`
...
2. **Managed hooks**: `~/.propai/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<propai>/dist/hooks/bundled/` (shipped with propai)
...
Hook packs can ship dependencies; they will be installed under `~/.propai/hooks/<id>`.
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before propai persists them.
...
- **Managed hooks** (`~/.propai/hooks/`): Shared across workspaces
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.propai/workspace`)
