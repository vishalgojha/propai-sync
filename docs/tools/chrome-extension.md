---
summary: "Chrome extension: let propai drive your existing Chrome tab"
read_when:
...
The propai Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate propai-managed Chrome profile.
...
propai then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the propai release (npm package) as static files. There is no separate “build” step.

After upgrading propai:

- Re-run `propai browser extension install` to refresh the installed files under your propai state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

propai ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `PROPAI_GATEWAY_TOKEN`)
...
- Open the tab you want propai to control.
...
- `ON`: attached; propai can drive that tab.
...
Debugging: `propai sandbox explain`
...
`propai browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `propai browser extension install` first to copy the extension to a stable location under your propai state directory.
...
- **This is not isolated** like the dedicated propai-managed profile.
