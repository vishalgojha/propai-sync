---
summary: "Direct `propai agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `propai agent` (direct agent runs)

`propai agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `propai message --target`).
...
```bash
propai agent --to +15555550123 --message "status update"
propai agent --agent ops --message "Summarize logs"
...
propai agent --to +15555550123 --message "Summon reply" --deliver
