---
summary: "propai plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends propai with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core propai yet (or you want to keep optional features out of your main
install).
...
```bash
propai plugins install @propai/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@propai/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@propai/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@propai/zalouser`
- [Matrix](/channels/matrix) — `@propai/matrix`
- [Nostr](/channels/nostr) — `@propai/nostr`
- [Zalo](/channels/zalo) — `@propai/zalo`
- [Microsoft Teams](/channels/msteams) — `@propai/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

propai plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `propai/plugin-sdk` import when
authoring plugins:

- `propai/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `propai/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `propai/plugin-sdk/telegram` for Telegram channel plugins.
- `propai/plugin-sdk/discord` for Discord channel plugins.
- `propai/plugin-sdk/slack` for Slack channel plugins.
- `propai/plugin-sdk/signal` for Signal channel plugins.
- `propai/plugin-sdk/imessage` for iMessage channel plugins.
- `propai/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `propai/plugin-sdk/line` for LINE channel plugins.
- `propai/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `propai/plugin-sdk/acpx`, `propai/plugin-sdk/bluebubbles`,
  `propai/plugin-sdk/copilot-proxy`, `propai/plugin-sdk/device-pair`,
  `propai/plugin-sdk/diagnostics-otel`, `propai/plugin-sdk/diffs`,
  `propai/plugin-sdk/feishu`,
  `propai/plugin-sdk/google-gemini-cli-auth`, `propai/plugin-sdk/googlechat`,
  `propai/plugin-sdk/irc`, `propai/plugin-sdk/llm-task`,
  `propai/plugin-sdk/lobster`, `propai/plugin-sdk/matrix`,
  `propai/plugin-sdk/mattermost`, `propai/plugin-sdk/memory-core`,
  `propai/plugin-sdk/memory-lancedb`,
  `propai/plugin-sdk/minimax-portal-auth`,
  `propai/plugin-sdk/nextcloud-talk`, `propai/plugin-sdk/nostr`,
  `propai/plugin-sdk/open-prose`, `propai/plugin-sdk/phone-control`,
  `propai/plugin-sdk/qwen-portal-auth`, `propai/plugin-sdk/synology-chat`,
  `propai/plugin-sdk/talk-voice`, `propai/plugin-sdk/test-utils`,
  `propai/plugin-sdk/thread-ownership`, `propai/plugin-sdk/tlon`,
  `propai/plugin-sdk/twitch`, `propai/plugin-sdk/voice-call`,
  `propai/plugin-sdk/zalo`, and `propai/plugin-sdk/zalouser`.

Compatibility note:

- `propai/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `propai status`, `propai status --all`,
  `propai channels status`, `propai channels resolve`, and doctor/config
...
- Set `PROPAI_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `PROPAI_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `PROPAI_PLUGIN_DISCOVERY_CACHE_MS` and
  `PROPAI_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

propai scans, in order:
...
- `<workspace>/.propai/extensions/*.ts`
- `<workspace>/.propai/extensions/*/index.ts`

3. Global extensions

- `~/.propai/extensions/*.ts`
- `~/.propai/extensions/*/index.ts`

4. Bundled extensions (shipped with propai, mostly disabled by default)

- `<propai>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `propai plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, propai logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. propai blocks candidates when:
...
Each plugin must include a `propai.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `propai.extensions`:
...
Security guardrail: every `propai.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `propai plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `propai.channel` and
install hints via `propai.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@propai/nextcloud-talk",
  "propai": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
propai can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.propai/mpm/plugins.json`
- `~/.propai/mpm/catalog.json`
- `~/.propai/plugins/catalog.json`

Or point `PROPAI_PLUGIN_CATALOG_PATHS` (or `PROPAI_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "propai": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, propai uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `propai.plugin.json` (`configSchema`).
...
propai augments `uiHints` at runtime based on discovered plugins:
...
```bash
propai plugins list
propai plugins info <id>
propai plugins install <path>                 # copy a local file/dir into ~/.propai/extensions/<id>
propai plugins install ./extensions/voice-call # relative path ok
propai plugins install ./plugin.tgz           # install from a local tarball
propai plugins install ./plugin.zip           # install from a local zip
propai plugins install -l ./extensions/voice-call # link (no copy) for dev
propai plugins install @propai/voice-call # install from npm
propai plugins install @propai/voice-call --pin # store exact resolved name@version
propai plugins update <id>
propai plugins update --all
propai plugins enable <id>
propai plugins disable <id>
propai plugins doctor
```

`plugins update` only works for npm installs tracked under `plugins.installs`.
If stored integrity metadata changes between updates, propai warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `propai voicecall`).
...
- Plugin-managed hooks show up in `propai hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `propai hooks`; enable/disable the plugin instead.
...
- When disabled, propai blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside propai (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `propai models auth login --provider <id> [--method <id>]`
...
- `config`: The current propai config
...
- Main package: `propai` (this repo)
- Plugins: separate npm packages under `@propai/*` (example: `@propai/voice-call`)

Publishing contract:

- Plugin `package.json` must include `propai.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `propai plugins install <npm-spec>` uses `npm pack`, extracts into `~/.propai/extensions/<id>/`, and enables it in config.
...
- CLI: `propai voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `propai.extensions` points at the built entrypoint (`dist/index.js`).
