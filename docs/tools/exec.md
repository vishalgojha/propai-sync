...
- `gateway`/`node` approvals are controlled by `~/.propai/exec-approvals.json`.
...
- propai sets `PROPAI_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  propai prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`propai security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `propai doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
