...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `propai sandbox explain`.
...
Legacy `agent.*` configs are migrated by `propai doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=propai-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${PROPAI_STATE_DIR:-$HOME/.propai}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
