...
- **gateway host** → `propai` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, propai also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.propai/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.propai/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `propai approvals allowlist ...`).
- `propai security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `propai doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
CLI: `propai approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, propai does not treat
...
topics, propai preserves the topic for the approval prompt and the post-approval follow-up.
