---
summary: "Agent tool surface for propai (browser, canvas, nodes, message, cron) replacing legacy `propai-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `propai-*` skills
title: "Tools"
---

# Tools (propai)

propai exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `propai-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `propai.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, propai logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`propai node run`).
...
propai tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `propai configure --section web`).
...
Control the dedicated propai-managed browser.
...
- Quick smoke: `propai nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `propai gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, propai uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, propai clamps `tools.sessions.visibility` to `tree`.
