...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in propai
...
Running propai via Node with `tsx` fails at startup with:

```
[propai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `propai status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch propai.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node propai.mjs status` works on Node 25.
