...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/propai`)
...
If the page fails to load, start the Gateway first: `propai gateway`.
...
you revoke it with `propai devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.propai/propai.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop propai`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. propai
...
```bash
propai gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
propai **blocks** Control UI connections without device identity.
...
```bash
PROPAI_CONTROL_UI_BASE_PATH=/propai/ pnpm ui:build
```
