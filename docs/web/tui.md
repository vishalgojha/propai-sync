...
```bash
propai tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `propai tui --deliver`
...
- Local shell commands receive `PROPAI_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `PROPAI_THEME=light` before launching `propai tui`.
- To force the original dark palette instead, set `PROPAI_THEME=dark`.
...
- Check the Gateway logs: `propai logs --follow`.
- Confirm the agent can run: `propai status` and `propai models status`.
...
- No agents in picker: check `propai agents list` and your routing config.
