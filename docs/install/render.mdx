...
Deploy propai on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/propai/propai)
...
repository configures everything needed to run propai:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: PROPAI_STATE_DIR
        value: /data/.propai
      - key: PROPAI_WORKSPACE_DIR
        value: /data/workspace
      - key: PROPAI_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/propai`.
...
If you use the original propai repository, Render will not auto-deploy your propai. To update it, run a manual Blueprint sync from the dashboard.
...
For propai, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any propai host.
