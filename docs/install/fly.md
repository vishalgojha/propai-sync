...
summary: "Step-by-step Fly.io deployment for propai with persistent storage and HTTPS"
read_when:
  - Deploying propai on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** propai Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
| `internal_port = 3000`         | Must match `--port 3000` (or `PROPAI_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `PROPAI_STATE_DIR = "/data"` | Persists state on the volume                                                |
...
- Non-loopback binds (`--bind lan`) require `PROPAI_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `propai.json` where they could be accidentally exposed or logged.
...
**Note:** With `PROPAI_STATE_DIR=/data`, the config path is `/data/propai.json`.
...
Or visit `https://my-propai.fly.dev/`

Paste your gateway token (the one from `PROPAI_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `PROPAI_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/propai.json` should be read on restart.
...
**Fix:** Ensure `PROPAI_STATE_DIR=/data` is set in `fly.toml` and redeploy.
