---
summary: "Optional Docker-based setup and onboarding for propai"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run propai on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full propai in Docker)
...
- builds the gateway image locally (or pulls a remote image if `PROPAI_IMAGE` is set)
...
- `PROPAI_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/propai/propai:latest`)
- `PROPAI_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `PROPAI_EXTENSIONS` — pre-install extension dependencies at build time (space-separated extension names, e.g. `diagnostics-otel matrix`)
- `PROPAI_EXTRA_MOUNTS` — add extra host bind mounts
- `PROPAI_HOME_VOLUME` — persist `/home/node` in a named volume
- `PROPAI_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `PROPAI_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `PROPAI_SANDBOX=1` for local builds.
- `PROPAI_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `PROPAI_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `PROPAI_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `PROPAI_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `PROPAI_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm propai-cli dashboard --no-open`.
...
```bash
export PROPAI_SANDBOX=1
export PROPAI_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `propai-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `PROPAI_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm propai-cli gateway probe
docker compose run -T --rm propai-cli devices list --json
```
...
`propai-cli` uses `network_mode: "service:propai-gateway"` so CLI commands can
...
network path instead of the bundled `propai-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `propai-cli`.
...
- [GitHub Container Registry package](https://github.com/propai/propai/pkgs/container/propai)

Use image name `ghcr.io/propai/propai` (not similarly named Docker Hub
images).
...
image instead, set `PROPAI_IMAGE` before running the script:

```bash
export PROPAI_IMAGE="ghcr.io/propai/propai:latest"
./docker-setup.sh
```

The script detects that `PROPAI_IMAGE` is not the default `propai:local` and
...
`docker-compose.yml` and helper files. `PROPAI_IMAGE` skips local image build
...
See [`ClawDock` Helper README](https://github.com/propai/propai/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t propai:local -f Dockerfile .
docker compose run --rm propai-cli onboard
docker compose up -d propai-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`PROPAI_EXTRA_MOUNTS` or `PROPAI_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm propai-cli dashboard --no-open
docker compose run --rm propai-cli devices list
docker compose run --rm propai-cli devices approve <requestId>
```
...
`PROPAI_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`propai-gateway` and `propai-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export PROPAI_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `PROPAI_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `PROPAI_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export PROPAI_HOME_VOLUME="PROPAI_home"
./docker-setup.sh
```
...
- If you change `PROPAI_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `PROPAI_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export PROPAI_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `PROPAI_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
dependencies into the image instead, set `PROPAI_EXTENSIONS` before
running `docker-setup.sh`:

```bash
export PROPAI_EXTENSIONS="diagnostics-otel matrix"
./docker-setup.sh
```
...
```bash
docker build --build-arg PROPAI_EXTENSIONS="diagnostics-otel matrix" .
```
...
- If you change `PROPAI_EXTENSIONS`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export PROPAI_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`PROPAI_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `PROPAI_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `PROPAI_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.propai`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/propai-config /path/to/propai-workspace
```
...
```bash
docker compose run --rm propai-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm propai-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm propai-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if propai is still
...
```bash
docker compose exec propai-gateway node dist/index.js health --token "$PROPAI_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `PROPAI_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm propai-cli config set gateway.mode local
docker compose run --rm propai-cli config set gateway.bind lan
docker compose run --rm propai-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`PROPAI_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.propai/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.propai/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  propai auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `propai sandbox recreate ...` command.
...
This builds `propai-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `propai-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "propai-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `propai-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`propai-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; propai provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `PROPAI_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `PROPAI_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `PROPAI_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-propai-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-propai-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-propai-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/propai/propai/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: propai runs commands with `sh -lc` (login shell), which
