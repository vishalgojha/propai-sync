---
summary: "propai on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up propai on DigitalOcean
  - Looking for cheap VPS hosting for propai
title: "DigitalOcean"
---

# propai on DigitalOcean

## Goal

Run a persistent propai Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status propai-gateway.service

# View logs
journalctl --user -u propai-gateway.service -f
```
...
```bash
propai config set gateway.bind tailnet
propai gateway restart
```
...
```bash
propai channels login whatsapp
# Scan QR code
```
...
- `~/.propai/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u propai --no-pager -n 50
```
