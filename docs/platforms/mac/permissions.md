...
- Same path: run the app from a fixed location (for propai, `dist/propai.app`).
...
```bash
sudo tccutil reset Accessibility ai.propai.mac
sudo tccutil reset ScreenCapture ai.propai.mac
...
Workaround: move files into the propai workspace (`~/.propai/workspace`) if you want to avoid per-folder grants.
