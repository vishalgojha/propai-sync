...
service via the external `propai` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.propai.gateway`
  (or `ai.propai.<profile>` when using `--profile`/`PROPAI_PROFILE`; legacy `com.propai.*` is supported).
