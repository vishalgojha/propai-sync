---
summary: "propai on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up propai on a Raspberry Pi
  - Running propai on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# propai on Raspberry Pi

## Goal

Run a persistent, always-on propai Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u propai -f
```

## 9) Access the propai Dashboard
...
```bash
ssh user@gateway-host 'propai dashboard --no-open'
```
...
(or `PROPAI_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/propai-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/propai-compile-cache
mkdir -p /var/tmp/propai-compile-cache
export PROPAI_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `PROPAI_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running propai, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit propai
```

```ini
[Service]
Environment=PROPAI_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/propai-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep propai state/cache on SSD-backed storage to avoid SD-card
...
Most propai features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled propai

# Enable if not
sudo systemctl enable propai

# Start on boot
sudo systemctl start propai
```
...
```bash
# Check logs
journalctl -u propai --no-pager -n 100

# Common fix: rebuild
cd ~/propai  # if using hackable install
npm run build
sudo systemctl restart propai
```
