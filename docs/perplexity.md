...
propai supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, propai also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `propai configure --section web`. It stores the key in
`~/.propai/propai.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.propai/.env` (or your service environment). See [Env vars](/help/faq#how-does-propai-load-environment-variables).
