...
This plan defines how propai should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- propai core owns ACP control plane concerns
...
propai should not reimplement ACP transport internals in core.
propai should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in propai, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under propai state dir
...
- ACP event stream is normalized to propai reply chunks
...
For the first production backend (`extensions/acpx`), propai and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses propai `sessionKey`)
...
- propai consumes ndjson events from `acpx --format json --json-strict`
