...
Hooks 提供了一个可扩展的事件驱动系统，用于响应智能体命令和事件自动执行操作。Hooks 从目录中自动发现，可以通过 CLI 命令管理，类似于 propai 中 Skills 的工作方式。
...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 propai 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `propai webhooks` 获取 Gmail 助手命令。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 propai 的行为

## 入门

### 捆绑的 Hooks

propai 附带三个自动发现的捆绑 hooks：

- **💾 session-memory**：当你发出 `/new` 时将会话上下文保存到智能体工作区（默认 `~/.propai/workspace/memory/`）
- **📝 command-logger**：将所有命令事件记录到 `~/.propai/logs/commands.log`
...
在新手引导期间（`propai onboard`），你将被提示启用推荐的 hooks。向导会自动发现符合条件的 hooks 并呈现供选择。
...
2. **托管 hooks**：`~/.propai/hooks/`（用户安装，跨工作区共享）
3. **捆绑 hooks**：`<propai>/dist/hooks/bundled/`（随 propai 附带）
...
Hook 包是标准的 npm 包，通过 `package.json` 中的 `propai.hooks` 导出一个或多个 hooks。使用以下命令安装：
...
Hook 包可以附带依赖；它们将安装在 `~/.propai/hooks/<id>` 下。
...
这些 hooks 不是事件流监听器；它们让插件在 propai 持久化工具结果之前同步调整它们。
...
- **托管 hooks**（`~/.propai/hooks/`）：跨工作区共享

### 2. 创建目录结构

```bash
mkdir -p ~/.propai/hooks/my-hook
cd ~/.propai/hooks/my-hook
```
...
metadata: { "propai": { "emoji": "🎯", "events": ["command:new"] } }
---
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认为 `~/.propai/workspace`）
...
```yaml
metadata: { "propai": { "events": ["command:new"] } } # Specific
```

而不是：

```yaml
metadata: { "propai": { "events": ["command"] } } # General - more overhead
```
...
2. 验证 HOOK.md 格式：
   ...
   ```bash
   cat ~/.propai/hooks/my-hook/HOOK.md
...
1. 创建 hook 目录：
   ...
   ```bash
   mkdir -p ~/.propai/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.propai/hooks/my-hook/handler.ts
   ```
...
- [捆绑 Hooks README](https://github.com/propai/propai/tree/main/src/hooks/bundled)
