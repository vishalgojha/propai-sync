...
read_when:
  - 启动新的 propai 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 propai 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — propai 个人助手（默认）

## 首次运行（推荐）

propai 为智能体使用专用的工作区目录。默认：`~/.propai/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- 脚本编写优先使用 `propai` CLI；mac 应用处理权限。
...
- 对于浏览器驱动的验证，使用带有 propai 管理的 Chrome 配置文件的 `propai browser`（tabs/status/screenshot）。
- 对于 DOM 检查，使用 `propai browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `propai browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要 snapshot 引用；CSS 选择器使用 `evaluate`）。
