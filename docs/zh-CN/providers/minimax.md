---
read_when:
  - 你想在 propai 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 propai 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
propai plugins enable minimax-portal-auth  # 如果已加载则跳过
propai gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/propai/propai/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `propai models list` 和 `propai models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `propai configure` 并选择 **MiniMax M2.1**，或
