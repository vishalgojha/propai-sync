...
通过一键模板在 Railway 上部署 propai，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/propai` — 控制面板 UI
...
- `PROPAI_STATE_DIR=/data/.propai`（推荐）
- `PROPAI_WORKSPACE_DIR=/data/workspace`（推荐）
- `PROPAI_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 propai 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
