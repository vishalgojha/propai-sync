...
- `propai health` 和 `propai status --deep` 现在通过 `ws://127.0.0.1:18789` 访问远程 Gateway 网关。
- `propai gateway {status,health,send,agent,call}` 在需要时也可以通过 `--url` 指定转发的 URL。

注意：将 `18789` 替换为你配置的 `gateway.port`（或 `--port`/`PROPAI_GATEWAY_PORT`）。
