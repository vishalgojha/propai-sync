...
你可以在 `~/.propai/propai.json` 中覆盖此设置：
...
所有日志配置都在 `~/.propai/propai.json` 的 `logging` 下。
...
- propai 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `propai plugins enable diagnostics-otel` 启用插件。
...
- `propai.tokens`（计数器，属性：`propai.token`、`propai.channel`、`propai.provider`、`propai.model`）
- `propai.cost.usd`（计数器，属性：`propai.channel`、`propai.provider`、`propai.model`）
- `propai.run.duration_ms`（直方图，属性：`propai.channel`、`propai.provider`、`propai.model`）
- `propai.context.tokens`（直方图，属性：`propai.context`、`propai.channel`、`propai.provider`、`propai.model`）

消息流：

- `propai.webhook.received`（计数器，属性：`propai.channel`、`propai.webhook`）
- `propai.webhook.error`（计数器，属性：`propai.channel`、`propai.webhook`）
- `propai.webhook.duration_ms`（直方图，属性：`propai.channel`、`propai.webhook`）
- `propai.message.queued`（计数器，属性：`propai.channel`、`propai.source`）
- `propai.message.processed`（计数器，属性：`propai.channel`、`propai.outcome`）
- `propai.message.duration_ms`（直方图，属性：`propai.channel`、`propai.outcome`）

队列 + 会话：

- `propai.queue.lane.enqueue`（计数器，属性：`propai.lane`）
- `propai.queue.lane.dequeue`（计数器，属性：`propai.lane`）
- `propai.queue.depth`（直方图，属性：`propai.lane` 或 `propai.channel=heartbeat`）
- `propai.queue.wait_ms`（直方图，属性：`propai.lane`）
- `propai.session.state`（计数器，属性：`propai.state`、`propai.reason`）
- `propai.session.stuck`（计数器，属性：`propai.state`）
- `propai.session.stuck_age_ms`（直方图，属性：`propai.state`）
- `propai.run.attempt`（计数器，属性：`propai.attempt`）
...
- `propai.webhook.error`
  - `propai.channel`、`propai.webhook`、`propai.chatId`、`propai.error`
- `propai.message.processed`
  - `propai.channel`、`propai.outcome`、`propai.chatId`、`propai.messageId`、`propai.sessionKey`、`propai.sessionId`、`propai.reason`
- `propai.session.stuck`
  - `propai.state`、`propai.ageMs`、`propai.queueDepth`、`propai.sessionKey`、`propai.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `propai doctor`。
