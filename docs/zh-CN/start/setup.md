...
summary: 设置指南：在保持最新的同时保持你的 propai 设置个性化
title: 设置
x-i18n:
...
- **配置：** `~/.propai/propai.json`（JSON/JSON5 格式）
- **工作区：** `~/.propai/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
- 运行 `propai setup`，然后 `propai channels login`，然后手动启动 Gateway 网关（`propai gateway`）。
...
- **配对允许列表**：`~/.propai/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.propai/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.propai/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.propai/workspace` 和 `~/.propai/` 保持为"你的东西"；不要将个人提示/配置放入 `propai` 仓库。
