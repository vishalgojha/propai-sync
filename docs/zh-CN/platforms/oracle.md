---
read_when:
  - 在 Oracle Cloud 上设置 propai
  - 寻找 propai 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 propai
summary: 在 Oracle Cloud 上运行 propai（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 propai

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 propai Gateway 网关。

Oracle 的免费层非常适合 propai（特别是如果你已经有 OCI 账户），但有一些权衡：
...
这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh propai` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@propai`（或使用 Tailscale IP）。
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `propai security audit`，并验证你没有意外地在公共接口上监听。
...
- **凭证权限：** `chmod 700 ~/.propai`
- **安全审计：** `propai security audit`
...
- `~/.propai/` — 配置、凭证、会话数据
- `~/.propai/workspace/` — 工作区（SOUL.md、记忆、产物）
