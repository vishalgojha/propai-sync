...
- 你可以在 Gateway 网关机器上运行 CLI（`propai`）（或通过 SSH）。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `propai.internal.`）并发布 `_propai-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.propai/workspace/canvas/index.html`。

2. 将节点导航到它（局域网）：

```bash
propai nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__PROPAI__/canvas/"}'
```

Tailnet（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 而不是 `.local`，例如 `http://<gateway-magicdns>:18793/__PROPAI__/canvas/`。

此服务器将实时重载客户端注入 HTML 并在文件更改时重新加载。
A2UI 主机位于 `http://<gateway-host>:18793/__PROPAI__/a2ui/`。
