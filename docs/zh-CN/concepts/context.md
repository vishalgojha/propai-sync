...
"上下文"是 **propai 在一次运行中发送给模型的所有内容**。它受模型的**上下文窗口**（token 限制）约束。

新手心智模型：

- **系统提示词**（propai 构建）：规则、工具、Skills 列表、时间/运行时，以及注入的工作区文件。
...
默认情况下，propai 注入一组固定的工作区文件（如果存在）：
