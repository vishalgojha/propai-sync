---
summary: "Use MiniMax M2.5 in propai"
read_when:
  - You want MiniMax models in propai
...
```bash
propai plugins enable minimax-portal-auth  # skip if already loaded.
propai gateway restart  # restart if gateway is already running
propai onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/propai/propai/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `propai.json`:
...
- Use `propai models list` and `propai models set minimax/MiniMax-M2.5` to switch.
...
- Running `propai configure` and selecting **MiniMax M2.5**, or
