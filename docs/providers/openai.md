---
summary: "Use OpenAI via API keys or Codex subscription in propai"
read_when:
  - You want to use OpenAI models in propai
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like propai.
...
```bash
propai onboard --auth-choice openai-api-key
# or non-interactive
propai onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. propai forwards both through the `openai/*` Responses path.
...
propai onboard --auth-choice openai-codex

# Or run OAuth directly
propai models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. propai
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

propai uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), propai also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. propai enables it by default for
...
propai, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), propai now auto-enables OpenAI server-side
