...
propai supports **camera capture** for agent workflows:
...
```bash
propai nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
propai nodes camera snap --node <id> --facing front
propai nodes camera clip --node <id> --duration 3000
propai nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`propai.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `propai` CLI to invoke camera commands on the macOS node.

Examples:

```bash
propai nodes camera list --node <id>            # list camera ids
propai nodes camera snap --node <id>            # prints MEDIA:<path>
propai nodes camera snap --node <id> --max-width 1280
propai nodes camera snap --node <id> --delay-ms 2000
propai nodes camera snap --node <id> --device-id <id>
propai nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
propai nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
propai nodes camera clip --node <id> --device-id <id>
propai nodes camera clip --node <id> --no-audio
```

Notes:

- `propai nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
propai nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
