...
read_when:
  - Setting up Zalo Personal for propai
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside propai.
...
- Install via CLI: `propai plugins install @propai/zalouser`
- Or from a source checkout: `propai plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `propai channels login --channel zalouser`
...
```bash
propai directory self --channel zalouser
propai directory peers list --channel zalouser --query "name"
propai directory groups list --channel zalouser --query "work"
```
...
- `propai pairing list zalouser`
- `propai pairing approve zalouser <code>`
...
- On startup, propai resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, propai stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in propai state. Example:
...
- propai sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, propai sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `propai channels logout --channel zalouser && propai channels login --channel zalouser`
...
- The channel now runs fully in propai without external CLI binaries.
