...
- DM messages can carry `message_thread_id`; propai routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    propai can stream partial replies in real time:
    ...
    - DM: propai keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: propai keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), propai falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, propai skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, propai automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, propai retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - propai pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.propai/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, propai enqueues system events like:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, propai preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, propai now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `PROPAI_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `PROPAI_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `PROPAI_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
