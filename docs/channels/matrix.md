...
Matrix is an open, decentralized messaging protocol. propai connects as a Matrix **user**
...
```bash
propai plugins install @propai/matrix
```

Local checkout (when running from a git repo):

```bash
propai plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `propai plugins install @propai/matrix`
   - From a local checkout: `propai plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: propai calls the same
     login endpoint, stores the access token in `~/.propai/credentials/matrix/credentials.json`,
...
- On first connection, propai requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  propai logs a warning.
...
`~/.propai/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `propai pairing list matrix`
  - `propai pairing approve matrix <CODE>`
...
- On startup, propai resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
