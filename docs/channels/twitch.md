...
read_when:
  - Setting up Twitch chat integration for propai
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. propai connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
propai plugins install @propai/twitch
```

Local checkout (when running from a git repo):

```bash
propai plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `PROPAI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "propai", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use PROPAI_TWITCH_ACCESS_TOKEN env var)
...
```bash
PROPAI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
