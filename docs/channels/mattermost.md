---
summary: "Mattermost bot setup and propai config"
read_when:
...
```bash
propai plugins install @propai/mattermost
```

Local checkout (when running from a git repo):

```bash
propai plugins install ./extensions/mattermost
```
...
4. Configure propai and start the gateway.
...
Native slash commands are opt-in. When enabled, propai registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, propai derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as propai.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to propai.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from propai, not `404`.
...
- Approve via:
  - `propai pairing list mattermost`
  - `propai pairing approve mattermost <CODE>`
...
Use these target formats with `propai message send` or cron/webhooks:
...
propai resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), propai sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, propai derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and propai run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="propai-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"propai-mattermost-interactions",
...
`propai message send` and cron/webhook deliveries.
