...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects propai to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current propai releases, so no separate plugin install
is required.
...
```bash
propai plugins install @propai/feishu
```
...
If you just installed propai, run the wizard:
...
2. Configuring app credentials in propai
...
1. You already ran `propai channels add` for Feishu
2. The gateway is running (`propai gateway status`)
...
```bash
propai pairing approve feishu <CODE>
```
...
2. Run `propai logs --follow` and look for `chat_id`
...
2. Run `propai logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `propai gateway status`  | Show gateway status           |
| `propai gateway install` | Install/start gateway service |
| `propai gateway stop`    | Stop gateway service          |
| `propai gateway restart` | Restart gateway service       |
| `propai logs --follow`   | Tail gateway logs             |
...
4. Check logs: `propai logs --follow`
...
5. Ensure the gateway is running: `propai gateway status`
6. Check logs: `propai logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.propai/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.propai/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
