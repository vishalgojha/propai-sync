...
Tlon is a decentralized messenger built on Urbit. propai connects to your Urbit ship and can
...
```bash
propai plugins install @propai/tlon
```

Local checkout (when running from a git repo):

```bash
propai plugins install ./extensions/tlon
```
...
By default, propai blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `propai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, propai replies in-thread.
