...
read_when:
  - You want propai to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables propai to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`propai onboard`) and `propai channels add` list optional channel plugins.
...
```bash
propai plugins install @propai/nostr
```

Use a local checkout (dev workflows):

```bash
propai plugins install --link <path-to-propai>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "propai@example.com",
        "lud16": "propai@example.com"
      }
    }
  }
}
