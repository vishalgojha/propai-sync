...
```bash
propai plugins install @propai/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
propai plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "propai" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `propai pairing list nextcloud-talk`
  - `propai pairing approve nextcloud-talk <CODE>`
