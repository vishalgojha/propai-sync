...
read_when:
  - Running propai behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of propai
...
- You run propai behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. propai checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. propai extracts the user identity from the configured header
...
- propai can stay on loopback HTTP behind the proxy.
...
If propai itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://propai.example.com
    to: http://propai-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`propai security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update propai config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `propai security audit` and review findings
