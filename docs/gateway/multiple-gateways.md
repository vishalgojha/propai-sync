---
summary: "Run multiple propai Gateways on one host (isolation, ports, and profiles)"
read_when:
...
- `PROPAI_CONFIG_PATH` — per-instance config file
- `PROPAI_STATE_DIR` — per-instance sessions, creds, caches
...
Profiles auto-scope `PROPAI_STATE_DIR` + `PROPAI_CONFIG_PATH` and suffix service names.
...
Base port = `gateway.port` (or `PROPAI_GATEWAY_PORT` / `--port`).
