...
- Default rolling log file is under `/tmp/propai/` (one file per day): `propai-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.propai/propai.json`:
  - `logging.file`
  - `logging.level`
...
`propai gateway` supports a per-gateway style switch:
