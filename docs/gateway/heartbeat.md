...
- During heartbeat runs, propai treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `propai sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, propai skips the heartbeat run entirely (no model call).
...
```bash
propai system event --text "Check for urgent follow-ups" --mode now
```
