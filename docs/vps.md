---
summary: "VPS hosting hub for propai (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/propai-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `PROPAI_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
