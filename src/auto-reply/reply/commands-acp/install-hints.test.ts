...
function withAcpConfig(acp: PropAiSyncConfig["acp"]): PropAiSyncConfig {
  return { acp } as PropAiSyncConfig;
}
...
describe("ACP install hints", () => {
  it("prefers explicit runtime install command", () => {
    const cfg = withAcpConfig({
      runtime: { installCommand: "pnpm PropAi Sync plugins install acpx" },
    });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("pnpm PropAi Sync plugins install acpx");
  });
  ...
  it("uses local acpx extension path when present", () => {
    ...
    expect(hint).toContain("PropAi Sync plugins install ");
  ...
  it("falls back to npm install hint for acpx when local extension is absent", () => {
    ...
    const cfg = withAcpConfig({ backend: "acpx" });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("PropAi Sync plugins install acpx");
  });
