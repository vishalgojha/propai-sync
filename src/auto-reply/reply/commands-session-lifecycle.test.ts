...
describe("/session idle and /session max-age", () => {
  ...
  it("shows active idle timeout when no value is provided", async () => {
    ...
    const result = await handleSessionCommand(createTelegramCommandParams("/session idle"), true);
  ...
  it("disables max age when set to off", async () => {
    ...
    const result = await handleSessionCommand(
      createTelegramCommandParams("/session max-age off"),
      true,
    );
  ...
  it("requires binding owner for lifecycle updates", async () => {
    ...
    expect(hoisted.setTelegramThreadBindingIdleTimeoutBySessionKeyMock).not.toHaveBeenCalled();
