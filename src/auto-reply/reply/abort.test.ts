...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "propai-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop PropAi Sync!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@PROPAI_bot", { botUsername: "PROPAI_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@PROPAI_bot", { botUsername: "PROPAI_bot" })).toBe(true);
