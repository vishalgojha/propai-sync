...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-commands-"));
...
function buildParams(commandBody: string, cfg: PropAiSyncConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/propai-session-store.json" },
    } as PropAiSyncConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/propai-agent-compact";
...
describe("handleCommands /config configWrites gating", () => {
  ...
  it("keeps /config set working for gateway operator.admin on protected account paths", async () => {
    ...
    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("Config updated");
    const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as PropAiSyncConfig;
