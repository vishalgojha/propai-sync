...
import { scheduleGatewaySigusr1Restart, triggerPropAiSyncRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting PropAi Sync in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPropAiSyncRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting PropAi Sync via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
