...
type AgentDefaults = NonNullable<NonNullable<PropAiSyncConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.propai/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 PropAi Sync ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: PropAiSyncConfig): string {
  const lines = ["ℹ️ Help", ""];
