  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your PropAi Sync state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your PropAi Sync state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectLoggingFindings(cfg: PropAiSyncConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: PropAiSyncConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: PropAiSyncConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
