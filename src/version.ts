...
declare const __PROPAI_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "PropAi Sync";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["PROPAI_VERSION"],
      runtimeVersion,
      env["PROPAI_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current PropAi Sync version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __PROPAI_VERSION__ === "string" ? __PROPAI_VERSION__ : undefined,
  bundledVersion: process.env.propai_BUNDLED_VERSION,
});
