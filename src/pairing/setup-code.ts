...
function resolveGatewayTokenFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return env.propai_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
}

function resolveGatewayPasswordFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    env.propai_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim() || undefined
  );
}

function resolveAuth(cfg: PropAiSyncConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
...
async function resolveGatewayTokenSecretRef(
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.propai_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
    );
