...
export const DEFAULT_LOG_DIR = resolvePreferredPropAiSyncTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "PropAiSync.log"); // legacy single-file path

const LOG_PREFIX = "PropAi Sync";
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: PropAiSyncConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg && !shouldSkipLoadConfigFallback()) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => PropAiSyncConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.propai_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[PropAi Sync] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
