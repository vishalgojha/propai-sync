...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.propai_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    PROPAI_PROFILE: profile,
    PROPAI_GATEWAY_PORT: String(port),
    PROPAI_LAUNCHD_LABEL: resolvedLaunchdLabel,
    PROPAI_SYSTEMD_UNIT: systemdUnit,
    PROPAI_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    PROPAI_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    PROPAI_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.propai_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    PROPAI_GATEWAY_TOKEN: gatewayToken,
    PROPAI_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    PROPAI_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    PROPAI_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    PROPAI_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    PROPAI_LOG_PREFIX: "node",
    PROPAI_SERVICE_MARKER: NODE_SERVICE_MARKER,
    PROPAI_SERVICE_KIND: NODE_SERVICE_KIND,
    PROPAI_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    PROPAI_STATE_DIR: sharedEnv.stateDir,
    PROPAI_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.propai_STATE_DIR;
  const configPath = env.propai_CONFIG_PATH;
