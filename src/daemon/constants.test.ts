...
describe("resolveGatewayLaunchAgentLabel", () => {
  it("returns default label when no profile is set", () => {
    ...
    expect(result).toBe("ai.propai.gateway");
  });

  it("returns profile-specific label when profile is set", () => {
    const result = resolveGatewayLaunchAgentLabel("dev");
    expect(result).toBe("ai.propai.dev");
  });
});
...
describe("resolveGatewaySystemdServiceName", () => {
  it("returns default service name when no profile is set", () => {
    ...
    expect(result).toBe("propai-gateway");
  });

  it("returns profile-specific service name when profile is set", () => {
    const result = resolveGatewaySystemdServiceName("dev");
    expect(result).toBe("propai-gateway-dev");
  });
});
...
describe("resolveGatewayWindowsTaskName", () => {
  it("returns default task name when no profile is set", () => {
    ...
    expect(result).toBe("PropAi Sync Gateway");
  });

  it("returns profile-specific task name when profile is set", () => {
    const result = resolveGatewayWindowsTaskName("dev");
    expect(result).toBe("PropAi Sync Gateway (dev)");
  });
});
...
describe("resolveGatewayServiceDescription", () => {
  it("prefers explicit description override", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { PROPAI_PROFILE: "work", PROPAI_SERVICE_VERSION: "1.0.0" },
        description: "Custom",
      }),
    ).toBe("Custom");
  });
  ...
  it("resolves version from explicit environment map", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { PROPAI_PROFILE: "work", PROPAI_SERVICE_VERSION: "local" },
        environment: { PROPAI_SERVICE_VERSION: "remote" },
      }),
    ).toBe("PropAi Sync Gateway (profile: work, vremote)");
  });
});
