...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.propai_GATEWAY_PORT).toBe("18789");
    expect(env.propai_GATEWAY_TOKEN).toBeUndefined();
    expect(env.propai_SERVICE_MARKER).toBe("PropAi Sync");
    expect(env.propai_SERVICE_KIND).toBe("gateway");
    expect(typeof env.propai_SERVICE_VERSION).toBe("string");
    expect(env.propai_SYSTEMD_UNIT).toBe("propai-gateway.service");
    expect(env.propai_WINDOWS_TASK_NAME).toBe("PropAi Sync Gateway");
    if (process.platform === "darwin") {
      expect(env.propai_LAUNCHD_LABEL).toBe("ai.propai.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", PROPAI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.propai_SYSTEMD_UNIT).toBe("propai-gateway-work.service");
    expect(env.propai_WINDOWS_TASK_NAME).toBe("PropAi Sync Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.propai_LAUNCHD_LABEL).toBe("ai.propai.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through PROPAI_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", PROPAI_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.propai_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to PROPAI_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.propai_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers PROPAI_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        PROPAI_GATEWAY_TOKEN: "propai-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.propai_GATEWAY_TOKEN).toBe("propai-token");
  });

  it("omits PROPAI_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".propai"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", PROPAI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".propai-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", PROPAI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".propai"));
  });

  it("uses PROPAI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", PROPAI_STATE_DIR: "/var/lib/PropAiSync" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/PropAiSync"));
  });

  it("expands ~ in PROPAI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", PROPAI_STATE_DIR: "~/propai-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/propai-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { PROPAI_STATE_DIR: "C:\\State\\\PropAiSync" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\\PropAiSync");
  });
});
