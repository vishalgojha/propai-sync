...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `propai-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      PROPAI_LAUNCHD_LABEL: `ai.propai.launchd-int-${testId}`,
      PROPAI_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
