...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false when systemctl is not present", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/propai-test-home" } });
  ...
  it("calls systemctl is-enabled when systemctl is present", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "propai-gateway.service"]);
      cb(null, "enabled", "");
    });
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/propai-test-home" } });
  ...
  it("returns false for the WSL2 Ubuntu 24.04 wrapper-only is-enabled failure", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "propai-gateway.service"]);
      const err = new Error(
        "Command failed: systemctl --user is-enabled propai-gateway.service",
    ...
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/propai-test-home" } }),
    ).rejects.toThrow(
      "systemctl is-enabled unavailable: Command failed: systemctl --user is-enabled propai-gateway.service",
    );
  });
  ...
  it("returns false when is-enabled cannot connect to the user bus without machine fallback", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/propai-test-home", USER: "", LOGNAME: "" },
      }),
  ...
  it("returns false when both direct and machine-scope is-enabled checks report bus unavailability", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/propai-test-home", USER: "debian" },
      }),
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "propai-gateway.service"]);
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled propai-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when PROPAI_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/propai-gateway.service",
    },
    {
      name: "uses profile-specific service name when PROPAI_PROFILE is set to a custom value",
      env: { HOME: "/home/test", PROPAI_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/propai-gateway-jbphoenix.service",
    },
    {
      name: "prefers PROPAI_SYSTEMD_UNIT over PROPAI_PROFILE",
      env: {
        HOME: "/home/test",
        PROPAI_PROFILE: "jbphoenix",
        PROPAI_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles PROPAI_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        PROPAI_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from PROPAI_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        PROPAI_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/PropAiSync gateway start --name "My Bot"')).toEqual([
      "/usr/bin/PropAiSync",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('PropAi Sync --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["PropAi Sync", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('PropAi Sync --path "C:\\\\Program Files\\\\\PropAiSync"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["PropAi Sync", "--path", "C:\\\\Program Files\\\\\PropAiSync"]);

    expect(
      splitArgsPreservingQuotes('PropAi Sync --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["PropAi Sync", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/PropAiSync gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads PROPAI_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/propai-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/PropAiSync gateway run",
          "EnvironmentFile=%h/.propai/.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.propai/.env") {
        return "PROPAI_GATEWAY_TOKEN=env-file-token\n";
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });

    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment?.propai_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/propai-gateway.service")) {
        return [
          ...
          "ExecStart=/usr/bin/PropAiSync gateway run",
          "EnvironmentFile=%h/.propai/.env",
          'Environment="PROPAI_GATEWAY_TOKEN=inline-token"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.propai/.env") {
        return "PROPAI_GATEWAY_TOKEN=env-file-token\n";
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment?.propai_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.propai_GATEWAY_TOKEN).toBe("file");
  });
  ...
  it("ignores missing optional EnvironmentFile entries", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/propai-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/PropAiSync gateway run",
          "EnvironmentFile=-%h/.propai/missing.env",
        ].join("\n");
      }
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.programArguments).toEqual(["/usr/bin/PropAiSync", "gateway", "run"]);
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/propai-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/PropAiSync gateway run",
          'EnvironmentFile=%h/.propai/first.env "%h/.propai/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.propai/first.env") {
        return "PROPAI_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.propai/second env.env") {
        return 'PROPAI_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      PROPAI_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "PROPAI_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "PROPAI_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "PROPAI_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      PROPAI_GATEWAY_TOKEN: "override-token",
      PROPAI_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/propai-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/PropAiSync gateway run",
          "EnvironmentFile=%h/.propai/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.propai/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'PROPAI_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "PROPAI_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      PROPAI_GATEWAY_TOKEN: "quoted token",
      PROPAI_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      PROPAI_GATEWAY_TOKEN: "file",
      PROPAI_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  const assertMachineRestartArgs = (args: string[]) => {
    expect(args).toEqual(["--machine", "debian@", "--user", "restart", "propai-gateway.service"]);
  };
  ...
  it("stops the resolved user unit", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "stop", "propai-gateway.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "propai-gateway-work.service"]);
        cb(null, "", "");
      });
    await assertRestartSuccess({ PROPAI_PROFILE: "work" });
  });
  ...
  it("keeps direct --user scope when SUDO_USER is root", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "propai-gateway.service"]);
        cb(null, "", "");
      });
