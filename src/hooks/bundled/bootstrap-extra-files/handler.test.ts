...
function createBootstrapExtraConfig(paths: string[]): PropAiSyncConfig {
  return {
    hooks: {
      internal: {
        entries: {
...
describe("bootstrap-extra-files hook", () => {
  it("appends extra bootstrap files from configured patterns", async () => {
    const tempDir = await makeTempWorkspace("propai-bootstrap-extra-");
  ...
  it("re-applies subagent bootstrap allowlist after extras are added", async () => {
    const tempDir = await makeTempWorkspace("propai-bootstrap-extra-subagent-");
