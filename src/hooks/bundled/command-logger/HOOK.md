...
homepage: https://docs.propai.ai/automation/hooks#command-logger
metadata:
  {
    "PropAi Sync":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.propai/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.propai/logs/commands.log ~/.propai/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/PropAiSync`:
   ```
   /home/username/.propai/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
