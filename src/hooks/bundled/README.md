...
This directory contains hooks that ship with PropAi Sync. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
```bash
PropAi Sync hooks enable session-memory
```
...
```bash
PropAi Sync hooks enable bootstrap-extra-files
```
...
```bash
PropAi Sync hooks enable command-logger
```
...
```bash
PropAi Sync hooks enable boot-md
```
...
metadata:
  { "PropAi Sync": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
```bash
PropAi Sync hooks list
```

Show hook details:

```bash
PropAi Sync hooks info session-memory
```

Check hook status:

```bash
PropAi Sync hooks check
```
...
```bash
PropAi Sync hooks enable session-memory
PropAi Sync hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.propai/propai.json`:
...
2. Restart gateway: `pkill -9 -f 'PropAi Sync.*gateway' && pnpm PropAi Sync gateway`
3. Enable the hook: `propai hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.propai.ai/automation/hooks
