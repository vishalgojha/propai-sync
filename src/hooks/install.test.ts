...
const fixtureRoot = path.join(os.tmpdir(), `propai-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@propai/test-hooks",
      version: "0.0.1",
      "PropAi Sync": { hooks: params.hooks },
  ...
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"PropAi Sync":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@propai/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@propai/test-hooks@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@propai/test-hooks@0.0.2-beta.1",
      name: "@propai/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@propai/test-hooks@beta"');
    }
  });
});
