...
const { makeStorePath } = createCronStoreHarness({ prefix: "propai-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses PROPAI_HOME for tilde expansion", () => {
    vi.stubEnv("PROPAI_HOME", "/srv/propai-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/propai-home", "cron", "jobs.json"));
  });
});
