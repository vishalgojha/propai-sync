...
<title>PropAi Sync Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.propaiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.propaiCanvasA2UIAction &&
        typeof window.propaiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.propaiSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("PropAi Sync:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android PropAi Sync node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.propaiSendUserAction === "function"
        ? window.propaiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.propai_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.propai_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>PropAi Sync Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
