...
import { discoverPropAiSyncPlugins } from "./discovery.js";
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`propai/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PropAiSyncPluginDefinition;
  register?: PropAiSyncPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PropAiSyncPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PropAiSyncPluginDefinition;
...
export function loadPropAiSyncPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverPropAiSyncPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "propai/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: PropAiSyncPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as PropAiSyncPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
