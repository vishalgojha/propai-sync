...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing PropAiSync.extensions; update the plugin package to include PropAiSync.extensions (for example ["./dist/index.js"]). See https://docs.propai.ai/help/troubleshooting#plugin-install-fails-with-missing-propai-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_PROPAI_EXTENSIONS: "missing_PROPAI_extensions",
  EMPTY_PROPAI_EXTENSIONS: "empty_PROPAI_extensions",
...
function ensurePropAiSyncExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_PROPAI_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json PropAiSync.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_PROPAI_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensurePropAiSyncExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from PropAiSync.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-PropAi Sync"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "PropAi Sync security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "PropAi Sync security audit --deep" after install.`,
    );
  }
