...
import type { PropAiSyncPluginService, PropAiSyncPluginServiceContext } from "./types.js";
...
function createRegistry(services: PropAiSyncPluginService[]) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: PropAiSyncPluginServiceContext[] = [];

    const serviceA: PropAiSyncPluginService = {
      id: "service-a",
      start: (ctx) => {
        starts.push("a");
        contexts.push(ctx);
    ...
    const serviceB: PropAiSyncPluginService = {
      id: "service-b",
      start: (ctx) => {
        starts.push("b");
        contexts.push(ctx);
    ...
    const serviceC: PropAiSyncPluginService = {
      id: "service-c",
      start: (ctx) => {
        starts.push("c");
        contexts.push(ctx);
