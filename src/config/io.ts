...
import type { PropAiSyncConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { comparePropAiSyncVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  PropAi Sync config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  PropAi Sync config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  ...
  pathSegments: string[],
): { changed: boolean; next: PropAiSyncConfig } {
...
function coerceConfig(value: unknown): PropAiSyncConfig {
...
function stampConfigVersion(cfg: PropAiSyncConfig): PropAiSyncConfig {
...
function warnIfConfigFromFuture(cfg: PropAiSyncConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = comparePropAiSyncVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer PropAi Sync (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as PropAiSyncConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PropAiSyncConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PropAiSyncConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PropAiSyncConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as PropAiSyncConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.propai_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.propai_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.propai_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.propai_WATCH_SESSION === "string" &&
        deps.env.propai_WATCH_SESSION.trim().length > 0
          ? deps.env.propai_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.propai_WATCH_COMMAND === "string" &&
        deps.env.propai_WATCH_COMMAND.trim().length > 0
          ? deps.env.propai_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `PROPAI_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: PropAiSyncConfig | null = null;
let runtimeConfigSourceSnapshot: PropAiSyncConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.propai_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.propai_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): PropAiSyncConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): PropAiSyncConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: PropAiSyncConfig;
  candidate: PropAiSyncConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: PropAiSyncConfig): PropAiSyncConfig {
...
export function loadConfig(): PropAiSyncConfig {
...
export async function readBestEffortConfig(): Promise<PropAiSyncConfig> {
