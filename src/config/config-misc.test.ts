...
import { buildWebSearchProviderConfig, withTempHome, writePropAiSyncConfig } from "./test-helpers.js";
import { PropAiSyncSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = PropAiSyncSchema.safeParse({
      $schema: "https://propai.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://propai.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = PropAiSyncSchema.safeParse({ $schema: 123 });
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writePropAiSyncConfig(home, {
        gateway: { bind: "${PROPAI_BIND}" },
      });

      const prev = process.env.propai_BIND;
      process.env.propai_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.propai_BIND;
        } else {
          process.env.propai_BIND = prev;
        }
      }
    });
  });
