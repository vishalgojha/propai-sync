...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("propai-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "propai-sandbox-browser-global" },
      agentBrowser: { network: "propai-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("propai-sandbox-browser-agent");
  });
