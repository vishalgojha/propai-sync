...
function isWhatsAppConfigured(cfg: PropAiSyncConfig): boolean {
...
function isGenericChannelConfigured(cfg: PropAiSyncConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: PropAiSyncConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: PropAiSyncConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: PropAiSyncConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: PropAiSyncConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: PropAiSyncConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: PropAiSyncConfig, pluginId: string): PropAiSyncConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
