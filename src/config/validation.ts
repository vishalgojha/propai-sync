...
import type { PropAiSyncConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: PropAiSyncConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: PropAiSyncConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: PropAiSyncConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = PropAiSyncSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as PropAiSyncConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as PropAiSyncConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as PropAiSyncConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as PropAiSyncConfig,
  };
}
