...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "propai-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ PROPAI_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-PropAi Sync",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-PropAi Sync#general");
  });
  ...
  it("derives session transcripts dir from PROPAI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { PROPAI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when PROPAI_STATE_DIR differs from stored paths", () => {
