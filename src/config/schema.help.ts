...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by PropAi Sync to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when PropAi Sync writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for PropAiSync. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "PropAi Sync version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version onboarding changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping PropAi Sync runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default PropAi Sync execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default PropAi Sync runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this PropAi Sync agent (for example codex, claude).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode: "PropAi Sync" (or legacy "clawd") or "extension" depending on connection/runtime strategy. Use the driver that matches your browser control stack to avoid protocol mismatches.',
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: propai-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `PropAi Sync security audit`.",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $PROPAI_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether PropAi Sync injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.propai/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `PropAi Sync plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.propai/extensions/<id>).",
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in PropAi Sync sessions.",
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, PropAi Sync starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in PropAiSync. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
  ...
  "channels.irc":
    "IRC channel provider configuration and compatibility settings for classic IRC transport workflows. Use this section when bridging legacy chat infrastructure into PropAiSync.",
  "channels.defaults":
  ...
  "channels.whatsapp.autoReply":
    "Enable WhatsApp auto-replies (default: false = read-only ingest). Enabling auto-reply can violate WhatsApp policies and risk account bans.",
  ...
  "channels.discord.eventQueue.listenerTimeout": `Canonical Discord listener timeout control in ms for gateway normalization/enqueue handlers. Default is ${DISCORD_DEFAULT_LISTENER_TIMEOUT_MS} in PropAi Sync; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.`,
