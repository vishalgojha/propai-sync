...
describe("shared/frontmatter", () => {
  ...
  test("resolvePropAiSyncManifestBlock returns undefined for invalid input", () => {
    expect(resolvePropAiSyncManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolvePropAiSyncManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolvePropAiSyncManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
