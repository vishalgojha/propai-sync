...
describe("agent components", () => {
  const createCfg = (): PropAiSyncConfig => ({}) as PropAiSyncConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): PropAiSyncConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/propai-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/PropAiSync",
    });
  });
});
