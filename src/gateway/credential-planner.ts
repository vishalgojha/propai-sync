...
type GatewaySecretDefaults = NonNullable<PropAiSyncConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${PROPAI_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(
    env,
    ["PROPAI_GATEWAY_TOKEN", "CLAWDBOT_GATEWAY_TOKEN"],
    includeLegacyEnv,
  );
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["PROPAI_GATEWAY_PASSWORD", "CLAWDBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}
