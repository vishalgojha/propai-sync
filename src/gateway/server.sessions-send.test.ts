...
const { createPropAiSyncTools } = await import("../agents/propai-tools.js");
...
type SessionSendTool = ReturnType<typeof createPropAiSyncTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createPropAiSyncTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["PROPAI_GATEWAY_PORT", "PROPAI_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.propai_GATEWAY_PORT = String(gatewayPort);
  process.env.propai_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("PROPAI_CONFIG_PATH missing in gateway test environment");
      }
