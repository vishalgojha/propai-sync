...
import { resolvePropAiSyncAgentDir } from "../agents/agent-paths.js";
...
import { ensurePropAiSyncModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, PropAiSyncConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.propai_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.propai_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.propai_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.propai_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.propai_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.propai_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.propai_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: PropAiSyncConfig;
  agentDir: string;
}): PropAiSyncConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<PropAiSyncConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.propai_CONFIG_PATH,
    token: process.env.propai_GATEWAY_TOKEN,
    skipChannels: process.env.propai_SKIP_CHANNELS,
    skipGmail: process.env.propai_SKIP_GMAIL_WATCHER,
    skipCron: process.env.propai_SKIP_CRON,
    skipCanvas: process.env.propai_SKIP_CANVAS_HOST,
    agentDir: process.env.propai_AGENT_DIR,
  ...
  process.env.propai_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.propai_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolvePropAiSyncAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-live-state-"));
  process.env.propai_STATE_DIR = tempStateDir;
  ...
  process.env.propai_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.propai-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolvePropAiSyncAgentDir();
  const sanitizedCfg: PropAiSyncConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-live-"));
  const tempConfigPath = path.join(tempDir, "propai.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.propai_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "PropAi Sync live tool probe (local, safe): " +
    ...
    process.env.propai_CONFIG_PATH = previous.configPath;
    process.env.propai_GATEWAY_TOKEN = previous.token;
    process.env.propai_SKIP_CHANNELS = previous.skipChannels;
    process.env.propai_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.propai_SKIP_CRON = previous.skipCron;
    process.env.propai_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.propai_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.propai_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.propai_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via PROPAI_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.propai-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
