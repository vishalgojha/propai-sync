...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("PropAi Sync doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with PROPAI_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with PROPAI_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.propai_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://propai-gateway.ai:18789",
      onConnectError,
    });
