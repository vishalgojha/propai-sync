    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "PropAi Sync", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-propai-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-propai-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "PropAi Sync", messages: [{ role: "user", content: "hi" }] },
          {
            "x-propai-agent-id": "beta",
            "x-propai-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from PropAiSync.");
      }
