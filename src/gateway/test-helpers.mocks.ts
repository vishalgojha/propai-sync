...
type GetReplyFromConfigFn = (
  ...
  configOverride?: PropAiSyncConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `propai-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.propai_CONFIG_PATH = path.join(root, "propai.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "propai.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "propai-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadPropAiSyncPlugins: () => pluginRegistryState.registry,
  };
});
