...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.propai_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.propai_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "PROPAI_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "PROPAI_RAW_STREAM_PATH",
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("PropAi Sync doctor")}" to repair, then retry.`,
    );
  }
  ...
  let cfgAtStart: PropAiSyncConfig;
  {
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `PropAi Sync config set gateway.auth.mode token` and `PropAi Sync config set gateway.auth.token <token>`.",
      );
    }
  }
  cfgAtStart = (
