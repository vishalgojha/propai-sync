...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "PROPAI_STATE_DIR",
  "PROPAI_CONFIG_PATH",
  "PROPAI_SKIP_BROWSER_CONTROL_SERVER",
  "PROPAI_SKIP_GMAIL_WATCHER",
  "PROPAI_SKIP_CANVAS_HOST",
  "PROPAI_BUNDLED_PLUGINS_DIR",
  "PROPAI_SKIP_CHANNELS",
  "PROPAI_SKIP_PROVIDERS",
  "PROPAI_SKIP_CRON",
  "PROPAI_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "propai-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.propai_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.propai_SKIP_GMAIL_WATCHER = "1";
  process.env.propai_SKIP_CANVAS_HOST = "1";
  process.env.propai_SKIP_CHANNELS = "1";
  process.env.propai_SKIP_PROVIDERS = "1";
  process.env.propai_SKIP_CRON = "1";
  process.env.propai_TEST_MINIMAL_GATEWAY = "1";
  process.env.propai_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "propai-test-no-bundled-extensions")
    : "propai-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".propai-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".propai-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__PropAiSyncTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__PropAiSyncTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["PROPAI_GATEWAY_TOKEN"]);
  const prev = process.env.propai_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.propai_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.propai_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
