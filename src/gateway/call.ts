...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.propai_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.propai_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set PROPAI_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: PropAi Sync doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__PROPAI_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
