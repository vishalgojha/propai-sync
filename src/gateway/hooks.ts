...
export function resolveHooksConfig(cfg: PropAiSyncConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: PropAiSyncConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-propai-token"] === "string"
      ? req.headers["x-propai-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
