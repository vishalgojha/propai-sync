...
const createTempDir = () => tempDirs.make("propai-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets PROPAI_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "PropAi Sync",
    });

    expect(env.propai_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("PropAi Sync");
  });

  it("overrides pre-existing PROPAI_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      PROPAI_SHELL: "wrong",
    });
    expect(env.propai_SHELL).toBe("acp-client");
  ...
  it("preserves PROPAI_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        PROPAI_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["PROPAI_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default PropAi Sync bridge", () => {
    ...
    expect(env.propai_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.propai_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "PropAi Sync",
        serverArgs: ["acp"],
        defaultServerCommand: "PropAi Sync",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("PROPAI_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "PropAi Sync", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "PropAi Sync", "dist", "entry.js");
    const shimPath = path.join(dir, "PropAiSync.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\\PropAiSync\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/propai-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/propai-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/propai-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
