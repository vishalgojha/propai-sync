...
export function triggerPropAiSyncRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.propai_SYSTEMD_UNIT,
      process.env.propai_PROFILE,
    );
  ...
  const label =
    process.env.propai_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.propai_PROFILE);
