...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses propai-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "propai-gateway" },
          { pid: process.pid, cmd: "propai-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'PropAi Sync'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple PropAi Sync pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "propai-gateway" },
          { pid: pid2, cmd: "propai-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-PropAi Sync pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks PropAi Sync processes — non-PropAi Sync listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "propai-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty PropAi Sync stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty PropAi Sync stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + PropAi Sync pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "propai-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'PropAi Sync'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("PropAi Sync")
      // inside the mid-loop flush: a non-PropAi Sync cmd between two entries must not
      // be pushed, but the following PropAi Sync entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-PropAi Sync entry first, then PropAi Sync entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid PropAi Sync entry must still be found.
      const stdout = `p0\ncpropai-gateway\np${stalePid}\ncpropai-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero PropAi Sync pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-PropAi Sync stdout (line 145)", () => {
    it("treats status 1 + non-PropAi Sync stdout as port-free (not an PropAi Sync process)", () => {
      // status 1 + non-empty stdout where no PropAi Sync pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill PropAi Sync pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-PropAi Sync output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no PropAi Sync pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
