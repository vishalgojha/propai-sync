...
import { POSIX_PROPAI_TMP_DIR, resolvePreferredPropAiSyncTmpDir } from "./tmp-propai-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredPropAiSyncTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `propai-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredPropAiSyncTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_PROPAI_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredPropAiSyncTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredPropAiSyncTmpDir", () => {
  it("prefers /tmp/PropAiSync when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_PROPAI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/PropAiSync when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_PROPAI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/PropAiSync when /tmp/PropAiSync is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/PropAiSync when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/PropAiSync is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/PropAiSync is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/PropAiSync is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback PropAi Sync temp dir/);
  });
