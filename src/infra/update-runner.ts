...
const DEFAULT_PACKAGE_NAME = "PropAi Sync";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["PropAi Sync", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "propai-update-preflight-"));
