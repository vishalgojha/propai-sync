...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.propai_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.propai_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set PROPAI_APNS_TEAM_ID and PROPAI_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.propai_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.propai_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.propai_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set PROPAI_APNS_PRIVATE_KEY_P8 or PROPAI_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading PROPAI_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createPropAiSyncPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    "PropAi Sync": createPropAiSyncPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
