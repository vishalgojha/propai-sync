...
function candidateBinDirs(opts: EnsurePropAiSyncPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `PropAi Sync` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "PropAi Sync");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.propai_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "PropAi Sync"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `PropAi Sync` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensurePropAiSyncCliOnPath(opts: EnsurePropAiSyncPathOpts = {}) {
  if (isTruthyEnvValue(process.env.propai_PATH_BOOTSTRAPPED)) {
    return;
  }
