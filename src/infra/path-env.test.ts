...
let ensurePropAiSyncCliOnPath: typeof import("./path-env.js").ensurePropAiSyncCliOnPath;

describe("ensurePropAiSyncCliOnPath", () => {
  const envKeys = [
    "PATH",
    "PROPAI_PATH_BOOTSTRAPPED",
    "PROPAI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensurePropAiSyncCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling PropAi Sync exists", () => {
    const tmp = abs("/tmp/propai-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "PropAi Sync");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.propai_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.propai_PATH_BOOTSTRAPPED = "1";
    ensurePropAiSyncCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/propai-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "PropAi Sync");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/propai-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "PropAi Sync");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "PropAi Sync");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/propai-path/case-linuxbrew");
