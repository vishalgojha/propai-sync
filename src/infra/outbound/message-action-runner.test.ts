...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred PropAi Sync tmp root", async () => {
    const tmpRoot = resolvePreferredPropAiSyncTmpDir();
    ...
    try {
      ...
      const hostTmpOutsidePropAiSync = path.join(os.tmpdir(), "outside-PropAi Sync", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
          params: {
            channel: "slack",
            target: "#C12345678",
            media: hostTmpOutsidePropAiSync,
            message: "",
          },
          sandboxRoot: sandboxDir,
          dryRun: true,
