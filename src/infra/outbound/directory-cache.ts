...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: PropAiSyncConfig | null = null;
  ...
  get(key: string, cfg: PropAiSyncConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: PropAiSyncConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: PropAiSyncConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: PropAiSyncConfig): void {
