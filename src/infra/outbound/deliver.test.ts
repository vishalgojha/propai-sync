...
import { resolvePreferredPropAiSyncTmpDir } from "../tmp-propai-dir.js";
...
const telegramChunkConfig: PropAiSyncConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: PropAiSyncConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes PropAi Sync tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredPropAiSyncTmpDir()]),
      }),
    );
  });

  it("includes PropAi Sync tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes PropAi Sync tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes PropAi Sync tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: PropAiSyncConfig = { channels: { signal: { mediaMaxMb: 2 } } };
