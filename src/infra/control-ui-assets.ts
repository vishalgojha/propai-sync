...
import { resolvePropAiSyncPackageRoot, resolvePropAiSyncPackageRootSync } from "./propai-root.js";
...
function hasControlUiWorkspace(root: string): boolean {
  return (
    fs.existsSync(path.join(root, "ui", "vite.config.ts")) ||
    fs.existsSync(path.join(root, "apps", "website", "vite.config.ts"))
  );
}
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolvePropAiSyncPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "PropAi Sync" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/PropAiSync -> .../dist/index.js)
