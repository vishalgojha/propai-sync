...
describe("ports helpers", () => {
  ...
  it("prints an propai-specific hint when port details look like another PropAi Sync instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another PropAi Sync instance is already running");
  });
