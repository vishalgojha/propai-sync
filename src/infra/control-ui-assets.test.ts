...
vi.mock("./propai-root.js", () => ({
  resolvePropAiSyncPackageRoot: vi.fn(async () => null),
  resolvePropAiSyncPackageRootSync: vi.fn(() => null),
}));
...
let PropAiSyncRoot: typeof import("./propai-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    PropAiSyncRoot = await import("./propai-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/PropAiSync");
    const wrapperArgv1 = abs("fixtures/bin/PropAiSync");
  ...
  it("uses resolvePropAiSyncPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/PropAiSync");
    (
      PropAiSyncRoot.resolvePropAiSyncPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/PropAiSync"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-PropAi Sync");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/propai-bundle");
    (
      PropAiSyncRoot.resolvePropAiSyncPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/propai-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
