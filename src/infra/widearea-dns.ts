...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.propai_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*propai-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "PropAi Sync";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "PropAi Sync");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "propai-gw");
  ...
  records.push(`_propai-gw._tcp IN PTR ${instanceLabel}._propai-gw._tcp`);
  records.push(`${instanceLabel}._propai-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._propai-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; propai-content-hash: ${contentHash}\n${contentBody}`;
}
