...
describe("resolveEffectiveHomeDir", () => {
  it("prefers PROPAI_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands PROPAI_HOME when set to ~", () => {
    const env = {
      PROPAI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for PROPAI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { PROPAI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when PROPAI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ PROPAI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { PROPAI_HOME: "/srv/propai-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/propai-home")}/x`);
  });
