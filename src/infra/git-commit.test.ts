...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the PropAi Sync package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "PropAi Sync");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "PropAi Sync", version: "2026.3.10" }),
      "utf-8",
    );
