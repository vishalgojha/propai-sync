...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__PROPAI_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "propai-root");
...
describe("resolvePropAiSyncPackageRoot", () => {
  let resolvePropAiSyncPackageRoot: typeof import("./propai-root.js").resolvePropAiSyncPackageRoot;
  let resolvePropAiSyncPackageRootSync: typeof import("./propai-root.js").resolvePropAiSyncPackageRootSync;

  beforeAll(async () => {
    ({ resolvePropAiSyncPackageRoot, resolvePropAiSyncPackageRootSync } =
      await import("./propai-root.js"));
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    await expect(
      resolvePropAiSyncPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
  ...
  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolvePropAiSyncPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
