 ...
 * returning control to the caller (typically `triggerPropAiSyncRestart` →
...
/**
 * Parse PropAi Sync gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("PropAi Sync")) {
 ...
 * Returns only PIDs that belong to PropAi Sync gateway processes (not the current process).
 */
