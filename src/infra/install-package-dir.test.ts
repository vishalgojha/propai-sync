...
describe("installPackageDir", () => {
  ...
  it("keeps the existing install in place when staged validation fails", async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "propai-install-package-dir-"));
    ...
    await expect(
      listMatchingDirs(installBaseDir, ".propai-install-stage-"),
    ).resolves.toHaveLength(0);
    await expect(
      listMatchingDirs(installBaseDir, ".propai-install-backups"),
    ).resolves.toHaveLength(0);
  });
  ...
  it("restores the original install if publish rename fails", async () => {
    ...
    const backupRoot = path.join(installBaseDir, ".propai-install-backups");
  ...
  it("warns and leaves the backup in place when the install base changes before backup cleanup", async () => {
    ...
    const backupRoot = path.join(preservedInstallRoot, ".propai-install-backups");
