...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: PropAiSyncConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: PropAiSyncConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: PropAiSyncConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: PropAiSyncConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: PropAiSyncConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: PropAiSyncConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: PropAiSyncConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
