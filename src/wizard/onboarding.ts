...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "PropAi Sync is a hobby project and still in beta. Expect sharp edges.",
      "By default, PropAi Sync is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "PropAi Sync is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run PropAiSync.",
      ...
      "",
      "Run regularly:",
      "PropAi Sync security audit --deep",
      "PropAi Sync security audit --fix",
      "",
      "Must read: https://docs.propai.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.propai.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
  ...
  const quickstartHint = `Configure details later via ${formatCliCommand("PropAi Sync configure")}.`;
  ...
  let localGatewayToken = process.env.propai_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword =
    process.env.propai_GATEWAY_PASSWORD ?? process.env.CLAWDBOT_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: PropAiSyncConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
