...
export function buildConfigureCandidates(config: PropAiSyncConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredPropAiSyncConfig?: PropAiSyncConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredPropAiSyncConfig ?? params.config;
  ...
  return [...propaiCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "PropAi Sync secrets configure",
