...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createPropAiSyncTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-test-"));

    try {
      await withEnvAsync(
        { PROPAI_STATE_DIR: stateDir, PROPAI_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: PropAi Sync --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/PropAiSync" } }\n}\n';
