...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["PROPAI_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-subagent-"));
  ...
  it("uses isolated temp state when PROPAI_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "propai-test-state"));
  });
});
