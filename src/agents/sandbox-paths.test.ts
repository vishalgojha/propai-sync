...
async function withOutsideHardlinkInPropAiSyncTmp<T>(
  ...
  const hardlinkPath = path.join(params.propaiTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.propaiTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.propaiTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.propaiTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const PropAiSyncTmpDir = resolvePreferredPropAiSyncTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred PropAi Sync tmp root",
      media: path.join(PropAiSyncTmpDir, "image.png"),
      expected: path.join(PropAiSyncTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred PropAi Sync tmp root",
      media: pathToFileURL(path.join(PropAiSyncTmpDir, "photo.png")).href,
      expected: path.join(PropAiSyncTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred PropAi Sync tmp root",
      media: path.join(PropAiSyncTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(PropAiSyncTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(PropAiSyncTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside PropAi Sync tmp root",
      media: path.join(os.tmpdir(), "outside-PropAi Sync", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked PropAi Sync tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(PropAiSyncTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(PropAiSyncTmpDir, { recursive: true });
      const symlinkPath = path.join(PropAiSyncTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked PropAi Sync tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked PropAi Sync tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInPropAiSyncTmp(
