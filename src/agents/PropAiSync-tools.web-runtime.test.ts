...
function findTool(name: string, config: PropAiSyncConfig) {
  const allTools = createPropAiSyncTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: PropAiSyncConfig; env?: NodeJS.ProcessEnv }) {
...
describe("PropAi Sync tools runtime web metadata wiring", () => {
