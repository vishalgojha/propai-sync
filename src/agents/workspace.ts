...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.propai_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".propai", `workspace-${profile}`);
  }
  return path.join(home, ".propai", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".propai";
