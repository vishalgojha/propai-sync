...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const PropAiSyncTmpDir = path.resolve(resolvePreferredPropAiSyncTmpDir());
  if (!isPathInside(PropAiSyncTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: PropAiSyncTmpDir });
  return resolved;
}
