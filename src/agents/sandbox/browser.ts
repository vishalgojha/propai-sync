...
import {
  ...
  DEFAULT_PROPAI_BROWSER_COLOR,
  DEFAULT_PROPAI_BROWSER_PROFILE_NAME,
...
const CDP_SOURCE_RANGE_ENV_KEY = "PROPAI_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_PROPAI_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_PROPAI_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_PROPAI_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "PropAiSync.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `PropAi Sync sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `PropAi Sync sandbox recreate --browser --agent ${agentId}`;
          }
          return "PropAi Sync sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        ...
        "PropAiSync.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `PROPAI_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `PROPAI_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `PROPAI_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `PROPAI_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `PROPAI_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    args.push("-e", "PROPAI_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_PROPAI_BROWSER_PROFILE_NAME)
    : null;
