...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "PropAiSync.sandbox=1");
  args.push("--label", `PropAiSync.sessionKey=${params.scopeKey}`);
  args.push("--label", `PropAiSync.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `PropAiSync.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markPropAiSyncExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "PropAiSync.configHash");
}
