...
import { resolvePropAiSyncMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
describe("resolvePropAiSyncMetadata install validation", () => {
  function resolveInstall(frontmatter: Record<string, string>) {
    return resolvePropAiSyncMetadata(frontmatter)?.install;
  }

  it("accepts safe install specs", () => {
    const install = resolveInstall({
      metadata:
        '{"PropAi Sync":{"install":[{"kind":"brew","formula":"python@3.12"},{"kind":"node","package":"@scope/pkg@1.2.3"},{"kind":"go","module":"example.com/tool/cmd@v1.2.3"},{"kind":"uv","package":"uvicorn[standard]==0.31.0"},{"kind":"download","url":"https://example.com/tool.tar.gz"}]}}',
    });
  ...
  it("drops unsafe brew formula values", () => {
    const install = resolveInstall({
      metadata: '{"PropAi Sync":{"install":[{"kind":"brew","formula":"wget --HEAD"}]}}',
    });
  ...
  it("drops unsafe npm package specs for node installers", () => {
    const install = resolveInstall({
      metadata: '{"PropAi Sync":{"install":[{"kind":"node","package":"file:../malicious"}]}}',
    });
    expect(install).toBeUndefined();
  });
  ...
  it("drops unsafe go module specs", () => {
    const install = resolveInstall({
      metadata: '{"PropAi Sync":{"install":[{"kind":"go","module":"https://evil.example/mod"}]}}',
    });
    expect(install).toBeUndefined();
  });
  ...
  it("drops unsafe download urls", () => {
    const install = resolveInstall({
      metadata: '{"PropAi Sync":{"install":[{"kind":"download","url":"file:///tmp/payload.tgz"}]}}',
    });
    expect(install).toBeUndefined();
  });
});
