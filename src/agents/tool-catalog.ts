...
type CoreToolDefinition = {
  ...
  includeInPropAiSyncGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const PropAiSyncTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInPropAiSyncGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:PropAi Sync": PropAiSyncTools,
