...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: PropAiSyncConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<PropAiSyncConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<PropAiSyncConfig["agents"]>["list"]>[number]["tools"];
  }): PropAiSyncConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createPropAiSyncCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createPropAiSyncCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createPropAiSyncCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createPropAiSyncCodingTools({
      config: cfg,
    ...
    const bobTools = createPropAiSyncCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createPropAiSyncCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createPropAiSyncCodingTools({
      config: cfg,
