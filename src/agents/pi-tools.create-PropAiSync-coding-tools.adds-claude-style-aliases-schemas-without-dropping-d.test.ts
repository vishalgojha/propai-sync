...
describe("createPropAiSyncCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-read-"));
    try {
      ...
      const contents = "Hello from PropAi Sync read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createPropAiSyncCodingTools({ sandbox });
