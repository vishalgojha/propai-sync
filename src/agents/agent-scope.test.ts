...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: PropAiSyncConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/PropAiSync" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: PropAiSyncConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: PropAiSyncConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: PropAiSyncConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: PropAiSyncConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: PropAiSyncConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: PropAiSyncConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: PropAiSyncConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/PropAiSync");
  });

  it("uses PROPAI_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "propai-home");
    vi.stubEnv("PROPAI_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as PropAiSyncConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".propai", "workspace"));
  });

  it("uses PROPAI_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "propai-home");
    vi.stubEnv("PROPAI_HOME", home);
    // Clear state dir so it falls back to PROPAI_HOME
    vi.stubEnv("PROPAI_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as PropAiSyncConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".propai", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/propai-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/propai-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "propai-agent-scope-"));
