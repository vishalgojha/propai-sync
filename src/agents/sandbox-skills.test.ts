...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "propai-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.propai_BUNDLED_SKILLS_DIR = bundledDir;
