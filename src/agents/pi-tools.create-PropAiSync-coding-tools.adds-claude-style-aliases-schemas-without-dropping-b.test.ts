...
const defaultTools = createPropAiSyncCodingTools({ senderIsOwner: true });
...
describe("createPropAiSyncCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: PropAiSyncConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createPropAiSyncCodingTools({
    ...
    const anthropicTools = createPropAiSyncCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: PropAiSyncConfig = {
      tools: {
        exec: {
    ...
    const allowed = createPropAiSyncCodingTools({
    ...
    const denied = createPropAiSyncCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createPropAiSyncCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
