...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; PropAi Sync handles all routing internally.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running PropAi Sync process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List PropAi Sync agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List PropAi Sync agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside PropAiSync.";
  }

  const lines = [
    "You are a personal assistant running inside PropAiSync.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control PropAi Sync's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## PropAi Sync CLI Quick Reference",
    "PropAi Sync is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- PropAi Sync gateway status",
    "- PropAi Sync gateway start",
    "- PropAi Sync gateway stop",
    "- PropAi Sync gateway restart",
    "If unsure, ask the user to run `PropAi Sync help` (or `PropAi Sync gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## PropAi Sync Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, PropAi Sync pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by PropAi Sync and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'PropAi Sync treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
