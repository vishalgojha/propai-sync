...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses PROPAI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      PROPAI_HOME: "/srv/propai-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/propai-home"), ".propai", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".propai", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("propai-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-workspace-hardlink-"));
    try {
