...
const GOOGLE_LIVE_SINGLE_KEY = "PROPAI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "PROPAI_LIVE_ANTHROPIC_KEY",
    listVar: "PROPAI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "PROPAI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `PROPAI_LIVE_${base}_KEY`;
