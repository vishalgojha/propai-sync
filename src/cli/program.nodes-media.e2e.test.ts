...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("propai-camera-snap-");
    expect(mediaPaths[1]).toContain("propai-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /propai-camera-clip-front-.*\.mp4$/,
    });
  });
  ...
  it("runs nodes canvas snapshot and prints MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /propai-canvas-snapshot-.*\.png$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /propai-camera-snap-front-.*\.jpg$/,
      },
      {
