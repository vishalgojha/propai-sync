    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "propai node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["propai node status", "Check node host service status."],
          ["propai node install", "Install the node host service."],
          ["propai node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.propai.ai/cli/node")}\n`,
    );
