...
${formatHelpExamples([
  ['propai agent --to +15555550123 --message "status update"', "Start a new session."],
  ['propai agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'propai agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'propai agent --to +15555550123 --message "Trace logs" --verbose on --json',
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.propai.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.propai.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['propai agents set-identity --agent main --name "PropAi Sync" --emoji "🦞"', "Set name + emoji."],
  ["propai agents set-identity --agent main --avatar avatars/PropAiSync.png", "Set avatar path."],
  [
    "propai agents set-identity --workspace ~/.propai/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "propai agents set-identity --identity-file ~/.propai/workspace/IDENTITY.md --agent main",
