...
export function registerMessagePollCommand(message: Command, helpers: MessageCliHelpers) {
  helpers
    ...
    .option("--poll-duration-hours <n>", "Poll duration in hours (when supported)")
    ...
    .option("--silent", "Send poll silently without notification (Telegram where supported)", false)
