...
describe("program routes", () => {
  ...
  it("matches health route and preloads plugins only for text output", () => {
    ...
    expect(shouldLoad(["node", "PropAi Sync", "health"])).toBe(true);
    expect(shouldLoad(["node", "PropAi Sync", "health", "--json"])).toBe(false);
  });

  it("returns false when status timeout flag value is missing", async () => {
    await expectRunFalse(["status"], ["node", "PropAi Sync", "status", "--timeout"]);
  });

  it("returns false for sessions route when --store value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "PropAi Sync", "sessions", "--store"]);
  });

  it("returns false for sessions route when --active value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "PropAi Sync", "sessions", "--active"]);
  });

  it("returns false for sessions route when --agent value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "PropAi Sync", "sessions", "--agent"]);
  });
  ...
  it("returns false for config get route when path argument is missing", async () => {
    await expectRunFalse(["config", "get"], ["node", "PropAi Sync", "config", "get", "--json"]);
  });

  it("returns false for config unset route when path argument is missing", async () => {
    await expectRunFalse(["config", "unset"], ["node", "PropAi Sync", "config", "unset"]);
  });
  ...
  it("passes config unset path correctly when root option values precede command", async () => {
    ...
    await expect(
      route?.run(["node", "PropAi Sync", "--profile", "work", "config", "unset", "update.channel"]),
    ).resolves.toBe(true);
  ...
  it("passes config unset path when root value options appear after subcommand", async () => {
    const route = expectRoute(["config", "unset"]);
    await expect(
      route?.run(["node", "PropAi Sync", "config", "unset", "--profile", "work", "update.channel"]),
    ).resolves.toBe(true);
    expect(runConfigUnsetMock).toHaveBeenCalledWith({ path: "update.channel" });
  });
  ...
  it("returns false for config get route when unknown option appears", async () => {
    await expectRunFalse(
      ["config", "get"],
      ["node", "PropAi Sync", "config", "get", "--mystery", "value", "update.channel"],
    );
  });

  it("returns false for memory status route when --agent value is missing", async () => {
    await expectRunFalse(["memory", "status"], ["node", "PropAi Sync", "memory", "status", "--agent"]);
  });

  it("returns false for models list route when --provider value is missing", async () => {
    await expectRunFalse(["models", "list"], ["node", "PropAi Sync", "models", "list", "--provider"]);
  });

  it("returns false for models status route when probe flags are missing values", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "PropAi Sync", "models", "status", "--probe-provider"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "PropAi Sync", "models", "status", "--probe-timeout"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "PropAi Sync", "models", "status", "--probe-concurrency"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "PropAi Sync", "models", "status", "--probe-max-tokens"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "PropAi Sync", "models", "status", "--probe-provider", "openai", "--agent"],
    );
  });
  ...
  it("returns false for models status route when --probe-profile has no value", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "PropAi Sync", "models", "status", "--probe-profile"],
    );
  });
