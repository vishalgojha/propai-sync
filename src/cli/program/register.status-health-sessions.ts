...
export function registerStatusHealthSessionsCommands(program: Command) {
  program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propai status", "Show channel health + session summary."],
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.propai.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.propai.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propai sessions", "List all sessions."],
          ["propai sessions --agent work", "List sessions for one agent."],
          ["propai sessions --all-agents", "Aggregate sessions across agents."],
          ["propai sessions --active 120", "Only last 2 hours."],
          ["propai sessions --json", "Machine-readable output."],
          ["propai sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.propai.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propai sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "propai sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["propai sessions cleanup --enforce", "Apply maintenance now."],
          ["propai sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["propai sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "propai sessions cleanup --enforce --store ./tmp/sessions.json",
