...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@propai/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@propai/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@propai/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@propai/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @propai/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@propai/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@propai/plugin-alpha@1.2.3",
        installPath: "/tmp/propai/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@propai/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @propai/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@propai/plugin-alpha@latest",
      true,
      "/tmp/propai/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@propai/plugin-alpha@latest",
      installPath: "/tmp/propai/extensions/alpha",
      version: "1.2.3",
