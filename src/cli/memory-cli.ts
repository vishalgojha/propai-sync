    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propai memory status", "Show index and provider status."],
          ["propai memory status --deep", "Probe embedding provider readiness."],
          ["propai memory index --force", "Force a full reindex."],
          ['propai memory search "meeting notes"', "Quick search using positional query."],
          [
            'propai memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["propai memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.propai.ai/cli/memory")}\n`,
    );
