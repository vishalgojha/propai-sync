...
export function renderRuntimeHints(
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.propai_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.propai_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("propai gateway install", env),
    startCommand: formatCliCommand("propai gateway", env),
