...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("propai-daemon-install-int-");
    configPath = path.join(tempHome, "propai.json");
    process.env.HOME = tempHome;
    process.env.propai_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.propai_GATEWAY_TOKEN).toBeUndefined();
  });
});
