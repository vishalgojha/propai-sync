...
import type { PropAiSyncConfig, ConfigFileSnapshot } from "../config/types.propai.js";
...
vi.mock("../infra/propai-root.js", () => ({
  resolvePropAiSyncPackageRoot: vi.fn(),
}));
...
const { resolvePropAiSyncPackageRoot } = await import("../infra/propai-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/propai-update-tests";
  let fixtureCount = 0;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolvePropAiSyncPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("propai-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolvePropAiSyncPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("PropAi Sync");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/propai-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "propai-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("propai update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as PropAiSyncConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolvePropAiSyncPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("propai-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ PROPAI_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.propai_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("propai-update-wizard");
    await withEnvAsync({ PROPAI_GIT_DIR: tempDir }, async () => {
      setTty(true);
