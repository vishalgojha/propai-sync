...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update propai and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["propai update", "Update a source checkout (git)"],
        ["propai update --channel beta", "Switch to beta channel (git + npm)"],
        ["propai update --channel dev", "Switch to dev channel (git + npm)"],
        ["propai update --tag beta", "One-off update to a dist-tag or version"],
        ["propai update --dry-run", "Preview actions without changing anything"],
        ["propai update --no-restart", "Update without restarting the service"],
        ["propai update --json", "Output result as JSON"],
        ["propai update --yes", "Non-interactive (accept downgrade prompts)"],
        ["propai update wizard", "Interactive update wizard"],
        ["propai --update", "Shorthand for propai update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run propai update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.propai.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propai update status", "Show channel + version status."],
          ["propai update status --json", "JSON output."],
          ["propai update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.propai.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
