...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set PROPAI_GATEWAY_TOKEN/PROPAI_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.propai_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.propai_GATEWAY_PASSWORD);
