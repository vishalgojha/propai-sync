...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      PROPAI_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when PROPAI_THEME=light", async () => {
    const mod = await importThemeWithEnv({ PROPAI_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when PROPAI_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ PROPAI_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats PROPAI_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ PROPAI_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("PROPAI_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      PROPAI_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ PROPAI_THEME: "light" });
    const darkMod = await importThemeWithEnv({ PROPAI_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.propai_THEME = "light";
