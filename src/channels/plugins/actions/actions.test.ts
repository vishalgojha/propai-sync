...
function telegramCfg(): PropAiSyncConfig {
  return { channels: { telegram: { botToken: "tok" } } } as PropAiSyncConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: PropAiSyncConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as PropAiSyncConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as PropAiSyncConfig;
...
function createSignalAccountOverrideCfg(): PropAiSyncConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as PropAiSyncConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as PropAiSyncConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as PropAiSyncConfig,
        accountId: undefined,
        params: {
