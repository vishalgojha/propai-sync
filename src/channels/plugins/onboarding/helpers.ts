...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: PropAiSyncConfig) => Array<string | number> | undefined;
}): PropAiSyncConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: PropAiSyncConfig) => PropAiSyncConfig | Promise<PropAiSyncConfig>;
  applySet?: (
    cfg: PropAiSyncConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => PropAiSyncConfig | Promise<PropAiSyncConfig>;
}): Promise<{
  cfg: PropAiSyncConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in PropAi Sync config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "PROPAI_SECRET",
      envVarFormatError:
        'Use an env var name like "PROPAI_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
