...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "PropAi Sync";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for PropAi Sync`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/PropAiSync",
          description: "Send a message to PropAi Sync",
          should_escape: false,
        },
      ],
    },
