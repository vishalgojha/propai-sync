...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        PROPAI_BASE_ENV: "base",
        PROPAI_TO_REMOVE: undefined,
      },
      env: {
        PROPAI_TEST_ENV: "ok",
      },
    ...
    expect(resolved.propai_BASE_ENV).toBe("base");
    expect(resolved.propai_TEST_ENV).toBe("ok");
    expect(resolved.propai_TO_REMOVE).toBeUndefined();
    expect(resolved.propai_CLI).toBe(PROPAI_CLI_ENV_VALUE);
  });
