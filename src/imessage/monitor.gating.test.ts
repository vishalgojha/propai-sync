...
function baseCfg(): PropAiSyncConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
    ...
    messages: {
      groupChat: { mentionPatterns: ["@PropAi Sync"] },
    },
...
function buildDispatchContextPayload(params: { cfg: PropAiSyncConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      message: {
        ...
        sender: "+15550001111",
        is_from_me: false,
        text: "@PropAi Sync hello",
        is_group: true,
      },
      opts: {},
      messageText: "@PropAi Sync hello",
      bodyText: "@PropAi Sync hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
  ...
  it("honors group allowlist and ignores pairing-store senders in groups", () => {
    ...
    const denied = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@PropAi Sync hi",
      bodyText: "@PropAi Sync hi",
      allowFrom: ["*"],
    ...
    const allowed = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@PropAi Sync ok",
      bodyText: "@PropAi Sync ok",
      allowFrom: ["*"],
      groupAllowFrom: ["chat_id:101"],
      groupPolicy: "allowlist",
      dmPolicy: "pairing",
