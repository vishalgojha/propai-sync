  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          "PropAi Sync": { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.propai_GATEWAY_PORT;
    process.env.propai_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.propai_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.propai_GATEWAY_PASSWORD;
