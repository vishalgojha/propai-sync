...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredPropAiSyncTmpDir: vi.fn(() => "/tmp/PropAiSync"),
}));
vi.mock("../infra/tmp-propai-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredPropAiSyncTmpDir.mockReturnValue("/tmp/PropAiSync");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".propai-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredPropAiSyncTmpDir.mockReturnValue("/tmp/propai-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "propai-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "propai-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredPropAiSyncTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "propai-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "propai-preferred", "downloads")}${path.sep}`),
    );
  });
