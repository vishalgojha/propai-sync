...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.propai_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/PropAiSync" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          "PropAi Sync": { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchPropAiSyncChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.propai_GATEWAY_PORT;
  process.env.propai_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.propai_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.propai_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayPassword === undefined) {
    ...
    process.env.propai_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
