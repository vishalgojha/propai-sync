...
vi.mock("./chrome.js", () => ({
  resolvePropAiSyncUserDataDir: vi.fn(() => "/tmp/propai-test/propai/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolvePropAiSyncUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "propai-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolvePropAiSyncUserDataDir).mockReturnValue(userDataDir);
