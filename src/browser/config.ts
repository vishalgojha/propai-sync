import type { BrowserConfig, BrowserProfileConfig, PropAiSyncConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_PROPAI_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_PROPAI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "PropAi Sync" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_PROPAI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_PROPAI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an PropAi Sync browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for propai/propai CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: PropAiSyncConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_PROPAI_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_PROPAI_BROWSER_PROFILE_NAME]
        ? DEFAULT_PROPAI_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "PropAi Sync";
