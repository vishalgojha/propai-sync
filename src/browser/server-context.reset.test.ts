...
function localPropAiSyncProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "PropAi Sync",
...
function createLocalPropAiSyncResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localPropAiSyncProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolvePropAiSyncUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "propai-reset-"));
    const profileDir = path.join(tempRoot, "PropAi Sync");
    ...
    const ops = createLocalPropAiSyncResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolvePropAiSyncUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "propai-reset-no-own-"));
    const profileDir = path.join(tempRoot, "PropAi Sync");
    fs.mkdirSync(profileDir, { recursive: true });
