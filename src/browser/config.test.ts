...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("PropAi Sync");
    expect(profile?.driver).toBe("PropAi Sync");
  ...
  it("derives default ports from PROPAI_GATEWAY_PORT when unset", () => {
    withEnv({ PROPAI_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ PROPAI_GATEWAY_PORT: undefined }, () => {
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        "PropAi Sync": { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("PropAi Sync");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to PropAi Sync profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps PropAi Sync default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("PropAi Sync");
    ...
    it("keeps PropAi Sync default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("PropAi Sync");
    ...
    it("keeps PropAi Sync default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
