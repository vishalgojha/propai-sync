...
export function resolvePropAiSyncUserDataDir(profileName = DEFAULT_PROPAI_BROWSER_PROFILE_NAME) {
...
export async function launchPropAiSyncChrome(
  ...
  const userDataDir = resolvePropAiSyncUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_PROPAI_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decoratePropAiSyncProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 PropAi Sync browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`PropAi Sync browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`PropAi Sync browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 PropAi Sync browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopPropAiSyncChrome(
