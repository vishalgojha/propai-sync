...
describe("extension-relay-auth SecretRef handling", () => {
  const ENV_KEYS = ["PROPAI_GATEWAY_TOKEN", "CLAWDBOT_GATEWAY_TOKEN", "CUSTOM_GATEWAY_TOKEN"];
  ...
  it("resolves file-backed gateway.auth.token SecretRef", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "propai-relay-file-secret-"));
