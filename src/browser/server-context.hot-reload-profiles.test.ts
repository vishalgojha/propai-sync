...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only PropAi Sync profile
    ...
    // 2. Simulate adding a new profile to config (like user editing propai.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.propai = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.propai?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", async () => {
    const cfg = loadConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const PropAiSyncProfile = resolveProfile(resolved, "PropAi Sync");
    expect(PropAiSyncProfile).toBeTruthy();
    ...
    const runtime = state.profiles.get("PropAi Sync");
