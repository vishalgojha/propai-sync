...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".propai-profile-decorated");
}
 ...
 */
export function decoratePropAiSyncProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_PROPAI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_PROPAI_BROWSER_COLOR).toUpperCase();
