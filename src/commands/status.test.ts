...
beforeAll(() => {
  envSnapshot = captureEnv(["PROPAI_PROFILE"]);
  process.env.propai_PROFILE = "isolated";
});
...
vi.mock("../infra/propai-root.js", () => ({
  resolvePropAiSyncPackageRoot: vi.fn().mockResolvedValue("/tmp/PropAiSync"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/propai/pnpm-lock.yaml",
      markerPath: "/tmp/propai/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.propai.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.propai.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("PropAi Sync status --all") ||
          line.includes("PropAi Sync --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("PROPAI_GATEWAY_TOKEN", "abcd1234", async () => {
