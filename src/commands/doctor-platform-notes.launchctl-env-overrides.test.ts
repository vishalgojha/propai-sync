...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "PROPAI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("PROPAI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv PROPAI_GATEWAY_TOKEN");
    expect(message).not.toContain("PROPAI_GATEWAY_PASSWORD");
  });
  ...
  it("treats SecretRef-backed credentials as configured", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async (name: string) =>
      name === "PROPAI_GATEWAY_PASSWORD" ? "launchctl-password" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
          password: { source: "env", provider: "default", id: "PROPAI_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        providers: {
    ...
    expect(message).toContain("PROPAI_GATEWAY_PASSWORD");
  });
