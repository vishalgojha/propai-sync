...
export function collectMissingDefaultAccountBindingWarnings(cfg: PropAiSyncConfig): string[] {
...
export function collectMissingExplicitDefaultAccountWarnings(cfg: PropAiSyncConfig): string[] {
  const warnings: string[] = [];
...
function scanTelegramAllowFromUsernameEntries(cfg: PropAiSyncConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: PropAiSyncConfig): Promise<{
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: PropAiSyncConfig): {
  config: PropAiSyncConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: PropAiSyncConfig): Promise<{
  config: PropAiSyncConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: PropAiSyncConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
...
function collectExecSafeBinScopes(cfg: PropAiSyncConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: PropAiSyncConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: PropAiSyncConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: PropAiSyncConfig): {
  config: PropAiSyncConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: PropAiSyncConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: PropAiSyncConfig): {
  config: PropAiSyncConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: PropAiSyncConfig = baseCfg;
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "PropAi Sync doctor --fix" to remove these keys.');
    }
  }
