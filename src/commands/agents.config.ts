...
function resolveAgentName(cfg: PropAiSyncConfig, agentId: string) {
...
function resolveAgentModel(cfg: PropAiSyncConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PropAiSyncConfig): AgentSummary[] {
