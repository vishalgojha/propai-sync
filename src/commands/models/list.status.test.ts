...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePropAiSyncAgentDir: vi.fn().mockReturnValue("/tmp/propai-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/propai-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/propai-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePropAiSyncAgentDir: mocks.resolvePropAiSyncAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/propai-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolvePropAiSyncAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/propai-dev/propai.json");
    expect(payload.auth.storePath).toBe("/tmp/propai-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/propai-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/propai-agent-custom");
