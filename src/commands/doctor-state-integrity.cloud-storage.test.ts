...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "PropAi Sync", ".propai");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/propai-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".propai");
  ...
  it("anchors cloud detection to OS homedir when PROPAI_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".propai");
    const originalPropAiSyncHome = process.env.propai_HOME;
    process.env.propai_HOME = "/tmp/propai-home-override";
    ...
    try {
      ...
      if (originalPropAiSyncHome === undefined) {
        delete process.env.propai_HOME;
      } else {
        process.env.propai_HOME = originalPropAiSyncHome;
      }
    }
  });
