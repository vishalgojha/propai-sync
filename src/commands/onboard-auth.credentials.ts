...
const resolveAuthAgentDir = (agentDir?: string) => agentDir ?? resolvePropAiSyncAgentDir();
...
export async function setElevenLabsApiKey(
  key: SecretInput,
  agentDir?: string,
  options?: ApiKeyStorageOptions,
) {
  ...
  upsertAuthProfile({
    ...
    credential: buildApiKeyCredential("elevenlabs", safeKey, undefined, options),
    agentDir: resolveAuthAgentDir(agentDir),
  });
}
...
export async function setGroqApiKey(
  ...
  upsertAuthProfile({
    ...
    credential: buildApiKeyCredential("groq", safeKey, undefined, options),
    agentDir: resolveAuthAgentDir(agentDir),
  });
}
