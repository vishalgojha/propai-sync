...
type EnvSnapshot = {
  HOME?: string;
  PROPAI_HOME?: string;
  PROPAI_STATE_DIR?: string;
  PROPAI_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    PROPAI_HOME: process.env.propai_HOME,
    PROPAI_STATE_DIR: process.env.propai_STATE_DIR,
    PROPAI_OAUTH_DIR: process.env.propai_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: PropAiSyncConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: PropAiSyncConfig) {
...
async function runStateIntegrityText(cfg: PropAiSyncConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "propai-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.propai_HOME = tempHome;
    process.env.propai_STATE_DIR = path.join(tempHome, ".propai");
    delete process.env.propai_OAUTH_DIR;
    fs.mkdirSync(process.env.propai_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when PROPAI_OAUTH_DIR is explicitly configured", async () => {
    process.env.propai_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: PropAiSyncConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints propai-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: PropAiSyncConfig = {};
    ...
    expect(text).toMatch(/PropAiSync sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/PropAiSync sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /PropAiSync sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
