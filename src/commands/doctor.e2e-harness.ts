...
export const resolvePropAiSyncPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadPropAiSyncPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolvePropAiSyncPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.propai_STATE_DIR;
  originalUpdateInProgress = process.env.propai_UPDATE_IN_PROGRESS;
  process.env.propai_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "propai-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.propai_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
