...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: PropAiSyncConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: PropAiSyncConfig) => { changed: boolean; next: PropAiSyncConfig };
  defaultModel: string;
  overrideConfig: PropAiSyncConfig;
  alreadyDefaultConfig: PropAiSyncConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: PropAiSyncConfig) => config,
      applyDefaultConfig: (config: PropAiSyncConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: PropAiSyncConfig = { agents: { defaults: {} } };
