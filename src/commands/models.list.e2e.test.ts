...
const ensurePropAiSyncModelsJson = vi.fn().mockResolvedValue(undefined);
const resolvePropAiSyncAgentDir = vi.fn().mockReturnValue("/tmp/propai-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensurePropAiSyncModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolvePropAiSyncAgentDir,
}));
...
beforeEach(() => {
  ...
  ensurePropAiSyncModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensurePropAiSyncModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensurePropAiSyncModelsJson).toHaveBeenCalled();
    expect(ensurePropAiSyncModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
