...
export async function setupAuthTestEnv(
  ...
  process.env.propai_AGENT_DIR = agentDir;
...
export function requirePropAiSyncAgentDir(): string {
  const agentDir = process.env.propai_AGENT_DIR;
  if (!agentDir) {
    throw new Error("PROPAI_AGENT_DIR not set");
  }
  return agentDir;
}
