...
import { writeConfigFile, type PropAiSyncConfig } from "../config/config.js";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service PROPAI_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (!tokenRefConfigured && !configuredGatewayToken && gatewayTokenForRepair) {
    const nextCfg: PropAiSyncConfig = {
      ...cfg,
      gateway: {
        ...cfg.gateway,
        auth: {
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing PropAi Sync gateway next.");
      }
    }
  }
