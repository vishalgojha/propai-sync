...
const TEMP_MEDIA_PREFIX = "propai-media-";
...
function createGroqAudioConfig(): PropAiSyncConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): PropAiSyncConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): PropAiSyncConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: PropAiSyncConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredPropAiSyncTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        PROPAI_AGENT_DIR: isolatedAgentDir,
