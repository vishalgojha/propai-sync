...
describe("buildSlackSlashCommandMatcher", () => {
  it("matches with or without a leading slash", () => {
    const matcher = buildSlackSlashCommandMatcher("PropAi Sync");

    expect(matcher.test("PropAi Sync")).toBe(true);
    expect(matcher.test("/PropAiSync")).toBe(true);
  });

  it("does not match similar names", () => {
    const matcher = buildSlackSlashCommandMatcher("PropAi Sync");

    expect(matcher.test("/propai-bot")).toBe(false);
    expect(matcher.test("propai-bot")).toBe(false);
  });
});
