...
// Prefix for propai-generated action IDs to scope our handler
const PROPAI_ACTION_PREFIX = "PropAi Sync:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${PROPAI_ACTION_PREFIX}`);

  // Handle PropAi Sync modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
