...
ZIP=${1:?"Usage: $0 propai-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/propai/propai/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^propai-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/propai/propai/releases/download/v${VERSION}/"}
