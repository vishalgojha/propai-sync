...
const EXPECTED_REPOSITORY_URL = "https://github.com/propai/propai";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "propai") {
    errors.push(`package.json name must be "propai"; found "${pkg.name ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`propai-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `propai-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
