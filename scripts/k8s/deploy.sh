#!/usr/bin/env bash
# Deploy PropAi Sync to Kubernetes.
#
...
#
# Environment:
#   PROPAI_NAMESPACE   Kubernetes namespace (default: PropAi Sync)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANIFESTS="$SCRIPT_DIR/manifests"
NS="${PROPAI_NAMESPACE:-PropAi Sync}"
...
Usage: ./scripts/k8s/deploy.sh [OPTION]
  ...
  (no args)        Deploy PropAi Sync (creates secret from env if needed)
...
_apply_secret() {
  ...
  if kubectl get secret propai-secrets -n "$NS" &>/dev/null; then
    EXISTING_SECRET=true
    EXISTING_TOKEN="$(kubectl get secret propai-secrets -n "$NS" -o jsonpath='{.data.propai_GATEWAY_TOKEN}' | base64 -d)"
    ANTHROPIC_VALUE="$(kubectl get secret propai-secrets -n "$NS" -o jsonpath='{.data.ANTHROPIC_API_KEY}' 2>/dev/null | base64 -d)"
    OPENAI_VALUE="$(kubectl get secret propai-secrets -n "$NS" -o jsonpath='{.data.OPENAI_API_KEY}' 2>/dev/null | base64 -d)"
    GEMINI_VALUE="$(kubectl get secret propai-secrets -n "$NS" -o jsonpath='{.data.GEMINI_API_KEY}' 2>/dev/null | base64 -d)"
    OPENROUTER_VALUE="$(kubectl get secret propai-secrets -n "$NS" -o jsonpath='{.data.OPENROUTER_API_KEY}' 2>/dev/null | base64 -d)"
  fi
  ...
  printf '%s' "$TOKEN" > "$TMP_DIR/PROPAI_GATEWAY_TOKEN"
  ...
  kubectl create secret generic propai-secrets \
    -n "$NS" \
    --from-file=PROPAI_GATEWAY_TOKEN="$TMP_DIR/PROPAI_GATEWAY_TOKEN" \
  ...
  kubectl apply --server-side --field-manager=PropAi Sync -f "$SECRET_MANIFEST" >/dev/null
  # Clean up any annotation left by older client-side apply runs.
  kubectl annotate secret propai-secrets -n "$NS" kubectl.kubernetes.io/last-applied-configuration- >/dev/null 2>&1 || true
  rm -rf "$TMP_DIR"
  trap - EXIT
  ...
  if $SHOW_TOKEN; then
    ...
    echo "  kubectl get secret propai-secrets -n $NS -o jsonpath='{.data.propai_GATEWAY_TOKEN}' | base64 -d && echo"
  fi
}
...
kubectl rollout restart deployment/PropAiSync -n "$NS" 2>/dev/null || true
echo ""
echo "Waiting for rollout..."
kubectl rollout status deployment/PropAiSync -n "$NS" --timeout=300s
echo ""
echo "Done. Access the gateway:"
echo "  kubectl port-forward svc/PropAiSync 18789:18789 -n $NS"
echo "  open http://localhost:18789"
echo ""
if $SHOW_TOKEN; then
  echo "Gateway token (paste into Control UI):"
  echo "  $(kubectl get secret propai-secrets -n "$NS" -o jsonpath='{.data.propai_GATEWAY_TOKEN}' | base64 -d)"
echo ""
fi
