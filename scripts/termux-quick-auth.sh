...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/propai/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "PropAi Sync Re-Auth" -c "After getting key, run: ssh $SERVER '~/propai/scripts/mobile-reauth.sh'" --id propai-auth
        ;;
    *)
