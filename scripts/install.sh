...
# PropAi Sync Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://propai.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one PropAi Sync."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${PROPAI_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 PropAi Sync Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://propai.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$PROPAI_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.propai.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_PROPAI_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.propai-* "$npm_root"/PropAiSync 2>/dev/null || true
}

extract_PROPAI_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_PROPAI_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/PropAiSync" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/PropAiSync"|"/usr/local/bin/PropAiSync")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/propai/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale PropAi Sync symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing PropAi Sync binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing PropAi Sync package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_PROPAI_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "PropAi Sync npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*PropAi Sync" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_PROPAI_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_PROPAI_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "PropAi Sync npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an PropAi Sync binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "PROPAI_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "PROPAI_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "PROPAI_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "PROPAI_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "PROPAI_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "PROPAI_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "PROPAI_BETA" "CLAWDBOT_BETA"
map_legacy_env "PROPAI_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "PROPAI_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "PROPAI_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "PROPAI_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "PROPAI_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "PROPAI_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${PROPAI_TAGLINE_INDEX:-}" ]]; then
        if [[ "${PROPAI_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((PROPAI_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${PROPAI_NO_ONBOARD:-0}
NO_PROMPT=${PROPAI_NO_PROMPT:-0}
DRY_RUN=${PROPAI_DRY_RUN:-0}
INSTALL_METHOD=${PROPAI_INSTALL_METHOD:-}
PROPAI_VERSION=${PROPAI_VERSION:-latest}
USE_BETA=${PROPAI_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/PropAiSync"
GIT_DIR=${PROPAI_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${PROPAI_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${PROPAI_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${PROPAI_VERBOSE:-0}"
PROPAI_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
PropAi Sync installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://propai.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/PropAiSync)
...
Environment variables:
  PROPAI_INSTALL_METHOD=git|npm
  PROPAI_VERSION=latest|next|<semver>
  PROPAI_BETA=0|1
  PROPAI_GIT_DIR=...
  PROPAI_GIT_UPDATE=0|1
  PROPAI_NO_PROMPT=1
  PROPAI_DRY_RUN=1
  PROPAI_NO_ONBOARD=1
  PROPAI_VERBOSE=1
  PROPAI_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://propai.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://propai.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://propai.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected PropAi Sync checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a PropAi Sync source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_PROPAI_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"PropAi Sync"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://propai.ai/install.sh | bash"
}
...
ensure_PROPAI_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/PropAiSync" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/PropAiSync" ]]; then
        ln -sf "$npm_root/propai/dist/entry.js" "${npm_bin}/PropAiSync"
        ui_info "Created PropAi Sync bin link at ${npm_bin}/PropAiSync"
    fi
    return 0
}

# Check for existing PropAi Sync installation
check_existing_PropAi Sync() {
    if [[ -n "$(type -P PropAi Sync 2>/dev/null || true)" ]]; then
        ui_info "Existing PropAi Sync installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make PropAi Sync show as \"command not found\" in new terminals."
...
warn_PROPAI_not_found() {
    ui_warn "Installed, but PropAi Sync is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t PropAi Sync 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named PropAi Sync; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_PROPAI_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P PropAi Sync 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/PropAiSync" ]]; then
        echo "${npm_bin}/PropAiSync"
        return 0
    fi
...
install_PROPAI_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/propai/propai.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing PropAi Sync from git checkout: ${repo_dir}"
    else
        ui_info "Installing PropAi Sync from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning PropAi Sync" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building PropAi Sync" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/PropAiSync" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/PropAiSync"
    ui_success "PropAi Sync wrapper installed to \$HOME/.local/bin/PropAiSync"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view propai dist-tags.beta 2>/dev/null || true)"
...
install_PropAi Sync() {
    local package_name="PropAi Sync"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            PROPAI_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="PropAi Sync"
        else
            PROPAI_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${PROPAI_VERSION}" ]]; then
        PROPAI_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${PROPAI_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing PropAi Sync v${resolved_version}"
    else
        ui_info "Installing PropAi Sync (${PROPAI_VERSION})"
    fi
    local install_spec=""
    if [[ "${PROPAI_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${PROPAI_VERSION}"
    fi

    if ! install_PROPAI_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_PROPAI_paths
        install_PROPAI_npm "${install_spec}"
    fi

    if [[ "${PROPAI_VERSION}" == "latest" && "${package_name}" == "PropAi Sync" ]]; then
        if ! resolve_PROPAI_bin &> /dev/null; then
            ui_warn "npm install propai@latest failed; retrying propai@next"
            cleanup_npm_PROPAI_paths
            install_PROPAI_npm "propai@next"
        fi
    fi

    ensure_PROPAI_bin_link || true

    ui_success "PropAi Sync installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${PROPAI_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_PROPAI_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (PropAi Sync not on PATH yet)"
        warn_PROPAI_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${PROPAI_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.propai/workspace-${profile}"
    else
        echo "${HOME}/.propai/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${PROPAI_CONFIG_PATH:-$HOME/.propai/propai.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run PropAi Sync onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but PropAi Sync not on PATH; skipping onboarding"
        warn_PROPAI_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run PropAi Sync onboard to retry"
        return
    }
}
...
if ! declare -F extract_PROPAI_semver >/dev/null 2>&1; then
...
resolve_PROPAI_version() {
    local version=""
    local raw_version_output=""
    local claw="${PROPAI_BIN:-}"
    if [[ -z "$claw" ]] && command -v PropAi Sync &> /dev/null; then
        claw="$(command -v PropAi Sync)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_PROPAI_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/propai/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/propai/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_PROPAI_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found PropAi Sync checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set PROPAI_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_PropAi Sync; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing PropAi Sync"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g PropAi Sync &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g PropAi Sync 2>/dev/null || true
        ...
        install_PROPAI_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/PropAiSync" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/PropAiSync"
    ...
    PROPAI_BIN="$(resolve_PROPAI_bin || true)"
    ...
    installed_version=$(resolve_PROPAI_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 PropAi Sync installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 PropAi Sync installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/PropAiSync"
        ui_kv "Update command" "PropAi Sync update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://propai.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running PropAi Sync doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                PROPAI_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                PROPAI_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                PROPAI_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run PropAi Sync doctor and PropAi Sync plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run PropAi Sync onboard later"
        else
            local config_path="${PROPAI_CONFIG_PATH:-$HOME/.propai/propai.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (PropAi Sync not on PATH yet)"
                    warn_PROPAI_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run PropAi Sync onboard to finish setup"
            return 0
        fi
    fi

    if command -v PropAi Sync &> /dev/null; then
        local claw="${PROPAI_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_PROPAI_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (PropAi Sync daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if PROPAI_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: PropAi Sync daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${PROPAI_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
