...
function stripWorkspacePropAiSyncDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.propai !== "workspace:*") {
    return false;
  }
  delete devDeps.propai;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspacePropAiSyncDevDependency(pkg);
