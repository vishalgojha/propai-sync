...
SMOKE_PREVIOUS_VERSION="${PROPAI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${PROPAI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (PROPAI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${PROPAI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${PROPAI_INSTALL_LATEST_OUT:-}"
fi
