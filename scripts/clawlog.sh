...
show_usage() {
    cat << EOF
clawlog - PropAi Sync Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View PropAi Sync logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    PropAi Sync logs flow through the macOS unified log (subsystem: ai.propai).
