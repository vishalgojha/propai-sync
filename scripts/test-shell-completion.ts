 ...
 * `PropAi Sync update`. Use it to verify the completion installation flow
...
\`PropAi Sync update\`, \`PropAi Sync doctor\`, and \`PropAi Sync onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# PropAi Sync Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`PropAi Sync completion --install\` later to enable.`));
    return;
  }
