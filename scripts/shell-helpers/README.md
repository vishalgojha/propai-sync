...
Inspired by Simon Willison's [Running PropAi Sync in Docker](https://til.simonwillison.net/llms/propai-docker).
...
On first command, ClawDock auto-detects your PropAi Sync directory:

- Checks common paths (`~/PropAiSync`, `~/workspace/PropAiSync`, etc.)
...
| `clawdock-cli <command>`  | Run PropAi Sync CLI commands                      |
...
| `clawdock-cd`        | Jump to the PropAi Sync project directory    |
| `clawdock-config`    | Open the PropAi Sync config directory        |
...
```bash
PropAi Sync channels login --channel whatsapp --verbose
```
...
2. Configure it in the PropAi Sync config
...
- Bash or Zsh shell
- PropAi Sync project (from `docker-setup.sh`)
