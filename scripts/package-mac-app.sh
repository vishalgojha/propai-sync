...
# Build and bundle PropAi Sync into a minimal .app we can open.
# Outputs to dist/PropAiSync.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/PropAiSync.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="PropAi Sync"
BUNDLE_ID="${BUNDLE_ID:-ai.propai.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/propai/propai/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/propai/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :PropAi SyncBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :PropAi SyncGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/PropAiSync"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/PropAiSync"
fi
chmod +x "$APP_ROOT/Contents/MacOS/PropAiSync"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/PropAiSync" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/propai/Resources/PropAiSync.icns" "$APP_ROOT/Contents/Resources/PropAiSync.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/propai/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying PropAi SyncKit resources"
PropAi SyncKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/PropAiSyncKit_PropAi SyncKit.bundle"
if [ -d "$PropAi SyncKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/PropAiSyncKit_PropAi SyncKit.bundle"
  cp -R "$PropAi SyncKIT_BUNDLE" "$APP_ROOT/Contents/Resources/PropAiSyncKit_PropAi SyncKit.bundle"
else
  echo "WARN: PropAi SyncKit resource bundle not found at $PropAi SyncKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running PropAi Sync"
killall -q PropAi Sync 2>/dev/null || true
