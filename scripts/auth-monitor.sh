...
#
# Suggested cron: */30 * * * * /home/admin/propai/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send PropAi Sync notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., propai-alerts)
...
STATE_FILE="$HOME/.propai/auth-monitor-state"
...
send_notification() {
    ...
    # Send via PropAi Sync if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use PropAi Sync
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via PropAi Sync to $NOTIFY_PHONE..."
            PropAi Sync send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: PropAi Sync Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! PropAi Sync is down. Run: ssh l36 '~/propai/scripts/mobile-reauth.sh'" "urgent"
    exit 1
