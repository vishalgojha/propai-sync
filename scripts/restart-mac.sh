#!/usr/bin/env bash
# Reset PropAi Sync like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${PROPAI_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="PropAi Sync.app/Contents/MacOS/PropAiSync"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/PropAiSync"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/PropAiSync"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/PropAiSync"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.propai.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/propai-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${PROPAI_RESTART_LOG:-/tmp/propai-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${PROPAI_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.propai/disable-launchagent"
ATTACH_ONLY=1
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.propai.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing PropAi Sync instances"
kill_all_PropAi Sync
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product PropAi Sync"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/PropAiSync.app" ]]; then
    APP_BUNDLE="/Applications/PropAiSync.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/PropAiSync.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/PropAiSync.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/PropAiSync.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set PROPAI_APP_BUNDLE to your installed PropAi Sync.app"
}
