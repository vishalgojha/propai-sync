...
type DriverMode = "token" | "webhook" | "PropAi Sync";
...
function resolveStateDir(): string {
  const override = process.env.propai_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.propai_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".propai");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver PropAi Sync] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) PropAi Sync spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|PropAi Sync> Driver transport mode (default: token)\n" +
    ...
    "  --propai-bin <path>        PropAi Sync CLI binary for driver=PropAi Sync (default: PropAi Sync)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  PROPAI_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  PROPAI_DISCORD_SMOKE_DRIVER\n" +
    "  PROPAI_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  PROPAI_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  PROPAI_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  PROPAI_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  PROPAI_DISCORD_SMOKE_AGENT\n" +
    "  PROPAI_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  PROPAI_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  PROPAI_DISCORD_SMOKE_POLL_MS\n" +
    "  PROPAI_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  PROPAI_DISCORD_SMOKE_PROPAI_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.propai_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.propai_DISCORD_SMOKE_DRIVER ||
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "PropAi Sync"
        ? "PropAi Sync"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.propai_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.propai_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.propai_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.propai_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.propai_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.propai_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.propai_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.propai_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.propai_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.propai_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const PropAiSyncBin =
    resolveArg("--propai-bin") || process.env.propai_DISCORD_SMOKE_PROPAI_BIN || "PropAi Sync";
...
async function PropAiSyncCliJson<T>(params: { PropAiSyncBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.propaiBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`PropAi Sync ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithPropAiSync(params: {
  PropAiSyncBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await PropAiSyncCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    PropAiSyncBin: params.propaiBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "PropAi Sync") {
    return await readMessagesWithPropAiSync({
      PropAiSyncBin: params.args.propaiBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `propai-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await PropAiSyncCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("PropAi Sync message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "PropAi Sync"
            ? await readMessagesWithPropAiSync({
                PropAiSyncBin: args.propaiBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from PropAiSync.`,
        diagnostics: {
          bindingCandidates: [
            {
