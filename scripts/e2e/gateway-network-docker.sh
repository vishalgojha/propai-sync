...
IMAGE_NAME="propai-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="propai-net-e2e-$$"
GW_NAME="propai-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "PROPAI_GATEWAY_TOKEN=$TOKEN" \
  -e "PROPAI_SKIP_CHANNELS=1" \
  -e "PROPAI_SKIP_GMAIL_WATCHER=1" \
